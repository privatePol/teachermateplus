# TeacherMate+ Production Data Promotion Guide

This guide explains how to safely promote your existing TeacherMate+ data from your current local environment into staging and production.
...
From `D:\teachermateplus`, use the provided helper script.
...
- [export_data_bundle.ps1](/d:/teachermateplus/ops/scripts/export_data_bundle.ps1)
...
source /etc/teachermateplus-staging/teachermateplus.env
/opt/teachermateplus-staging/.venv/bin/python manage.py migrate --noinput
/opt/teachermateplus-staging/.venv/bin/python manage.py collectstatic --noinput
/opt/teachermateplus-staging/.venv/bin/python manage.py check
```
...
- [import_data_bundle.sh](/d:/teachermateplus/ops/scripts/import_data_bundle.sh)

Example:

```bash
cd /opt/teachermateplus-staging
bash ./ops/scripts/import_data_bundle.sh /etc/teachermateplus-staging/teachermateplus.env /tmp/setup_bundle.json
```
...
```bash
cd /opt/teachermateplus-staging
bash ./ops/scripts/import_data_bundle.sh /etc/teachermateplus-staging/teachermateplus.env /tmp/setup_bundle.json --flush
```
...
bash ./ops/scripts/import_data_bundle.sh /etc/teachermateplus/teachermateplus.env /tmp/setup_bundle.json
```
...
```bash
cd /opt/teachermateplus
source /etc/teachermateplus/teachermateplus.env
/opt/teachermateplus/.venv/bin/python manage.py migrate --noinput
/opt/teachermateplus/.venv/bin/python manage.py check
```
...
```bash
mysqldump -u TeacherMate+_staging_user -p --databases TeacherMate+_staging > TeacherMate+_staging_backup.sql
```
