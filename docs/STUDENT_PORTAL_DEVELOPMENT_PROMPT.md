...
````text
You are Codex working in the EduGradesPro V1 repository.

Goal:
Build the initial Student Portal foundation for EduGradesPro as a local-only development stage. The portal should let students securely log in and view only their own tenant-owned records. The first implementation should be conservative, read-only, tenant-scoped, and easy to review.
...
- Update `CHANGE_LOG.md`, `EDUGRADESPRO_CONTEXT.md`, and relevant docs when behavior changes.
...
5. EduGradesPro shows the email destination for invitation/activation.
6. Admin confirms provisioning.
7. EduGradesPro creates or links the `User` account.
8. EduGradesPro creates an active `StudentAccountLink`.
9. EduGradesPro sends an activation link or temporary credential to the registered student email if email sending is enabled.
