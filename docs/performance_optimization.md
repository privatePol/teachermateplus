# EduGradesPro Performance Optimization Notes
...
EduGradesPro already has several production-friendly patterns in place:
...
- If rows are missing for enrolled students, EduGradesPro recomputes only the missing student rows for that offering and period.
...
DJANGO_CACHE_LOCATION=edugradespro-local
```
...
EduGradesPro uses Django static files. Production should serve static files through nginx or equivalent, and `collectstatic` remains part of deployment.
