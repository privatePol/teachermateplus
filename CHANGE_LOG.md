...
- Added Phase 1 `Exit Pulse` under Faculty Portal Classroom Tools. Assigned faculty can launch a five-minute anonymous Learning Check for an accepted active class, share a secure public link or dynamic QR code, poll the live response count, extend once, close/cancel, and review owner-only aggregate results plus optional anonymous written feedback after closure or expiry. The feature includes tenant/campus and assignment ownership enforcement, `exit_pulse.use`, a tenant feature switch, signed-browser duplicate control, public rate limits, server-side expiry, faculty-action audits, rule-based custom-question safeguards, and no attendance, grading, faculty-evaluation, history, chart, export, or administrator-reporting behavior.
