...
- Expanded the active role-based Admin Help Guide with exact `Where to start` menu paths and numbered page-opening instructions for dashboard scope, academic setup, enrollment/faculty assignment, grade formula setup, course-template assignment, submissions, reopen requests, corrections, reports, accounts, and Superadmin controls.
- Added a permission-filtered Admin Help Guide section for changing a published grading template through the Hotfix workflow, including apply-mode selection, impact review, configured approval steps, `APPLY HOTFIX` confirmation, eligible recomputation, skipped submitted offerings, and the required Correction of Grades route for official submitted grades.
...
- Admin Help Guide wording no longer implies that ordinary periodic gradebooks have separate Approved or Posted statuses. The guide accurately describes the current `Draft`, `Submitted`, and `Reopened` states while keeping template, reopen-request, and correction approvals separate.
