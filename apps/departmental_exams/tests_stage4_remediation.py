...
class CAOPropagationExclusionTests(Stage4TestCase):
    ...
    def test_contribution_and_question_activity_exclude_draft_rows_from_default_rewrite(self):
        ...
        FacultyContribution.objects.create(cycle_course=contribution_parent, faculty_user=self.admin, source_assignment=assignment_one, source_campus=self.campus, quota_snapshot=contribution_configuration.questions_required_per_faculty, configuration_revision_snapshot=contribution_configuration.revision)
        question_contribution = FacultyContribution.objects.create(cycle_course=question_parent, faculty_user=self.admin, source_assignment=assignment_two, source_campus=self.campus, quota_snapshot=question_configuration.questions_required_per_faculty, configuration_revision_snapshot=question_configuration.revision)
        Question.objects.create(contribution=question_contribution, question_text="Which value is retained?", choice_a="A", choice_b="B", choice_c="C", choice_d="D", correct_answer="A", difficulty=Question.Difficulty.EASY, position=1)
