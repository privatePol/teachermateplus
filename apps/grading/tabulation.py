...
class PeriodSummaryLayoutService:
    """Build the shared read-only hierarchy and display values used by Summary reports."""
    ...
    @classmethod
    def build_layout(cls, period, activities):
        components = list(
        ...
        for activity in activities:
            if not activity.template_component.is_active:
                continue
            if activity.template_subcomponent and not activity.template_subcomponent.is_active:
                continue
            if activity.template_detail and not activity.template_detail.is_active:
                continue
        ...
        for component in components:
            ...
            subcomponents = [subcomponent for subcomponent in component.subcomponents.all() if subcomponent.is_active]
            component_layout = {
                ...
                "weight_percentage": Decimal(component.weight_percentage or 0),
                "colspan": 1,
            }
            if subcomponents:
                for subcomponent in subcomponents:
                    ...
                    for detail in [detail for detail in subcomponent.details.all() if detail.is_active]:
                        detail_activities = sorted(
                            ...
                            key=lambda activity: (
                                cls.activity_sort_key(activity.title),
                        ...
                        component_layout["sections"].append(
                            {
                                ...
                                "color_class": cls.section_color_class(subcomponent.name or subcomponent.code),
                                "uses_nested": True,
                                ...
                                "colspan": sum(group["colspan"] for group in visible_groups) + 1,
                            }
                        )
                    else:
                        ordered = sorted(
                ...
                component_layout["sections"].append(
                    {
                        "id": component.id,
                        "label": component.name.upper(),
                        "color_class": cls.section_color_class(component.name or component.code),
                        "uses_nested": False,
    ...
    @classmethod
    def build_row_values(cls, row, layout, numeric_score_map):
            ...
            for section in block["sections"]:
                if section["uses_nested"]:
                    ...
                    for group in section["groups"]:
                        ...
                        section_values["groups"].append({"activity_values": values, "average": average})
                    if section.get("detail_computation_mode") == "AVERAGE_ACTIVITIES":
                        section_score = cls.average(section_activity_values) or Decimal("0")
                    else:
                    ...
                    block_values["sections"].append(
                        {"uses_nested": False, "activity_values": values, "average": average}
                    )
        ...
        for component in layout["exam_components"]:
            ...
            if exam_value is None:
                    ...
                    if section["uses_nested"]:
                        ...
                        if nested_has_data:
                            section_scores.append(
                                cls.average(section_activity_values)
                                if section.get("detail_computation_mode") == "AVERAGE_ACTIVITIES"
                ...
                exam_value = section_scores[0] if len(section_scores) == 1 else cls.average(section_scores)
            exam_values.append(exam_value)
        return {"class_standing_blocks": class_standing_blocks, "exam_values": exam_values}

    @staticmethod
    def print_group_columns(layout, *, quizzes):
        columns = []
        for block in layout["class_standing_blocks"]:
            for section in block["sections"]:
                is_quiz = section["color_class"] == "summary-group-quizzes"
                if is_quiz != quizzes:
                    continue
                if section["uses_nested"]:
                    for group in section["groups"]:
                        columns.extend(group["activity_columns"])
                else:
                    columns.extend(section["activity_columns"])
        return columns

    @classmethod
    def print_group_average(cls, layout, row_values, *, quizzes):
        block_results = []
        for block_index, block in enumerate(layout["class_standing_blocks"]):
            ...
            for section_index, section in enumerate(block["sections"]):
                is_quiz = section["color_class"] == "summary-group-quizzes"
                if is_quiz == quizzes:
                    matching_sections.append((section, row_values["class_standing_blocks"][block_index]["sections"][section_index]))
            if not matching_sections:
                continue
            section_weight_total = sum(section["weight_percentage"] for section, _values in matching_sections) or Decimal("100")
            block_numeric = Decimal("0")
            block_has_data = False
            for section, values in matching_sections:
                average = values.get("average")
                block_has_data = block_has_data or average is not None
                block_numeric += (section["weight_percentage"] / section_weight_total) * (average or Decimal("0"))
            block_results.append(
                {
                    "average": FacultyGradingService._round(block_numeric) if block_has_data else None,
                    "weight_percentage": block["weight_percentage"],
                }
            )
        if not block_results or not any(result["average"] is not None for result in block_results):
            return None
        block_weight_total = sum(result["weight_percentage"] for result in block_results) or Decimal("100")
        combined = sum(
            (result["weight_percentage"] / block_weight_total) * (result["average"] or Decimal("0"))
            for result in block_results
        )
        return FacultyGradingService._round(combined)


class PeriodicGradePrintDataService:
    """Build a bounded-query, read-only periodic-grade HTML report payload."""
    ...
    def _decimal_display(value):
        if value in (None, ""):
            return PeriodicGradePrintDataService.EM_DASH
        try:
    ...
    def _score_display(cls, score):
        ...
        if score.is_excused:
            return "EXEMPT"
        return f"{cls._decimal_display(score.raw_score)}/{cls._decimal_display(score.computed_score)}"

    @classmethod
    def build(cls, *, offering, period, enrollments, is_final_period, grade_label, final_exam_label):
        activities = list(
        ...
        layout = PeriodSummaryLayoutService.build_layout(period, activities)
        score_rows = list(
            StudentActivityScore.objects.filter(
                activity_id__in=[activity.id for activity in activities],
                activity__is_active=True,
                is_active=True,
            ).only("student_id", "activity_id", "raw_score", "computed_score", "is_excused")
        )
        ...
        period_grade_map = {
            row.student_id: row
            for row in StudentPeriodGrade.objects.filter(offering=offering, template_period=period).only(
                "student_id", "period_grade", "class_standing_grade"
            )
        }
        final_grade_map = (
            {
                row.student_id: row.final_grade
                for row in StudentFinalGrade.objects.filter(offering=offering).only("student_id", "final_grade")
            }
            if is_final_period
            else {}
        )
        participation_columns = PeriodSummaryLayoutService.print_group_columns(layout, quizzes=False)
        quiz_columns = PeriodSummaryLayoutService.print_group_columns(layout, quizzes=True)
        rows = []
        for number, enrollment in enumerate(enrollments, start=1):
            grade_row = period_grade_map.get(enrollment.student_id)
            row_values = PeriodSummaryLayoutService.build_row_values(
                {"student": enrollment.student, "component_scores": {}},
                layout,
                numeric_score_map,
            )
            exam_value = (
                grade_row.period_grade if is_final_period and grade_row else
                row_values["exam_values"][0] if row_values["exam_values"] else None
            )
            official_grade = final_grade_map.get(enrollment.student_id) if is_final_period else (
                grade_row.period_grade if grade_row else None
            )
            rows.append(
                {
                    "number": number,
                    "student_name": f"{enrollment.student.last_name}, {enrollment.student.first_name}",
                    "official_grade": cls._official_grade_display(official_grade),
                    "participation_values": [
                        cls._score_display(score_map.get((enrollment.student_id, column["id"])))
                        for column in participation_columns
                    ],
                    "participation_average": cls._decimal_display(
                        PeriodSummaryLayoutService.print_group_average(layout, row_values, quizzes=False)
                    ),
                    "quiz_values": [
                        cls._score_display(score_map.get((enrollment.student_id, column["id"])))
                        for column in quiz_columns
                    ],
                    "quiz_average": cls._decimal_display(
                        PeriodSummaryLayoutService.print_group_average(layout, row_values, quizzes=True)
                    ),
                    "class_standing_average": cls._decimal_display(
                        grade_row.class_standing_grade if grade_row else None
                    ),
                    "exam_grade": cls._decimal_display(exam_value),
                }
            )
        default_exam_label = f"{(period.name or period.code or 'PERIOD').strip().upper()} EXAM"
        return {
            "layout": layout,
            "participation_columns": participation_columns,
            "participation_colspan": len(participation_columns) + 1,
            "quiz_columns": quiz_columns,
            "quiz_colspan": len(quiz_columns) + 1,
            "total_columns": 7 + len(participation_columns) + len(quiz_columns),
            "grade_label": grade_label,
            "exam_label": (
                final_exam_label
                if is_final_period
                else layout["exam_components"][0]["label"] if layout["exam_components"]
                else default_exam_label
            ),
            "rows": rows,
        }
