        ...
        "topics": [
            {
                ...
                "actions": [
                    ...
                    {"name": "Automatic Generation Readiness", "does": "Opens the same seven-column management report on screen and print: sequential row number, course, offered campuses, narrow Final items, monitoring Contribution Progress, one generation status, and bulleted actions.", "when": "Use it before or after the deadline. Progress compares Submitted question volume with effective quota times distinct offered campuses; it is monitoring only, not the generation minimum. The end-of-report summary counts only the currently rendered authorized rows by their displayed status. Initialize a roster labeled not initialized; Synchronize only an initialized roster whose eligible Faculty list changed.", "avoid": "Do not treat progress or the status summary as a separate readiness calculation. The generator still applies unique-question, 30/50/20 difficulty, campus-policy, and exact Set A/B checks internally.", "result": "One of READY FOR GENERATION, WAITING FOR DEADLINE, NEEDS QUESTIONS, FACULTY INCOMPLETE, BLOCKED, GENERATED, or EXEMPT, plus concise action bullets and matching summary counts.", "editable": "Both screen and print are GET-only, aggregate-only, and never start generation."},
