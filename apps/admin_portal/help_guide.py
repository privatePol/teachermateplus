        ...
        "topics": [
            {
                ...
                "actions": [
                    ...
                    {"name": "Print All Draft Contributions", "does": "Prints every Draft contribution in one selected authorized cycle, grouped and numbered by sorted course, then adds a numbered Draft Summary by Faculty with distinct course-contribution counts.", "when": "Use it for cycle-wide Draft follow-up without relying on the current browser rows.", "avoid": "Submitted and Exempt records are excluded, and no question or answer content is retrieved for display.", "result": "Courses are ordered by code/title; the faculty footer is ordered case-insensitively by last name then first name.", "editable": "The report is GET-only and read-only."},
                    {"name": "Print Faculty Submission Summary", "does": "Prints a numbered faculty summary for one selected cycle and the effective Default Campus, counting distinct Draft and Submitted course contributions.", "when": "Use it to compare submission totals inside the current tenant, campus, and exact exam-department authority.", "avoid": "Do not treat Courses as question count or broaden a missing Default Campus to all campuses.", "result": "Each faculty row shows Draft, Submitted, and Courses totals; Exempt, other-campus, and unauthorized records are omitted.", "editable": "The report is GET-only and read-only."},
