        ...
        "topics": [
            ...
            },
            {
                ...
                "steps": [
                    ...
                    "Use the Release Center tabs to keep Questionnaire Releases and Answer Key Releases separate. In Bulk Answer Key Release, filter the already-authorized rows by course code/name, responsible/Exam Department, examination-unit campus, and live release status. Select All Visible changes only visible, filtered, eligible, enabled rows. Changing any filter automatically deselects rows that become hidden or unavailable while preserving selected rows that remain visible.",
                    "For several courses, Bulk Answer Key Release preserves each displayed primary-owned course/revision pair. Review the selected and visible-eligible counts, apply one common Available From/Until window, and confirm that all sessions for all selected courses concluded. AJAX preserves the active tab, filters, and scroll position while the existing server revalidates the complete batch; ordinary form submission remains available without JavaScript. Any stale, incomplete, unauthorized, or superseded row rolls back the complete batch; an identical retry reuses the active release without artificial history.",
