        ...
        "topics": [
            ...
            },
            {
                ...
                "steps": [
                    ...
                    "Use the Release Center tabs to keep Questionnaire Releases and Answer Key Releases separate. In Bulk Answer Key Release, first select an explicit Target Campus and load its recipient courses. The course code/name search, responsible/Exam Department, and Release Status controls only filter those rows. Changing Target Campus clears course selections; another campus requires a separate explicit release. Select All Visible changes only visible, filtered, eligible, enabled rows. Changing any filter automatically deselects rows that become hidden or unavailable while preserving selected rows that remain visible.",
                    "For several courses, Bulk Answer Key Release preserves each displayed target campus, recipient course, and primary-owned exact revision. Equivalent member courses require their own selection; selecting one member never releases to every equivalent course. Review the selected and visible-eligible counts, apply one common Available From/Until window, and confirm that all sessions for all selected courses concluded. AJAX preserves the active tab, filters, and scroll position while the existing server revalidates the complete batch; ordinary form submission remains available without JavaScript. Any stale, incomplete, unauthorized, or superseded row rolls back the complete batch; an identical retry reuses the active release without artificial history.",
