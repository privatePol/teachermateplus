        ...
        "topics": [
            {
                ...
                "how_to_open": ["Open Departmental Exam Builder in the Admin Portal.", "Choose Faculty Contribution Status.", "For new Automatic cycles: configure cycle defaults, open the cycle, then use Manage Course Exams and choose Prepare Faculty Contributions to select courses, review opening requirements and open the confirmed selection. Configuration Ready does not confirm opening eligibility.", "Standardized exams use No Sections automatically. Explicitly check Departmental Exam for an eligible Draft blueprint. Enable Structured Case/Scenario Exam Lifecycle for Departmental Automatic sections and Cases; configure exact section quotas before first Open. Recommend a common Linked MCQ count per Case that divides its section quota (20 items can use four 5-question Cases); mixed whole Cases and standalone MCQs are supported without a mandatory Case size.", "Existing exams remain classification-unconfirmed until explicitly classified. First Open freezes classification. Open cycles are the default; use Draft setup or Past / Closed for other cycles. Exam Readiness remains the pre-cycle assignment report, not a cycle-status list."],
        ...
        "topics": [
            {
                ...
                "steps": [
                    ...
                    "Use Manage Course Exams to review only courses within your exact current exam responsibility, then choose Prepare Faculty Contributions to check opening requirements and open eligible courses. Configuration Ready does not confirm opening eligibility. One grouped examination is shared across its listed campuses, including Cubao, Fairview, and Taytay where applicable; it is not one examination per campus. Use PRINT ALL for the complete authorized course-code-sorted listing in the selected Open, Draft or Past/Closed cycle status with cycle, responsibility, configuration, inclusion, and readiness context but no action controls. Use Print Exempt Courses for the authoritative Exempt-only reconciliation list with category and reason. Neither report changes course or exemption state.",
                    "For historical Manual Review, use Administer to confirm the one responsible exam department and reviewer; a null or inactive responsible department remains fail-closed. New Automatic setup does not require these assignments, but still requires the existing tenant/campus permissions. Responsibility is not assigned separately per campus.",
                    ...
                    "On Course Examination Configuration, authorized exact-responsibility users may inherit each cycle default or set an intentional course override, then record coverage and optional instructions. Effective values show DEFAULT, OVERRIDE, or NOT CONFIGURED. Removing a deadline override restores the cycle deadline, or NOT CONFIGURED when the cycle has none. When the tenant enables Structured Case/Scenario Exam Lifecycle, use Configure Exam Structure / Blueprint while every equivalent-unit member is still Draft and has never opened; the first Open permanently freezes mode, section titles, order, and exact quotas. Eligible Faculty may create Word-copy/paste Case-Based Question Groups against that frozen structure in Manual Review or Departmental Automatic mode. Automatic generation selects whole Cases, preserves Linked MCQ order, and prints immutable rich narratives and section headings. Invalid Cases are excluded as whole units with warnings; remaining units must meet exact item, section and configured campus quotas. Difficulty is preferred, with target versus actual reported.",
