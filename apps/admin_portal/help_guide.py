        ...
        "topics": [
            {
                ...
                "steps": [
                    ...
                    "From Faculty Contribution Status, open Automatic Generation Readiness and select one authorized cycle. Review the seven shared screen/print columns, beginning with sequential No. and including monitoring-only Submitted volume over effective quota times distinct offered campuses, one Automatic Exam Generation Status, and bulleted Action Needed. The Report Summary counts only rendered authorized rows by that existing displayed status. Initialize a roster labeled not initialized; Synchronize only an initialized roster whose eligible Faculty list changed while intake is Open and before its effective deadline. For Closed intake, ask an authorized administrator to review the lifecycle. After expiry, existing contributions remain preserved; prepare a fresh cycle for revised preparation. The report works before the deadline and never starts processing or exposes confidential questions.",
                    "The scheduled Automatic deadline processor isolates each grouped examination in its own time-bounded process. A time limit displays as Automatic generation exceeded the per-course processing time limit and requires administrator review; the batch continues with later courses and preserves that Closed terminal result. Reopening and Restore are unavailable after the effective deadline; review technical failures separately without reopening intake.",
                    "Automatic SUFFICIENT_POOL excludes unresolved Blocked Drafts with warnings and uses only eligible Submitted questions. Strict contributor policy and Manual behavior still require resolution. Every section must satisfy its exact quota using whole Cases and standalone questions; one section's surplus cannot fill another's shortage. After the deadline, preserve the record and prepare a new cycle if further contributions are needed.",
                    "Only before the existing effective deadline, Reopen Contributions may let the original owner finish an existing unfinished Draft when its retained source has durable proof of prior eligibility even though the assignment, offering, or roster evidence is now historical. A future replacement deadline cannot authorize reopening an already expired intake. The workspace is labeled Reopened Draft and remains editable only until the replacement deadline; reopening does not reactivate or rewrite those historical records. Submitted contributions remain immutable, and a current generation anywhere in an equivalency group blocks Draft mutation.",
