        ...
        "topics": [
            {
                ...
                "how_to_open": ["Open Departmental Exam Builder in the Admin Portal.", "Choose Faculty Contribution Status.", "For new Automatic cycles follow: Cycle defaults, Course configuration, Optional exam structure, Prepare/open contributions, Monitor submissions, Generation status, then Release/print. Prepare Faculty Contributions remains the cycle-level bulk action. It selects only visible Ready rows, and filtering clears hidden selections before the signed review. Configuration Ready does not confirm opening eligibility.", "DEPTAL is a classification, not a readiness result. With Structured Case/Scenario Exam Lifecycle enabled, an eligible Automatic Included DEPTAL unit with no prior blueprint or history displays No Sections — default (created on Open). Readiness GETs do not save it; authorized Open creates and freezes it atomically. Configurators may instead save optional sections before Open. No Sections continues to allow Cases and standalone questions in the implicit section; no standalone-only restriction is introduced.", "Existing exams remain classification-unconfirmed until explicitly classified. First Open freezes classification. Open cycles are the default; use Draft setup or Past / Closed for other cycles. Exam Readiness remains the pre-cycle assignment report, not a cycle-status list."],
                "check_first": ["The governing cycle and course contribution must be Open.", "Only exact course configurators may initialize or synchronize; an assigned reviewer receives monitoring only.", "Manage Cycles alone does not grant contributor monitoring.", "Duplicate prevention covers the primary grouped course and equivalent members in this cycle. Only accepted questions use quota; a duplicate-free count does not prove section/campus/whole-Case feasibility. Under Automatic Sufficient Pool, Blocked Drafts release reservations while their content remains stored; valid reactivation must reacquire identities and may stop on a conflict. Legacy collision preflight is read-only and reports only record IDs/counts; resolve actual collisions explicitly without rewriting Submitted history."],
                "actions": [
                    {"name": "Enable contribution deadline email reminders", "does": "Enables a private, consolidated Question Bank email per eligible faculty member and Manila deadline date.", "when": "In Configurable Features, enable the tenant setting after Departmental Exam Builder is enabled; schedule the daily command only through a separately approved operations gate.", "avoid": "Do not treat the setting as proof that scheduling or SMTP is active. The email omits course and campus details and never changes contributor status.", "result": "When scheduled, eligible faculty with pending editable Drafts are reminded at or after 9:00 a.m. on the preceding Manila calendar day.", "editable": "Only authorized Configurable Features managers may change the setting."},
                    {"name": "Enable Reuse My Previous Questions", "does": "In Configurable Features, turn on Reuse My Previous Questions for the tenant after the Departmental Exam Builder is enabled.", "when": "Use the authorized settings page when faculty should copy their own earlier Submitted questions into an editable Draft of the exact same course.", "avoid": "This setting does not share Cases or permit copying another contributor's questions. Faculty still need current destination and source-campus permissions.", "result": "Eligible faculty see the reuse link in an editable Question Bank Draft; disabling the setting hides the link and denies direct reuse requests.", "editable": "Only users already authorized to manage Configurable Features may change this tenant setting."},
                    ...
                    {"name": "Print All Draft Contributions", "does": "Prints every Draft contribution in one selected authorized cycle, grouped and numbered by sorted course, then adds a numbered three-column Draft Summary by Faculty with distinct course-contribution counts.", "when": "Use it for cycle-wide Draft follow-up without relying on the current browser rows.", "avoid": "Submitted and Exempt records are excluded, and no question or answer content is retrieved for display.", "result": "Courses are ordered by code/title; the non-splitting faculty footer is ordered case-insensitively by last name then first name and collapses responsively on narrower screens.", "editable": "The report is GET-only and read-only."},
                    {"name": "Print Faculty Submission Summary", "does": "Prints a numbered faculty summary for one selected cycle across the viewer's complete authorized Contributor Monitoring campus scope, counting distinct Draft and Submitted course contributions.", "when": "Use it to compare submission totals inside the current tenant and exact campus/exam-department authority.", "avoid": "Do not treat a contributor's displayed Default Campus as a report filter or treat Courses as question count.", "result": "Each faculty row shows Default Campus (or Not set), Draft, Submitted, and Courses totals; Exempt, out-of-scope campus, and unauthorized records are omitted.", "editable": "The report is GET-only and read-only."},
                    {"name": "Automatic Generation Readiness", "does": "Opens the same seven-column management report on screen and print: sequential row number, course, offered campuses, narrow Final items, monitoring Contribution Progress, one generation status, and bulleted actions.", "when": "Use it before or after the deadline. Progress compares Submitted question volume with effective quota times distinct offered campuses; it is monitoring only, not the generation minimum. The end-of-report summary counts only the currently rendered authorized rows by their displayed status. Initialize a roster labeled not initialized; Synchronize only an initialized roster whose eligible Faculty list changed while intake is Open and before its effective deadline. For Closed or expired intake, an authorized Automatic generation manager may open a reasoned correction window with a future deadline; selected Submitted contributors receive a new Draft while their original submission remains historical.", "avoid": "Do not treat progress, aggregate sufficiency, or the status summary as proof that exact Set A/B generation is feasible. The report does not run the exact solver; authoritative generation still applies unique-question, campus-policy, minimum-overlap, exact item-count, and Set A/B checks. The 30/50/20 difficulty mix is a preferred target and a mismatch alone does not block Automatic generation.", "result": "One of EXACT FEASIBILITY PENDING, WAITING FOR DEADLINE, NEEDS QUESTIONS, FACULTY INCOMPLETE, BLOCKED, GENERATED, or EXEMPT, plus concise action bullets and matching summary counts. Generated rows show the preferred target, actual Set A and Set B difficulty counts, and a warning when they differ.", "editable": "Both screen and print are GET-only, aggregate-only, and never start generation."},
                    ...
                    {"name": "Correct a reviewer Case after reopening", "does": "Keeps the current reviewer Case attached to returned contributor Drafts, including after their last copied member is deleted.", "when": "Reopen the Automatic grouped unit with a reason and future deadline, return the selected Submitted contribution, then direct its owner to the reviewer Case in Question Bank. The owner can edit or replace only their current questions, choose replacement position and submit before the new deadline.", "avoid": "Do not use standalone replacements to bypass an incomplete reviewer Case. An empty or one-question correction Case blocks authoritative readiness; its prior generated and released revisions remain historical.", "result": "After the new deadline and successful readiness, generation can create a replacement revision. Release its Questionnaire and Answer Key separately. No in-app approval step is involved.", "editable": "Only the authorized owner's current Draft questions during the correction window."},
        ...
        "topics": [
            {
                ...
                "steps": [
                    ...
                    "Use Manage Course Exams to review only courses within your exact current exam responsibility. Row actions follow lifecycle: Configure course for incomplete setup; Review and open only when opening confirmation is available; View configuration and Monitor submissions after Open; and View preserved configuration or Generation status for history. Prepare Faculty Contributions remains the cycle-level bulk action. Configuration Ready does not confirm opening eligibility. Ready, Blocked, Already open, Preserved history, and Exempt badges include text as well as color and remain separate from the DEPTAL classification. One grouped examination is shared across its listed campuses; it is not one examination per campus.",
                    "Administer controls responsibility and Included/Exempt management. Assigning an Exam Department does not automatically assign that department's Area Chair and does not grant any permission. Automatic Generation may have no responsible department and uses no course reviewer; Manual Review requires an active responsible department and an explicitly selected eligible reviewer. Existing RBAC and assignment policy remain authoritative across every participating campus.",
                    ...
                    "On Course Examination Configuration, authorized exact-responsibility users may inherit each cycle default or set an intentional course override, then record coverage and optional instructions. Effective values show DEFAULT, OVERRIDE, or NOT CONFIGURED. Removing a deadline override restores the cycle deadline, or NOT CONFIGURED when the cycle has none. With Structured Case/Scenario Exam Lifecycle enabled, an Automatic Included consistently configured DEPTAL unit that is entirely never-opened Draft, has no blueprint or incompatible activity, and satisfies every other opening requirement displays No Sections — default (created on Open). GET/readiness does not create it. Single-course or signed bulk Open revalidates under lock, creates exactly one primary-owned No Sections blueprint, records its default origin and final-item count, then uses authoritative validation and permanently freezes the structure in the existing transaction. Existing explicit, conflicting, alias-owned, invalid, frozen, generated, released, contributed, rostered, or historical units are never defaulted or replaced. Optional sections remain configurable before Open, and Cases remain permitted in No Sections' implicit section.",
                    ...
                    "From Faculty Contribution Status, open Automatic Generation Readiness and select one authorized cycle. Review the seven shared screen/print columns, beginning with sequential No. and including monitoring-only Submitted volume over effective quota times distinct offered campuses, one Automatic Exam Generation Status, and bulleted Action Needed. The Report Summary counts only rendered authorized rows by that existing displayed status. Initialize a roster labeled not initialized; Synchronize only an initialized roster whose eligible Faculty list changed while intake is Open and before its effective deadline. For Closed or expired intake, an authorized Automatic generation manager may open a reasoned correction window with a future deadline; selected Submitted contributors receive a new Draft while their original submission remains historical. The report works before the deadline and never starts processing or exposes confidential questions.",
                    "The scheduled Automatic deadline processor isolates each grouped examination in its own time-bounded process. A time limit displays as Automatic generation exceeded the per-course processing time limit and requires administrator review; the batch continues with later courses and preserves that Closed terminal result. Restore remains subject to its existing policy. A justified Automatic correction may reopen after expiry; processing resumes after the new deadline and checks readiness again.",
                    "Automatic SUFFICIENT_POOL excludes unresolved Blocked Drafts with warnings and uses only eligible Submitted questions. Strict contributor policy and Manual behavior still require resolution. Every section must satisfy its exact quota using whole Cases and standalone questions; one section's surplus cannot fill another's shortage. After the deadline, use a reasoned correction window when the same examination requires contributor fixes; generation still applies the frozen section and Case rules.",
                    "An authorized Automatic generation manager may reopen an Open or Closed grouped unit, including after expiry, with a reason and future deadline. The manager may select current Submitted contributors to return to Draft through historical successor copies; unfinished Drafts may also be completed where retained eligibility proof permits it. If an affected Admin Case includes another editable Draft, complete that Draft's submission before reopening another member of the Case. The workspace is labeled Reopened Draft and remains editable only until the replacement deadline; reopening does not reactivate or rewrite those historical records. Original Submitted contributions remain immutable. Reopening supersedes any current generation and revokes active Questionnaire and Answer Key releases; new output and releases require separate actions after correction.",
                ...
                "actions": [
                    ...
                    {
                        "name": "Release Exam for Printing",
            ...
            },
            {
                ...
                "menu_path": "Admin Portal -> Departmental Exam Builder -> Exam Readiness",
                "steps": [
            ...
            },
            {
                ...
                "steps": [
                    ...
                    "Use the Release Center tabs to keep Questionnaire Releases and Answer Key Releases separate. Both tabs keep selected exact targets while course code/name, Department, and Release Status filters change what is visible. The selected total and hidden-selected count show the complete batch; use Show selected or Clear selection as needed. Select All Visible changes only visible, eligible rows. Changing the cycle clears affected selections. In Bulk Answer Key Release, first select an explicit Target Campus and load its recipient courses; changing campus clears only Answer Key selections. Use View details to review one authorized primary-course, recipient-course, and campus target without changing selections or filters, including on Past / Closed cycles. Details show the exact current-final revision, availability/status, required session attestation, release or revoke controls, target history, and authorized Who Viewed links. Browse scoped Answer Key history by authorized campus, including inactive campuses marked history only; they cannot receive new releases and are never shown as Faculty-available. Before revoking from the summary, details, or scoped history, confirm the displayed campus, recipient course, and exact release revision. A successful revoke refreshes the current release view; if the revoke succeeds but refresh fails, use Retry refresh without submitting another revoke. Detail navigation, review confirmation, and a revoke keep the selected cycle and tab; Answer Key navigation also keeps the target campus. An unsuccessful ordinary individual release returns the target details with the entered window and validation errors. Historical and bulk-ineligible scoped targets remain in the history index.",
                    "If Answer Key batch review finds an invalid selection or window, use Back to Release Center to correct it. Back retains your selected campus only while that campus remains authorized for Answer Key release; the cycle and Answer Key tab remain selected.",
                    "For either bulk release, review every selected target, including hidden selections, before confirming the shared window. Questionnaire rows have a View details action for exact revisions, Admin Print Set A/B, Automatic Audit, release controls, and history; loading details never runs an audit. For Answer Keys, each target keeps its campus, recipient course, primary-owned exact revision, and required all-sessions-concluded confirmation. Equivalent member courses require their own selection. The final release revalidates every row, and stale, incomplete, unauthorized, or superseded rows roll back the complete batch.",
                    "Legacy unscoped releases retain their original history but no longer grant Faculty access; explicitly reissue each intended campus/course after reviewing the current final revision and attestation. Use Who Viewed for an exact campus/course Answer Key release to review Faculty users who successfully opened the actual authorized Set A or Set B page, with first/last timestamps. Menu visibility, denied requests, Answer Key printing, and Checking Master printing do not count as Viewed.",
