        ...
        "topics": [
            {
                ...
                "steps": [
                    ...
                    "From Faculty Contribution Status, open Automatic Generation Readiness and select one authorized cycle. Review the seven shared screen/print columns, beginning with sequential No. and including monitoring-only Submitted volume over effective quota times distinct offered campuses, one Automatic Exam Generation Status, and bulleted Action Needed. The Report Summary counts only rendered authorized rows by that existing displayed status. Initialize a roster labeled not initialized; Synchronize only an initialized roster whose eligible Faculty list changed. The report works before the deadline and never starts processing or exposes confidential questions.",
                    "The scheduled Automatic deadline processor isolates each grouped examination in its own time-bounded process. A time limit displays as Automatic generation exceeded the per-course processing time limit and requires administrator review; the batch continues with later courses and does not retry that Closed terminal result until an authorized Reopen Contributions or eligible Restore resets processing.",
                    "Reopen Contributions may let the original owner finish an existing unfinished Draft when its retained source has durable proof of prior eligibility even though the assignment, offering, or roster evidence is now historical. The workspace is labeled Reopened Draft and remains editable only until the replacement deadline; reopening does not reactivate or rewrite those historical records. Submitted contributions remain immutable, and a current generation anywhere in an equivalency group blocks Draft mutation.",
