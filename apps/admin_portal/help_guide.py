        ...
        "topics": [
            {
                ...
                "actions": [
                    ...
                    {"name": "Automatic Generation Readiness", "does": "Opens the same six-column management report on screen and print: course, offered campuses, Final items, monitoring Contribution Progress, one generation status, and bulleted actions.", "when": "Use it before or after the deadline. Progress compares Submitted question volume with effective quota times distinct offered campuses; it is monitoring only, not the generation minimum. Initialize a roster labeled not initialized; Synchronize only an initialized roster whose eligible Faculty list changed.", "avoid": "Do not treat progress as technical usable-pool readiness. The generator still applies unique-question, 30/50/20 difficulty, campus-policy, and exact Set A/B checks internally.", "result": "One of READY FOR GENERATION, WAITING FOR DEADLINE, NEEDS QUESTIONS, FACULTY INCOMPLETE, BLOCKED, GENERATED, or EXEMPT, plus concise action bullets.", "editable": "Both screen and print are GET-only, aggregate-only, and never start generation."},
        ...
        "topics": [
            {
                ...
                "steps": [
                    ...
                    "From Contributor Completion, open Automatic Generation Readiness and select one authorized cycle. Review the six shared screen/print columns, including monitoring-only Submitted volume over effective quota times distinct offered campuses, one Automatic Exam Generation Status, and bulleted Action Needed. Initialize a roster labeled not initialized; Synchronize only an initialized roster whose eligible Faculty list changed. The report works before the deadline and never starts processing or exposes confidential questions.",
