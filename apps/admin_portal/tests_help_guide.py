...
class AdminHelpGuideTests(TestCase):
    ...
    def test_departmental_exam_configurer_receives_stage41_help(self):
        ...
        self.assertContains(role_response, "Configuration Ready does not confirm opening eligibility")
        self.assertContains(role_response, "Prepare Faculty Contributions")
    ...
    def test_departmental_exam_output_guides_cover_release_and_audit_operations(self):
        ...
        self.assertIn("Select All Visible", guide_text)
        self.assertIn("course code/name", guide_text)
        self.assertIn("automatically deselects rows that become hidden", guide_text)
        self.assertIn("preserving selected rows that remain visible", guide_text)
        self.assertIn("active tab, filters, and scroll position", guide_text)
        ...
        self.assertIn("confirmed Standardized or Departmental", guide_text)
        self.assertIn("neutral EXAMINATIONS heading", guide_text)
        ...
        self.assertContains(practical, "Select All Visible")
        self.assertContains(practical, "responsible/Exam Department")
        self.assertContains(practical, "automatically deselects rows that become hidden")
        self.assertContains(practical, "displayed-record badge")
        self.assertContains(practical, "Letter (default), A4, or Legal")
        self.assertContains(practical, "neutral EXAMINATIONS heading")
