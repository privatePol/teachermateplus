...
class DepartmentDropdownLabelTests(TestCase):
    ...
    def test_active_offering_list_uses_schedule_text_and_orders_rows_by_section_course(self):
        CourseOffering.objects.create(
            tenant=self.tenant,
            campus=self.fairview,
            department=self.fairview_college,
        ...
        self.assertEqual(response.status_code, 200)
        content = response.content.decode("utf-8")
        active_content = content.split("Inactive Course Offerings", 1)[0]
        self.assertIn("<th>Schedule Text</th>", active_content)
        self.assertNotIn("<th>Campus</th>", active_content)
        self.assertNotIn("<th>Term</th>", active_content)
        self.assertLess(active_content.index("<th>Course</th>"), active_content.index("<th>Section</th>"))
        self.assertLess(active_content.index("<th>Section</th>"), active_content.index("<th>Schedule Text</th>"))
        self.assertLess(active_content.index("<th>Schedule Text</th>"), active_content.index("<th>Room/Office/Lab</th>"))
