...
class FacultyHelpGuideTests(TestCase):
    ...
    def test_revised_faculty_guide_explains_zero_blank_and_base_50(self):
        ...
        self.assertContains(response, "<h3>Question Bank</h3>")
        self.assertContains(response, "Choose Question Bank, then open the grouped course workspace.")
        self.assertContains(response, "separates Contribution status and deadline from Released materials")
        self.assertContains(response, "Every material shows its own availability and Open until time")
        self.assertContains(response, "neither uses the Contribution deadline")
        self.assertContains(response, "Add these questions to section")
        self.assertContains(response, "No Sections exams need no selector")
        self.assertContains(response, "Show or Hide Case")
        self.assertContains(response, "Case content remains present in printed output")
        self.assertNotContains(response, "Choose Question Contributions")
        ...
        self.assertContains(response, "confirmed Standardized or Departmental examination heading")
        self.assertContains(response, "neutral EXAMINATIONS heading")
        ...
        self.assertContains(response, "Background graphics is disabled")
    ...
    def test_case_editor_guide_explains_controls_limits_and_scope(self):
        ...
        for wording in (
            ...
            "Single Rule", "Double Rule", "Remove Rule",
            "the normal table grid remains", "These are not final exam numbers",
            "Section assignment required", "use Edit to select a frozen Exam Section",
            "Each real frozen section shows its final-exam item target",
            "empty sections show zero",
            "separate from the overall faculty contribution quota",
            "No Sections exams retain the overall count without section targets",
            "receive no final-exam target",
            "Uncertain Word blank paragraphs",
