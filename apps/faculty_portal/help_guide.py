        ...
        "topics": [
            {
                ...
                "actions": [
                    ...
                    {"name": "Download and Upload CSV", "does": "Downloads the exact UTF-8 template, validates an append-only CSV, and creates a confidential server-side preview for 30 minutes.", "when": "Use it for multiple questions after replacing or deleting the labeled sample row. Keep the page open while the measured upload, validation, and persisted import progress are active.", "avoid": "Do not change header spelling, order, case, or spacing; do not include tabs, control or bidirectional formatting characters; and do not exceed 2 MB, 200 nonblank rows, or the remaining quota. Resume after a transient connection or worker interruption; if access, assignment, revision, or quota changed, review the workspace and start a fresh preview.", "result": "Errors and warning-only duplicates appear by row; confirmation commits short durable chunks, resumes transient interruptions from the last committed row, safely discards incomplete rows after a terminal conflict, and exposes questions only after the complete batch finishes.", "editable": "Imported questions remain editable Draft questions until final submission."},
