        ...
        "topics": [
            {
                ...
                "actions": [
                    ...
                    {"name": "Create Case-Based Question Group", "does": "When the tenant enables Structured Case/Scenario lifecycle for a frozen Manual Review exam, creates one Case / Scenario by pasting clipboard content from Microsoft Word, reviewing the server-sanitized preview, and then adding ordinary Linked MCQs beneath it. You can include up to 50 tables per Case, including nested tables. Overall content limits still apply. Supported content includes paragraphs, headings, bold/italic, lists, multiple accounting tables and safe merged cells, left/center/right alignment, Unicode and ₱, superscript/subscript, and TMP \\(...\\) or \\[...\\] notation. Harmless Word spacer paragraphs and repeated spacing are compacted while meaningful table-cell paragraphs remain distinct.", "when": "Use it only in a mutable structured Manual Review contribution. Select the frozen Exam Section when the exam uses sections; every Linked Question automatically uses that same section and counts as one quota item.", "avoid": "Do not upload a DOCX for Case content, paste images/diagrams or native Word equations, or expect pixel-perfect Word layout. Replace native equations with TMP LaTeX or Unicode. Never paste another contributor's confidential content.", "result": "TeacherMate+ stores bounded canonical semantic HTML only after the server removes unsupported Word metadata, active markup, and non-meaningful Word spacing artifacts. Preview and Save use the same server authority. If rejected source cannot be safely formatted, the normal editor stays free of raw markup and a non-executable recovery copy is shown. The Case narrative does not count toward quota; every Linked MCQ retains its own choices, correct answer, difficulty, and order.", "editable": "The Case and Linked Questions are editable/reorderable only while the contribution remains mutable. A Case must contain at least one Linked Question before Final Submission."},
