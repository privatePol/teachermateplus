...
- Implementation is complete on `feat/departmental-exam-builder`. It provides Fixed Item Count for All Courses and Configure Item Count per Course modes; separate `final_item_count` and `questions_required_per_faculty`; coverage and optional instructions; cycle and course Draft/Open/Closed workflows; explicit Open for Faculty Contribution; mode/fixed-count consistency; responsibility and inclusion guards; tenant/campus/RBAC/direct-deny/reviewer-read-only/superuser restrictions; bounded SHA-256/length audit evidence; MariaDB-safe preflight ordering; and global cycle -> parent -> configuration locking. The dedicated Departmental Exam Builder group has exactly two menu entries and no Stage 5-7 route or permission exposure.
- Gate 3 validation passed: Django system check; migration drift check with no changes detected; expected migration plan; focused Stage 4 suite 81 passed with one expected SQLite concurrency skip; full departmental-exams package 153 passed with one expected SQLite concurrency skip; Stage 1-3 regressions 72/72; Admin help and roles 30/30; query/propagation batching checks; and scoped Git diff check. Final Sol High independent re-review: B — Approved after non-blocking documentation/test follow-up. No Blocker, High, or Medium implementation finding remains; implementation is approved to proceed to selective staging review after this documentation refresh.
- Migration required: `apps/departmental_exams/migrations/0002_stage4_course_configuration.py`. It has not been applied to the normal local database, staging, or production, and no additional migration is required. MariaDB migration smoke, MariaDB concurrency smoke, and browser smoke remain pending safe staging-environment checks.
- Gate 5 selective staging passed and Gate 6 staged-diff validation passed with verdict A — Staged diff validated; ready for commit authorization. Stage 4 is approved for local commit. Migration `apps/departmental_exams/migrations/0002_stage4_course_configuration.py` is required but remains unapplied. No push, migration application, staging deployment, production deployment, or service restart occurred. MariaDB migration smoke, MariaDB concurrency smoke, and browser smoke remain pending safe staging-environment checks. Stage 5 has not started: no faculty contribution encoding, assignment workflow, question submission, generation, answer-key, PDF, Pair Code, or QR implementation is exposed.
...
- Pending before release are browser smoke for confirmations, errors, responsive behavior, reviewer read-only visibility, and reason privacy; MariaDB migration/concurrency smoke in a safe staging environment; staging deployment; and production deployment. Stage 4 implementation, Gate 5 selective staging, and Gate 6 staged-diff validation are complete; Stage 4 is approved for local commit. Stage 5 has not started.
