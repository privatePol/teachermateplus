...
- User account security now includes:
  ...
  - authentication audit events and failed-login lockout state resolve one normalized client IP through a trusted-proxy-aware resolver; `X-Forwarded-For` and `X-Real-IP` are honored only when the direct peer matches `DJANGO_TRUSTED_PROXY_IPS`, otherwise `REMOTE_ADDR` is used and forwarded headers are ignored
