...
- Faculty Grade Summary desktop tables use the original scroll container plus a JavaScript-synchronized floating horizontal scrollbar that appears only while an overflowing table is visible. Its track reflects the rendered table width, its position follows resize/sidebar changes through `ResizeObserver`, and keyboard Left/Right/Home/End control works when focused. No., Student No., Student Name, and Status remain sticky; the four table-header row offsets are measured at runtime below the Faculty top bar. The table remains usable without JavaScript through its normal horizontal scrollbar, while phone and print layouts keep their existing compact/print presentation.
