...
- Students use a cryptographically random public URL or its dynamically generated QR code without logging in. The bearer token stays in the browser URL fragment, is removed from browser history before a CSRF-protected POST opens the survey, and is never placed in the Nginx/Gunicorn request path. The page collects exactly one of four learning-status reactions and, only when the faculty enabled them, optional 200-character answers to either or both predefined written prompts. It does not request identity, verify attendance, write grades, expose classmates, or calculate a faculty score.
- Anonymous duplicate control uses a signed HttpOnly browser token, a session-specific HMAC hash with a database uniqueness constraint, transactional submission, and cache-backed browser/IP rate limits. Different browsers on one campus IP remain allowed. The control reduces casual duplicates but cannot prove student identity or stop a user who clears cookies or changes browsers. LocMemCache counters are best-effort per Gunicorn worker; database uniqueness and lifecycle validation are the correctness controls. Cache failure returns a temporary error without storing the response.
- Faculty may extend a live session once by five minutes, close it early, or cancel it. Server-side status and expiry checks govern every public submission. A draft/live session is automatically cancelled if its accepted assignment, owner relationship, offering, or snapshotted academic scope is no longer eligible. Live polling exposes only the total count; written answers remain hidden until the session is closed or expired. Owner-only results contain reaction counts/percentages, enrolled count, response rate, understanding rate, support-needed rate, and optional anonymous text. Exit Pulse has no charts, exports, administrator reporting, attendance integration, or gradebook integration.
- Anonymous browser-token hashes expire after about 24 hours. Results-page cleanup is limited to the opened session, and the idempotent `anonymize_exit_pulse_identifiers` command supports `--dry-run` and `--tenant-id`; the production cron template runs it hourly. Cleanup preserves reaction and optional written-response data and never reports identifier values.
- Faculty create/start, extend, close, and cancel actions follow the existing scoped audit convention. Public responses create no personally identifying audit event, and logs do not include response text, browser identifiers, cookies, or full public tokens. Exit Pulse uses the existing ReportLab QR support and adds no dependency.
