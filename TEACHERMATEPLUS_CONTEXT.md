...
- Stage 4 - Course configuration/readiness: independent cycle defaults and course overrides; questions-required-per-faculty; final questionnaire item count; deadline, instructions, and coverage; Draft/Open/Closed lifecycle; historical immutability; bulk default application; and audit. Implementation, developer validation, final independent review, commit preparation, local commit, and feature-branch push are complete. Approved integration candidate: `614ef27eb9c25eaf1403267118e61dea0e4719a5`. Controlled integration validation is the current release gate; production deployment and external MariaDB/browser smoke remain pending.
...
- Current Stage 4 behavior uses independent 50-75 cycle defaults and per-course `DEFAULT`/`OVERRIDE` sources, readiness, first-open immutable snapshots, bounded cycle -> parent -> configuration -> downstream locking, exact tenant/campus/responsible-department scope, and RBAC/direct-deny controls. Cycle-default confirmation uses an explicit POST form action and a signed POST-body state bound to the actor, tenant, cycle, purpose, revision timestamp, and expiry; confidential instructions and administrative reasons do not enter actions, paths, query strings, or redirects. Broad course-configuration before/after audits represent `additional_instructions` only as SHA-256 plus character length, while retaining other non-confidential evidence. Authorized Closed-cycle override-removal GETs fail with the established lifecycle `404` before form/state construction; authorization denial remains first. `departmental_exams.0003_cao_default_override_counts` remains required, unapplied, and unchanged. No additional migration is required. Migration application: Not performed.
...
- Developer validation of the four-item remediation passed: 46/47 focused tests and 66/67 combined Stage 4 tests passed, with the same explicit SQLite-only concurrency skip; Django check and migration drift passed. The accepted Low test-boundary observation remains a non-blocking follow-up only: the permanent course-instruction confidentiality regression mocks `AuditService.log_event`, while an independent disposable-database probe confirmed the persisted audit JSON is safe. No migration was applied; no normal database was changed; no permission or menu item was added; and no Stage 5-8 implementation began. MariaDB migration smoke, MariaDB concurrency smoke, and real-browser smoke remain pending.
