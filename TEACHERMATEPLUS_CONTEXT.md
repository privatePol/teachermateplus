...
- Exempt is Draft-only and requires an approved category plus a trimmed 10-500-character reason. Ordinary users require active exact-scope `departmental_exams.configure`; tenant, responsible-department/campus, active user/role/UserRole/permission, and direct-deny rules remain authoritative. Course, offering, responsible-department, reviewer, and `CourseExamConfiguration` data are preserved. Configuration alone does not block Exempt and remains dormant while Exempt; substantive faculty contribution or question activity blocks Exempt, and no downstream data is deleted or reset.
...
- Pending before release are browser smoke for confirmations, errors, responsive behavior, reviewer read-only visibility, and reason privacy; MariaDB concurrency smoke for administration-versus-transition races and lock behavior; staging deployment; and production deployment. Stage 4 remains planned only for Fixed Item Count for All Courses, Configure Item Count per Course, `final_item_count`, `questions_required_per_faculty`, coverage/instructions, deadline, and the publish/open-for-contribution gate.
