...
- Students use a cryptographically random public URL or its dynamically generated QR code without logging in. The page collects exactly one of four learning-status reactions and, only when the faculty enabled them, optional 200-character answers to either or both predefined written prompts. It does not request identity, verify attendance, write grades, expose classmates, or calculate a faculty score.
- Anonymous duplicate control uses a signed HttpOnly browser token, a session-specific HMAC hash with a database uniqueness constraint, transactional submission, and cache-backed browser/IP rate limits. Different browsers on one campus IP remain allowed. The control reduces casual duplicates but cannot prove student identity or stop a user who clears cookies or changes browsers. Stored browser hashes are designed for lazy anonymization after about 24 hours and are never shown to faculty or written to student audit events.
- Faculty may extend a live session once by five minutes, close it early, or cancel it. Server-side status and expiry checks govern every public submission. Live polling exposes only the total count; written answers remain hidden until the session is closed or expired. Owner-only results contain reaction counts/percentages, enrolled count, response rate, understanding rate, support-needed rate, and optional anonymous text. Phase 1 intentionally has no history dashboard, comparison, charts, exports, administrator reporting, attendance integration, or gradebook integration.
