...
- Stage 4 - Course configuration/readiness is integrated in the approved staging release `3804bbd666a4cc7ef072789c2bfeff1d1708ae2d`. Stage 4.1 is the current uncommitted and unpushed usability/default-deadline amendment; its implementation, independent-review remediation, campus-display deduplication remediation, automated validation, and local authenticated browser smoke are complete. Staging and production deployment remain pending, and production remains untouched/frozen.
- Stage 5 - Faculty contribution and persistent question bank: Not started.
- Stage 6 - Set A/Set B generation, answer keys, Department Head review/approval, locking/versioning: Not started. Department Head initiation of generation remains pending CAO confirmation.
- Stage 7 - Secure PDF/export outputs, separate answer keys, Pair Codes, QR verification: Not started.
...
- Migration `apps/departmental_exams/migrations/0004_stage41_default_contribution_deadline.py` is schema plus data: legacy non-null course deadlines backfill as `OVERRIDE`, while null deadlines remain `NULL/NULL`. Migration `0004` is applied to the normal Windows-local SQLite database at `D:\teachermateplus\db.sqlite3`; no local migration operation is pending and no local model/migration drift exists. Staging and production migration application have not occurred and require separately authorized deployment gates. No additional migration was created for the independent-review or campus-display remediation.
...
- Exempt requires an approved category plus a trimmed 10-500-character reason. Ordinary users require active exact-scope `departmental_exams.configure`; tenant, responsible-department/campus, active user/role/UserRole/permission, and direct-deny rules remain authoritative. Course, offering, responsible-department, reviewer, and `CourseExamConfiguration` data are preserved. Stage 4 permits an activity-free Open configuration to transition to Closed/dormant atomically during Exempt; substantive faculty contribution or question activity still blocks Exempt, and no downstream data is deleted or reset.
- Restore to Included is Draft-only and requires a trimmed 10-500-character restoration reason. It clears the active exemption category/reason, preserves configuration and snapshots, and does not use Exempt's contribution/question blocker. Restore revalidates the assigned reviewer atomically, retaining an eligible reviewer or clearing and auditing an ineligible reviewer; it does not delete downstream data.
