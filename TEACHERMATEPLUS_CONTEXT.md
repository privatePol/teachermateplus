...
- The report is diagnostic and read-only. It uses the Admin header tenant/campus scope and defaults Academic Year/Term to the active academic scope when available. Its page filters are campus, academic year, term, search, and category; forged IDs are resolved only through server-scoped querysets.
- Course Offering exceptions are active, non-archived offerings in the selected scope with zero active `Enrollment.Status.ACTIVE` rows. Dropped, withdrawn, incomplete, and inactive enrollment rows do not count. Active assignment rows are shown as zero, one, or multiple faculty names; the report does not require acceptance because it reconciles assignment records rather than grading authority.
- Faculty exceptions are active users with an active, scoped `FACULTY` role and zero active faculty-assignment rows for the selected offering term. There is no separate employee ID, department, or employment-status profile in the current data model, so the report uses the existing username as Faculty ID and does not invent those fields. CSV output is formula-safe and follows the same scoped filtered rows. No automatic correction, audit-per-view, or academic mutation is performed.
- The report presents total and exception count/denominator/percentage cards, neutral contextual notices when an exception category reaches 80% of its selected-scope denominator, compact count-bearing tabs, category-specific CSV labels, and allowlisted sorting. The No Enrollment tab uses a real filter-preserving next-page link as its non-JavaScript baseline and progressively enhances it into authenticated 50-row partial appends. The Faculty tab retains 50-row pagination. Offering sorts are course code, course title, section, and assigned faculty; faculty sorts are faculty name and faculty ID. Invalid sort parameters safely fall back to the category default. Offering CSV exports separate formula-safe Section Code and Section Name values.
- Focused validation is complete: the final reviewer-correction run discovered and passed 16 tests in 46.595 seconds. Coverage includes authorized/denied access, unauthorized export/lazy modes, campus-specific permission isolation, mismatched/cross-tenant periods, active enrollment and assignment rules, scoped warning ratios, conventional and enhanced lazy loading, full-filtered and formula-safe CSV export, safe RBAC reversal, search/sorting/filter preservation, and a measured bounded-query guard. The page used 70 queries for both one row and fifty rows with multiple assignments, confirming no per-row N+1 growth. No reconciliation business rule changed.
