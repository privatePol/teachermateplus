...
Last updated by Codex: 2026-08-03

## Purpose
This file preserves continuity between Codex sessions for TeacherMate+ V1.

## Current Session Summary
### Campus-grouped Exam Department selector UX
- Date/base: 2026-08-03; `D:\teachermateplus-bulk-exam-department-hotfix`, branch `hotfix/bulk-exam-department-assignment`, unchanged HEAD `e8f0176fd30074fb35787b876901f81fe2f03d49`, matching `origin/main`.
...
- Changed files: `apps/admin_portal/course_exam_department.py`, `apps/admin_portal/forms.py`, `apps/admin_portal/views.py`, `apps/admin_portal/tests_bulk_exam_department_assignment.py`, `templates/admin_portal/academics/bulk_exam_department_assignment.html`, and this handoff. No model or migration change is required.
- Validation: `git diff --check`, `python -B manage.py check`, and `python -B manage.py makemigrations --check --dry-run` passed. The required two-module command passed 52/52 in 23.483 seconds. An earlier run had 51 passes plus one test-only exact-markup assertion error because Django correctly added `selected` to the empty option; the assertion was corrected and the full rerun passed.
- Browser smoke/next gate: after a local runtime restart, authenticated browser smoke passed for the Bulk Responsible Exam Department selector, Bulk Current Exam Department filter, and Course Create/Edit Exam Department selector. It verified native non-selectable campus optgroup headings, correct campus membership, first-position empty options, campus-qualified labels, and distinct exact Department identities. The correction remains uncommitted and unpushed; no staging or production deployment occurred during that browser-smoke gate, and no migration is required. After selective staging and cached review, the next separately authorized gate is commit.

### Exam Department dropdown sorting remediation
- Date/base: 2026-08-03; `D:\teachermateplus-bulk-exam-department-hotfix`, branch `hotfix/bulk-exam-department-assignment`, unchanged HEAD `e8f0176fd30074fb35787b876901f81fe2f03d49`, matching `origin/main`. No staging, commit, push, migration application, normal-database access, server restart, deployment, or protected-log/secret access occurred.
...
- Changed files: `apps/admin_portal/course_exam_department.py`, `apps/admin_portal/views.py`, `apps/admin_portal/tests_bulk_exam_department_assignment.py`, and this handoff. No migration is required or created.
- Validation: `git diff --check`, `python -B manage.py check`, and `python -B manage.py makemigrations --check --dry-run` passed. `python -u -B manage.py test apps.admin_portal.tests_bulk_exam_department_assignment apps.admin_portal.tests_department_dropdown_labels --verbosity 1` passed 52/52 in 63.159 seconds with no failures, errors, or reported skips. Django logging was redirected outside the worktree.
...
### Bulk Exam Department Assignment hotfix
- Date/base/worktree: 2026-08-03; isolated worktree `D:\teachermateplus-bulk-exam-department-hotfix`, branch `hotfix/bulk-exam-department-assignment`, based exactly on fetched `origin/main` `0ccf911afd4c0f799069c04302fd67cac544bacc`. The dirty Stage 5 worktree at `D:\teachermateplus` was inspected only through filename-safe Git status/ref commands and was not modified. Protected `logs/system.log` and secrets were not opened or manipulated.
- Completed: added `/admin-portal/academics/courses/exam-departments/bulk/` and a visible Courses-list action for users with `courses.update`. The page uses current tenant and supported Course/Department scope, active records only, campus-qualified exact Department choices, server-side code/title and assignment filters, an optional exact-current-Department filter, a scrollable table, visible-row select-all, clear selection, live count, default non-overwrite behavior, and an explicit unchecked replacement control with a warning and browser confirmation. The three independent-review blockers are remediated: Course create/edit now reuses the exact `CODE — Name — CAMPUS-CODE` label without changing ordinary Course Department labels; editing preserves only the current active same-tenant exact out-of-scope Exam Department and still rejects any forged alternative; and replacement parsing is strict at both HTTP and service boundaries. Feature-disabled behavior still matches the existing Course form.
- Service semantics: `BulkExamDepartmentAssignmentService` independently rechecks active user, Admin Portal access, `courses.update`, current tenant, active exact Department, and every active scoped Course; rejects missing, malformed, duplicate, inactive, cross-tenant, or unauthorized IDs; requires `replace_existing` to be an actual `bool`; locks and authoritatively revalidates the scoped target Department first; then locks all selected Course rows in primary-key order inside the same `transaction.atomic()` block. The complete set is validated before mutation, and target, updated, same-target unchanged, non-replacement skipped, and total counts remain mutually consistent. Each changed Course gets a bounded per-Course `UPDATE` audit with only old/new Exam Department ID/label data. Course campus/ordinary department, CourseOffering department, existing CycleCourse responsibility, reviewer, and lifecycle state are untouched; future cycles use the updated exact FK.
- Changed files: `apps/admin_portal/course_exam_department.py`, `apps/admin_portal/forms.py`, `apps/admin_portal/views.py`, `apps/admin_portal/urls.py`, `apps/admin_portal/tests_bulk_exam_department_assignment.py`, `templates/admin_portal/academics/course_list.html`, `templates/admin_portal/academics/bulk_exam_department_assignment.html`, `CHANGE_LOG.md`, `TEACHERMATEPLUS_CONTEXT.md`, and this handoff.
- Validation: final remediation-focused Course/bulk/dropdown tests passed 51/51 in 16.518 seconds; the required Admin scope/inactive Department/Course ownership/reviewer/feature/migration set passed 106/106 in 175.392 seconds. The 51-test command proves campus-qualified exact option IDs, current-only preservation, forged target/Course denial, direct-deny/service authorization, strict raw replacement tokens and service booleans, structural target locking/stale revalidation, accurate counts, no audits for unchanged/skipped rows, and rollback of both Course and audit writes when the second audit fails. `git diff --check`, `python -B manage.py check`, and `python -B manage.py makemigrations --check --dry-run` passed; migration drift is `No changes detected`. Inspection-only `python -B manage.py migrate --plan --no-color` exited 0 and listed the pre-existing full migration plan for the empty isolated-worktree database; it applied nothing. One earlier 43-test attempt had 42 passes plus one test-only `ModelChoiceIteratorValue` conversion error; the assertion was corrected and the required 51-test command passed. The complete `apps.departmental_exams` suite was not rerun during remediation; its earlier 167-pass/one expected SQLite-skip evidence remains historical and is not new remediation validation. SQLite does not prove MariaDB lock scheduling.
- Migration/release state: no migration is required or created. No migration was applied to the normal Windows development database. No commit, push, deployment, staging/training/production access, or service restart occurred. Automated request/rendering coverage passed; no real-browser smoke was run because service restart was forbidden.
- Pending/risk: independent read-only re-review is the recommended next gate; this remediation does not claim that re-review passed. MariaDB row-lock scheduling and real-browser desktop/narrow-width smoke remain unperformed and are not claimed. Before any later commit, recheck the exact ten-file inventory, final diff, branch/HEAD, and protected Stage 5 worktree status.

### Departmental Exam Builder Stage 4.1 commit preparation
...
- Release state: Stage 4.1 implementation and campus-display deduplication remediation are complete and prepared for the separate Commit gate, but remain uncommitted and unpushed. The approved Stage 4 staging release remains `3804bbd666a4cc7ef072789c2bfeff1d1708ae2d`. No staging, training, or production access or deployment occurred; production remains untouched/frozen. Staging and production migration application, deployment, and service restart remain pending separately authorized gates.
...
- Exact next step after successful selective staging: Stage 4.1 Commit. Do not commit, push, deploy, access staging/training/production, apply or reverse another migration, modify the normal database, or restart services without the corresponding separate authorization.
