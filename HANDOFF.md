## TMP-Answer-Key-Scope-Review-Remediation - 2026-09-11 independent review gate

- Baseline/gates: reviewed and remediated the complete uncommitted implementation, then prepared its single authorized commit in `D:\teachermateplus-answer-key-scope`, branch `fix/answer-key-campus-course-scope`, from exact pre-commit HEAD `e3d2f7a1169323d1e99cae7021db18df5fb1729a`. Push, migration application, deployment, and restart remain separate gates.
- Review/remediation: found that direct service callers could submit recipients from more than one explicit campus in one `bulk_release()` call, contrary to the one-campus-per-submission rule. The service now validates a single target-campus ID before creating any release. The focused regression proves a mixed-campus batch is rejected with no writes, while separate explicit campus submissions create only their selected recipient rows and never expand to a later campus. The existing atomicity regression was corrected to exercise a same-campus invalid second recipient so it continues to test transactional rollback beyond the new boundary.
- Scope verdict: explicit single-campus/single-course and single-campus/multiple-course releases pass; mixed-campus submission is rejected; equivalent-course access remains recipient-course and target-campus specific; non-target direct view/print/Checking Master paths deny; legacy unscoped rows remain inaccessible pending explicit governed reissue; target-scoped Who Viewed/revoke/replace behavior remains intact. No confidential question or answer content was added to audit/history.
- Validation: final combined matrix ran 192 tests in 3220.034s: 191 passed, one failed, zero errors, zero skips. The sole failure, `QuestionnairePrintReleaseTests.test_summary_displays_actual_persisted_set_and_difficulty_counts`, is the separately documented unchanged baseline mismatch outside this scope; the modified assertion in that module and all Answer Key scope/equivalency/release/AJAX/help-guide tests passed. The new scope module passed 16/16 before remediation; the mixed-campus regression passed 1/1 after the service fix; and the final two corrected service regressions passed 2/2 before the definitive matrix. Post-handoff `manage.py check` reported zero issues; `makemigrations --check --dry-run` reported `No changes detected`; read-only `migrate --plan` exited 0 and applied nothing; and `git diff --check` exited 0 with line-ending notices only.
- Runtime limits: authenticated browser smoke was NOT RUN because the enabled computer-use inventory contained no browser and both in-app browser acquisition paths reported unavailable. A disposable `%TEMP%` SQLite fixture/server was removed after that check; no normal database was migrated. MariaDB/InnoDB DDL and concurrency validation were NOT RUN.
- Changed-file scope: the implementation remains the existing 20-file set documented below. This review changed only `apps/departmental_exams/answer_key_release.py`, `apps/departmental_exams/tests_answer_key_release_scope.py`, and this handoff within that set. No staging, commit, push, deployment, migration application to a normal database, restart, reset, clean, stash, amend, rebase, revert, force operation, or protected-checkout edit occurred.
- Commit preparation: the complete unstaged and staged payload was inspected against the exact 20-path allowlist; the two pre-existing cross-feature test edits retain meaningful assertions, migration `0025` is ordered after `0024` and preserves legacy history with a reverse guard, and no unrelated/log/secret/runtime path is included. One local commit is authorized; no push is authorized.
- Pending/exact next steps: under separate authorization, verify the local commit and publish it without force. Before rollout, inventory legacy rows without confidential content, apply migration `0025` under a separately authorized deployment, perform explicit per-campus/course reissue, and complete authenticated browser plus MariaDB/InnoDB validation.

## TMP-Answer-Key-Campus-Course-Scope - 2026-09-11 implementation gate

- Worktree: `D:\teachermateplus-answer-key-scope`; branch `fix/answer-key-campus-course-scope`; HEAD `e3d2f7a1169323d1e99cae7021db18df5fb1729a` (stored origin/main baseline; not freshly fetched).
- Completed: explicit one-campus recipient selection; equivalent member isolation with primary revision ownership; scoped persistence, faculty access and target lifecycle/audit enforcement; legacy deny pending reissue. No all-campus wildcard is offered. Several campuses require separate explicit submissions.
- Migration: `0025_answer_key_release_target_scope`, after departmental_exams `0024_structured_exam_lifecycle_freeze`, tenants `0005_enable_existing_sis_api_feature`, and swappable User dependency. New runtime default `SCOPED`; existing rows classified `LEGACY_UNSCOPED`. PROTECT FKs `recipient_course` and `target_campus` are nullable only for legacy under `ck_de_key_target_scope`; FK indexes are automatic. `uq_de_key_target_active` covers recipient/course-campus-active marker and replaces course-wide uniqueness. Existing window/status/attestation constraints and indexes remain. Original IDs, timestamps, actors, status and audit records stay intact. Legacy rows do not block new scoped rows. Reverse guard runs before removing any scope field/constraint and rejects reversal if scoped history exists; no merge, deletion or automatic conversion to global release.
- Validation: project Python 3.14 / Django 5.2.12 with `config.settings` (local), `DB_ENGINE=django.db.backends.sqlite3`, `DB_NAME=:memory:` and `DJANGO_LOG_DIR=%TEMP%\TMP-Answer-Key-Campus-Course-Scope-logs`. The finalized Answer Key behavior run passed 59/59; subsequent focused checks passed the inactive-campus/equivalency fixtures 2/2, the final equivalent-member recipient-label and target-row path 1/1, the final campus-header assertion 1/1, the final Admin guide contract 1/1, and Release Center form/JavaScript source contracts 2/2. The new 0025 forward-preservation/reverse-guard migration test passed 1/1 with an unambiguous process exit 0. `manage.py check` reported zero issues; `makemigrations --check --dry-run` reported `No changes detected`; changed Python files parsed/compiled successfully; `git diff --check` exited 0 with line-ending notices only; and read-only `migrate --plan` included 0025 and applied nothing. Test runner migrations were confined to disposable test databases; no development DB migration was applied.
- Broader regression: 154 related tests ran against an in-flight pre-final test/doc snapshot: 151 passed and three failed. Two failures were obsolete expectations loaded before the final campus-header and help-copy corrections; each final assertion subsequently passed alone. The remaining `test_summary_displays_actual_persisted_set_and_difficulty_counts` mismatch was reproduced unchanged at exact baseline `e3d2f7a1169323d1e99cae7021db18df5fb1729a`, so the broad run is truthfully not green but shows no new unresolved failure from this scope change.
- Browser runtime: no enabled browsers returned by browser automation inventory; no Playwright or Node executable found. Actual browser selection/payload checks NOT RUN. MariaDB/InnoDB concurrency NOT RUN.
- Changed files: models.py, migration 0025, answer_key_release.py, services.py, forms.py, stage6_views.py, faculty_views.py, tests_answer_key_release.py, new tests_answer_key_release_scope.py, tests_exam_course_equivalency.py and tests_questionnaire_print_release.py (scope expansions reported before changing obsolete global-release fixtures/Answer Key pane assertions; all under apps/departmental_exams); static/js/departmental_exam_release_center.js; admin _answer_key_release_pane.html and answer_key_viewers.html plus faculty contribution_list.html under templates/departmental_exams; Admin/Faculty help_guide.py; CHANGE_LOG.md; TEACHERMATEPLUS_CONTEXT.md; HANDOFF.md.
- Pending/risks: independent diff review, separately authorized development/production migration application and rollout, authenticated browser smoke, and MariaDB/InnoDB concurrency validation. Legacy rows intentionally deny Faculty access until an administrator explicitly reissues each intended campus/course; deployed legacy inventory and operator communication are rollout prerequisites. No commit/push/deployment/restart at this gate. Original `D:\teachermateplus` remains protected.

Read-only deployed inventory guidance (NOT executed here):
1. Responsible operator: verify deployed SHA and read-only database connection; inspect applied migration names and actual table schema before querying. Do not run new-model ORM against the pre-0025 schema.
2. On the pre-0025 release table, SELECT only release ID, cycle_course ID, generation_revision ID, status/active_marker, windows, released/revoked actor IDs and timestamps, and attestation_version. Join cycle/course solely for tenant/course identifiers; summarize ACTIVE/scheduled/ended/revoked counts per tenant. Inspect current_marker/status and Set A/B count/position completeness without selecting answers, question text or choices.
3. Inventory participating campus/course offering snapshots separately. They establish current candidate targets, not historical release intent. Count existing audit events by release ID/action; retain original records. Do not infer historical campuses from default campuses, current assignments, or audit request context.
4. Compare pre/post-migration release IDs and original field values and audit counts. Only scope classification is backfilled; target FKs remain NULL for legacy. Preserve backups and do not deploy old application code over new scoped records.

Scoped reissue procedure after separately authorized rollout:
1. Authorized release administrator: review legacy inventory and confirm intended campus/course targets operationally. All legacy releases are inaccessible to Faculty until reissued, including previously scheduled/currently active records.
2. Open Answer Key Releases; select Target Campus; Load recipient courses; select intended member courses and verify the primary-owned exact current-final revision. Do not infer or automatically copy a legacy release's intended scope.
3. Enter the appropriate availability window and freshly confirm all examination sessions for selected courses have concluded. Release; repeat explicitly for each other intended campus.
4. Verify target status and eligible Faculty access; verify non-target campus/course direct view, print and Checking Master denial. Use the new release's Who Viewed history; old history stays attached to the old release. Revoke/replace only the intended target.

Previous unresolved handoff entries follow unchanged.

# HANDOFF.md

Last updated by Codex: 2026-09-05
