...
- Date: 2026-07-26
- Git checkpoint: branch `feat/departmental-exam-builder`; HEAD `e06cce744dfb2f249ca2dcac131e462890549a96`; HEAD message `feat(departmental-exams): add course inclusion workflow`. The Stage 1-2 foundation is commit `558fa476a08b93ac39615bf30e37307ad6a9c370` (`feat(departmental-exams): add stage 1-2 foundation`). Stage 3 application code is implemented, validated, and committed; it has not been pushed or deployed. This documentation correction is still uncommitted.
...
- Current worktree/safety: only `CHANGE_LOG.md`, `TEACHERMATEPLUS_CONTEXT.md`, and this handoff contain the documentation correction; `logs/system.log` remains the only other known modified file and must never be opened, read, diffed, edited, staged, restored, discarded, stashed, reset, cleaned, or committed. Keep the index empty until a separate selective-staging approval, and never use broad staging.
...
- Next gates: separately review and selectively stage/commit exactly these three documentation files. After that documentation commit, obtain explicit approval before any push, deployment, migration application, service restart, or Stage 4 work. Stage 4 has not begun and remains planned only for Fixed Item Count for All Courses, Configure Item Count per Course, `final_item_count`, `questions_required_per_faculty`, coverage/instructions, deadline, and the publish/open-for-contribution gate.
