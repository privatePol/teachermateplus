...
- UI: the desktop Grade Summary now has a native synchronized floating horizontal scrollbar that appears only while the overflowing table is visible. The normal table scrollbar remains the no-JavaScript fallback. The native scrollbar supports pointer/trackpad use; focused keyboard navigation supports Left/Right/Home/End. `ResizeObserver` synchronizes table width and measured multi-row header offsets after window/sidebar/layout changes. Existing No., Student No., Student Name, and Status sticky columns remain visible; the four header rows stick below the Faculty top bar. Print and compact phone layouts are intentionally unchanged, and the floating control avoids the bottom-left Faculty Help & Privacy controls.
