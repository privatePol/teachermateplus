...
- Date/gate/baseline: 2026-08-23; implementation/remediation plus focused validation in new clean worktree `D:\teachermateplus-automatic-open-restore-mainline`, branch `fix/automatic-open-restore-mainline`, exact fetched `origin/main` / unchanged HEAD `b71a8023f9d0da5fe44108b108890352c3cdb2ad`. The dirty source checkout `D:\teachermateplus` and its protected `logs/system.log` were not edited. No staging, commit, push, deployment, restart, or normal-database migration was authorized or performed.
- Completed: governed Automatic Generation `OPEN` / `EXEMPT` Restore now requires the preserved configuration and `timezone.now() < configuration.active_contribution_deadline`; missing, reached, and passed effective deadlines deny without mutation, and neither a new deadline nor the current cycle default is used. The existing cycle/course/configuration are locked in the transition transaction; a current generated revision blocks Restore. Success changes the course to Included, reopens the configuration, clears close markers, increments its revision, resets all Automatic processing fields, revalidates the reviewer, calls the existing `ContributionRosterService._synchronize_locked(..., initializing=False)`, and writes the existing Restore audit inside the same transaction. Draft/Submitted contributions, questions, import batches/rows, eligibility-source history, offering snapshots, deadline/instruction/opening history, and historical generated revisions remain preserved. Manual Open and Draft Restore behavior are unchanged.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/departmental_exams/services.py`; new `apps/departmental_exams/tests_automatic_open_restore.py`; and `templates/admin_portal/guide.html`. No model or migration file changed.
- Validation: exactly eight dotted regression selectors from `AutomaticOpenRestoreTests` passed 8/8 in 34.277s with zero failures/skips, covering success/deadline preservation, missing/reached/passed denial, roster activation/creation/blocking, contribution/question/import/source/offering preservation and question reuse, Automatic reset plus `process_due()` `NOT_DUE`, roster/audit rollback, current-generation denial, and unrelated-course isolation. `git diff --check` exited zero with line-ending notices only; `python manage.py check` reported zero issues; `python manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`. Test setup applied migrations only to its disposable in-memory database. No broad module or project suite ran.
- Pending/risks/next step: authenticated Admin/Faculty browser smoke, MariaDB lock-scheduling verification, and broader compatibility tests were not run under this narrow gate. Perform a short independent read-only review of the exact six-file diff, with particular attention to parent/configuration lock order, the deadline boundary, transaction rollback across roster/audit writes, and unchanged Manual/Draft/RBAC/direct-DENY behavior before any separately authorized staging or commit.
