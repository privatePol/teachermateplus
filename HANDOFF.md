...
Last updated by Codex: 2026-07-15
...
- Date: 2026-07-15
- Current pass: completed Exit Pulse Phase 2 Checkpoint 3 Assignment Comparison only. Added the faculty-only GET route `/faculty/exit-pulse/assignment-comparison/`, server-side academic-year/term/question-type filters, five aggregate summary cards, a responsive 20-row database-paginated comparison table, latest-result and owned-history navigation, dashboard/history links, and Faculty Help Guide coverage. No dashboard/Class History redesign, chart, export, ranking/evaluation, administrator analytics, anonymous public workflow, model/migration, attendance, grading, commit, or push change was made.
...
- Privacy/performance/accessibility: comparison aggregation is performed in reusable service/database annotations with one bounded assignment-row query independent of displayed row count; no per-row response loads or current-roster lookups are used. Written feedback, student identities, anonymous hashes, and bearer tokens are never selected for display. The page uses escaped template output, one H1, associated filter errors, visible state text, a focusable responsive table, labelled progress bars, descriptive links, accessible pagination, and neutral instructional-use wording.
...
- Date: 2026-07-14
- Current pass: completed Exit Pulse Phase 2 Checkpoint 2 only: enhanced faculty dashboard, owned Class History, historical-assignment authorization, server-side filters, 20-row pagination, accessible summary/table presentation, focused tests, and concise documentation. Assignment Comparison, cross-assignment UI, charts, exports, administrator analytics, public anonymous workflow changes, schema changes, unrelated refactoring, commit, and push were not performed.
...
- History behavior: the page displays assignment context/current-or-historical state, completed count/date range, terminal-only session rows, result links, and GET filters for inclusive local dates, question type, normalized case-insensitive topic text, and `CLOSED`/`EXPIRED` status. It uses stable newest-first ordering, 20-row pagination, safe invalid-page fallback, filter-preserving links, reset and empty states, escaped topic/question content, and only `Enabled`/`Not enabled` for written feedback. Anonymous feedback text remains available solely on the existing owner-authorized result page.
...
- Pending/known risks: desktop/mobile and assistive-technology browser smoke remains manual. LocMemCache remains per-worker as previously documented, and anonymous browser controls can legitimately yield a response rate over 100%. Historical retention continues to follow stored session ownership; any future institutional purge policy must be handled separately. Assignment Comparison and charts remain deferred to Checkpoint 3 and must not start without explicit approval.
...
- Pending work / decisions before UI: history/comparison authorization must deliberately include only the faculty member's allowed current/historical assignments and existing tenant/campus scope. The Phase 2 UI must label legacy single-session denominators as estimated/unavailable and show missing-denominator counts rather than implying precision. Anonymous submissions can legitimately exceed a stored enrollment count because duplicate control is browser-based; rates are not silently capped at 100%. Decide the neutral UI wording for that case. Question-type comparability warnings, filters, pagination, pages, and query review remain for later checkpoints.
...
- Completed work: added the dedicated `exit_pulse` Django app with scoped session/anonymous-response models, migrations, question safeguards, five-minute lifecycle/one extension, secure public token and dynamic ReportLab SVG QR, signed-browser duplicate control, cache-backed browser/IP rate limits, normal anonymous CSRF, public mobile survey, five-second faculty polling, terminal owner-only aggregates/comments, tenant feature switch, `exit_pulse.use`, Faculty Classroom Tools navigation, faculty action audits, concise guide/context/changelog documentation, and environment examples. Reused accepted `FacultyAssignment`/active offering rules, request tenant/campus scope, RBAC decorators/effective permissions, `AuditService`, `SystemSettingService`, client-IP resolution, Bootstrap templates, ReportLab, and existing cache counter conventions; no overlapping anonymous/public survey feature existed.
- Privacy/security boundaries: no student identity fields, login, attendance proof, grade write, public IDs, classmates/results on the survey, live comments, or anonymous student audit events. Browser identifiers are signed, session-HMAC hashed, never displayed/logged, uniquely constrained per session, and marked for lazy anonymization after about 24 hours. Public submission is POST-only, CSRF-protected, state/expiry checked under a transaction, duplicate-race safe, rate-limited, and rejects malformed tokens/reaction codes/overlength text. Cancelled sessions do not reveal written comments.
- Models/migrations: `ExitPulseSession` snapshots tenant, campus, faculty, accepted assignment/offering, academic scope, course/section, topic/question/prompts, secure token, status/times, creator, and extension state. `ExitPulseResponse` stores one stable reaction code, optional 200-character answers, and the expiring anonymous hash. Applied `exit_pulse.0001_initial` and `rbac.0027_seed_exit_pulse_permission` successfully to the local database.
...
- Validation performed: final `python manage.py test apps.exit_pulse.tests -v 1` passed 31/31. The Exit Pulse plus Faculty Guide slice passed 36/36; the two targeted Configuration Management render/save regressions passed 2/2. `python manage.py migrate --plan` showed only the two expected migrations and `python manage.py migrate` applied both successfully. `python manage.py check` passed, `python manage.py makemigrations --check --dry-run` reported no changes, and `git diff --check` passed with line-ending warnings only. The focused tests cover authorization/scope/feature disable, questions, lifecycle, server expiry, comments, anonymous access/CSRF/duplicates/rate limits, malformed/state handling, identifier anonymization, aggregates/zero safety, XSS, QR, and count polling.
- Pending work / risk: the in-app browser could not establish an active local browser connection, so desktop/mobile visual smoke remains manual. Anonymous browser control is deliberately best-effort: clearing cookies or changing browsers can produce another response, and it is not student identity or attendance verification. Cache rate limits are per cache backend/process when LocMemCache is used; deploy a shared cache if cross-worker enforcement is required. Expired technical hashes are lazily anonymized when results are opened, so deployments needing strict timed cleanup should schedule the service cleanup in a future maintenance command/job. No custom written-feedback prompt was added; Phase 1 uses only the two predefined prompts.
