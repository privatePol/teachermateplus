...
- Findings 2/3 corrected: client/server inspection scanners mask quoted strings, separate comments only at safe token boundaries and reject split tokens, unclosed comments/strings or unquoted escapes. Supported commented bottom rules survive; stylesheet rules are detected after scanning. Later conflicting all-edge overrides reject atomically instead of creating false double rules. Reversed order, equivalent repeated rules/grids and zero-width/none shorthands are supported; conflicting repeats, bottom longhands and border important declarations reject. Full grammar/limitations remain in `frontend/case-editor/README.md`.
- Scope: retained all 19 prior paths; sole addition `frontend/case-editor/border-fixtures.json` is test-only and shared by JavaScript/Python (38 equivalent cases). No runtime dependency, package/lock/license, model or migration change. Existing Single/Double/Remove, upstream merge/split, sanitizer restrictions, recovery, ownership/RBAC/lifecycle/revision/CSRF protections and every MAX_ budget remain. CSS/bundle references consistently use `case-review-20260910`.
- Newly executed targeted tests: frontend `node --test --test-name-pattern='shared border|rejected border' compatibility.test.mjs` passed 2/2, zero failures/skips (4.721s); Django `python -B -m django test apps.departmental_exams.tests_faculty_cases.ScenarioContentTests.test_shared_border_review_fixtures apps.departmental_exams.tests_faculty_cases.FacultyCaseWorkflowTests.test_csv_import_unplaced_recovery_and_existing_edit_route --no-color --verbosity 1 --noinput` passed 2/2 (23.801s). Actual HTTP CSV upload/preview/confirm, workspace/detail, immutable generated snapshots and existing Edit placement recovery are exercised; missing-placement submission rejection remains asserted. These targeted tests overlap the complete suites, not additional distinct tests.
- Newly executed frontend: `node --test compatibility.test.mjs` passed 37/37 twice (196.406s initially; 157.109s after decimal-comparison hardening), zero failures/cancellations/skips. Final stylesheet-important guard and final rebuilt bundle then passed `node --test --test-name-pattern='shared border|rejected border|self-hosted bundle|final bundle' compatibility.test.mjs`, 4/4 (5.321s), including all 38 shared fixtures and atomic recovery. These are overlapping validations, not summed distinct counts. Independent immutable-HEAD probe also passed: original padded raw 94,003 characters; previous canonical 49,276/1,551 elements unchanged; new normalization 88,744/3,345 elements. First rejection is specifically the 50,000 canonical-character budget; both size and node bounds are exceeded. Do not erase uncertain blanks or increase limits to accept this raw fixture.
- Newly executed complete Django command: `python -B -m django test apps.departmental_exams.tests_faculty_cases apps.faculty_portal.tests_help_guide apps.departmental_exams.tests_stage5_csv.Stage5CSVTests.test_atomic_confirm_appends_purges_payload_and_replays_idempotently apps.departmental_exams.tests_stage5_csv.Stage5CSVTests.test_second_preview_becomes_stale_after_first_confirmation apps.departmental_exams.tests_stage5_csv.Stage5CSVTests.test_wrong_owner_and_forged_file_hash_are_owner_scoped_404 apps.departmental_exams.tests_stage5_csv.Stage5CSVTests.test_sanitized_preview_content_is_identical_to_persisted_question --no-color --verbosity 1 --noinput`: 79/79 passed in 501.090s, zero failures/errors/skips. After decimal hardening and stronger member-ID/placement/snapshot assertions, `python -B -m django test apps.departmental_exams.tests_faculty_cases.ScenarioContentTests apps.departmental_exams.tests_faculty_cases.FacultyCaseWorkflowTests.test_csv_import_unplaced_recovery_and_existing_edit_route --no-color --verbosity 1 --noinput` passed 39/39 in 25.773s. After the final stylesheet-important guard, `python -B -m django test apps.departmental_exams.tests_faculty_cases.ScenarioContentTests --no-color --verbosity 1 --noinput` passed 38/38 in 2.994s. Disposable test databases destroyed; final SimpleTestCase-only run needed none. These runs overlap; earlier recorded 73/73 is historical only.
- Final review caught and corrected one equivalent-fixture precision discrepancy: Python's float formatting rounded grid widths differently from JavaScript. Both now compare decimal spelling with only insignificant zeros removed. The existing important-declaration rejection also covers stylesheet borders, preventing an important grid from silently overriding an inline rule. No accepted width budget or general CSS cascade implementation was added. Final review maps all three original findings to code and regression evidence; no remaining blocker for the authorized automated scope.
- Newly executed final checks: `python -B -m django check --no-color` zero issues; `python -B -m django makemigrations --check --dry-run --no-color` no changes; isolated `python -B -m django migrate --plan --no-color` exit 0, 839 existing-plan lines. All MAX_ constants match HEAD. Tracked and new-file Git whitespace checks passed. Final `node build.mjs` twice produced identical SHA256 `407C66995927EF1C961CFBB4EB727D3187B9E3913C1483F79AB0B90531022086` (418,592 bytes); 30 shipped MIT package notices retained without Git changes. Expected negative-test CSRF warning and Git LF/CRLF notices only. Earlier 35/73 results remain historical evidence only.
- Isolation: `python -B -m django`, dotenv disabled, dummy test secret, base local settings, SQLite path and log directory under `C:\Users\Lenovo\AppData\Local\Temp\tmp-case-remediation-20260910-e734`, LocMemCache, disposable test databases. No staging/production data, repository logs or .env contents accessed; migration 0025 remains unchanged.
- Browser acceptance still pending: computer-use skill initialization succeeded but native app inventory failed with unavailable pipe (os error 2); connected inventory returned no apps/browsers. No Firefox/Chromium caret, typing, keyboard selection, collapsed borders, CSP/network or Print/PDF acceptance is claimed. Actual failing Word clipboard HTML/text pair is still absent; original paste rejection is unresolved. Rich MCQ fields and Case-aware generation/printing remain deferred. Deploy matching server/editor/display assets together; older code strips accounting classes, so rollback after rule-bearing saves requires compatible readers/writers or separately approved recovery.
- Gate: TARGETED REMEDIATION + FOCUSED VALIDATION PASS for the three confirmed findings. Next recommended gate: COMMIT PREPARATION, separately authorized. All 20 paths remain unstaged; no commit, push, deployment, restart, dependency installation or non-test migration occurred. Keep the browser/Word and coordinated-rollback limitations above open.

### Case Numbering / Accounting Rules / Clipboard Preservation — Implementation
- Baseline verified: clean branch `feat/structured-case-rich-editor-phase2a`, HEAD `4b9d23a0e26c96fca6aa8a5b927c0ecd2d4e7f93`. No staging, commit, push, deployment, restart, non-test migration, secrets/log access or dependency changes are authorized. Scope is the 19 inspected files listed below.
- Completed: shared read-only section/Case/member/standalone presentation numbering and matching workspace/Case-detail rendering; cell-only Single/Double/Remove Rule with mixed selections, upstream merge/split preservation guards and shared fixed-border CSS; bounded explicit Word border import with rejection of unsupported meaningful rules; preservation of uncertain Word blank paragraphs; private finite phase/category diagnostics; versioned assets, guidance and regression coverage. Original source is not rewritten merely by opening. IDs, stored positions, answers, membership, mutation services, generated snapshots, RBAC/lifecycle/revisions/409/CSRF and all safety budgets remain unchanged.
- Changed files (19): `CHANGE_LOG.md`, `HANDOFF.md`, `TEACHERMATEPLUS_CONTEXT.md`, `apps/departmental_exams/scenario_content.py`, `apps/departmental_exams/faculty_views.py`, `apps/departmental_exams/tests_faculty_cases.py`, `apps/faculty_portal/help_guide.py`, `apps/faculty_portal/tests_help_guide.py`, `frontend/case-editor/compatibility.js`, `frontend/case-editor/extensions.js`, `frontend/case-editor/compatibility.test.mjs`, `frontend/case-editor/README.md`, `static/js/departmental_exam_case_editor.js`, `static/css/departmental_exam_case_editor.css`, `static/vendor/tiptap/3.31.3/tmp-case-editor.bundle.js`, `templates/departmental_exams/faculty/_question_card.html`, `templates/departmental_exams/faculty/contribution_workspace.html`, `templates/departmental_exams/faculty/case_detail.html`, `templates/departmental_exams/faculty/case_form.html`.
- Final frontend: `node --test compatibility.test.mjs` passed 35/35, zero failures/cancellations/skips, 84.483s, including final bundled initialization, all diagnostic categories, mixed rule selections/history, merge/split, legacy fixtures, repeated Python canonicalizer cycles and recovery/privacy checks. Earlier runs exposed obsolete blank-deletion expectations, padded-content budget rejection, history grouping and an incorrect diagnostic-test phase expectation; all corrected. Focused retests passed 3/3 (history/recovery) and 4/4 (diagnostic privacy/recovery/fatal handling); these are repetitions, not additional distinct coverage. The padded 25-table fixture explicitly rejects under unchanged limits rather than deleting uncertain blanks; supported 14/25-table fixtures and padded 14-table content remain separately exercised. Supported 14-table raw/normalized/canonical lengths: 32,493/27,562/27,562; 25-table: 58,123/49,276/49,276.
- Final backend: `python -B -m django test apps.departmental_exams.tests_faculty_cases apps.faculty_portal.tests_help_guide --no-color --verbosity 1` passed 73/73, zero failures/errors/skips, 156.390s; disposable test database destroyed normally. This covers authenticated numbering, NO_SECTIONS, standalone-first sections, interleaving, deletion/reorder, accounting Preview/Create/Edit/reopen and 51-table safe redisplay, plus existing ownership/RBAC/direct-DENY/lifecycle/409/CSRF/Stage 6 boundaries. Two earlier complete runs also passed 73/73 before final refinements; repetitions are not additional coverage.
- Checks: `python -B -m django check --no-color` passed with zero issues; `python -B -m django makemigrations --check --dry-run --no-color` reported no changes; `python -B -m django migrate --plan --no-color` exited 0 against a unique temporary SQLite path (839 output lines; existing departmental_exams.0001 through 0025). AST comparison confirmed every MAX_ constant equals HEAD. Source/template/test/docs diff reviewed; `git diff --check` passed, index empty, no untracked artifacts and exactly 19 approved changed files. Expected negative-test CSRF warning and Git LF/CRLF notices only; no unexpected final warnings.
- Build: `node build.mjs` twice with existing dependencies produced identical final SHA256 `30B8FE770E1CFEDB791123403902EB69D7F673A07BC68560422785B414AC8EF3` (417,562 bytes). Both builds verified 30 shipped MIT packages and retained license notices; package/lock/notice files have no Git diff. Versioned CSS/bundle URLs use `case-rules-20260910`. Browser execution/layout acceptance is not implied by the build or JSDOM tests.
- Isolation: use existing portable Node 24.21.0; Python commands are `python -B -m django` with `PYTHON_DOTENV_DISABLED=1`, `DJANGO_SETTINGS_MODULE=config.settings.base`, `DJANGO_ENV=local`, dummy `DJANGO_SECRET_KEY`, `DB_ENGINE=django.db.backends.sqlite3`, unique temporary `DB_NAME` and `DJANGO_LOG_DIR`. Only disposable test-database migrations are used. No models/migrations are changed; 0025 remains unchanged. Dependencies/package lock and retained MIT notices remain unchanged.
- Remaining acceptance: actual failing Word clipboard HTML/text pair absent; its exact rejection cause is unresolved. Obtain the smallest actual failing selection and both recovery payloads, not DOCX extraction. Computer-use native pipe returned unavailable (os error 2); connected inventory returned no browsers. No actual Firefox/Chromium caret, typing, selection, layout, border-intersection, CSP/network, Print/PDF or Word acceptance is claimed. User-reported organized toolbar, successful linked saves and plain-text Preview do not prove rich-paste acceptance. Preserve the original cursor uncertainty and unresolved linked-MCQ original validation predicate/standalone POST incident.
- Compatibility/rollback: retain additive RICH_HTML_V1; deploy server/editor/versioned display assets together. Older releases strip accounting classes and can lose rule appearance/content on render/save; do not downgrade rule-bearing content without a compatible reader/writer or separately approved recovery. Full border grammar, collapsed-grid/merged-cell semantics and limitations are in `frontend/case-editor/README.md`. Existing Case pages/Preview browser printing remains an acceptance target; Case-aware generation/printing remains deferred.
- Gate: IMPLEMENTATION + FOCUSED VALIDATION PASS for the independently supported automated scope. Next recommended gate: COMMIT PREPARATION, retaining unresolved actual Word/browser acceptance and deferred Case-aware generation/printing. No staging, commit, push, deployment, restart, non-test migration or dependency upgrade occurred.

### Case Editor Smoke Remediation — Implementation / Focused Validation
- Baseline: branch `feat/structured-case-rich-editor-phase2a`, unchanged HEAD `5cffb93d8f01bfff4d01153b7ab160778e52d4c3`, clean starting worktree/index. Scope is exactly the ten approved files; no staging/commit/push/deployment/restart or non-test migration is authorized.
- Completed: shared narrow block-cell boundary whitespace rule before parser conversion and in compatibility comparison; atomic rejected paste with separate bounded readonly HTML/text recovery, retry/dismiss and truthful nonfatal status; fatal serialization blocks Save/Preview; independent Preview errors and stale-response suppression; six labeled compact responsive toolbar groups with keyboard navigation; wrapper-only focus forwarding, editable height/focus/caret styling; versioned CSS/bundle URLs. Existing sanitizer, safety budgets, dependencies/notices, migration 0025, MCQ/generation/printing remain unchanged.
- Changed files: `frontend/case-editor/compatibility.js`, `frontend/case-editor/compatibility.test.mjs`, `static/js/departmental_exam_case_editor.js`, `static/css/departmental_exam_case_editor.css`, `templates/departmental_exams/faculty/case_form.html`, `static/vendor/tiptap/3.31.3/tmp-case-editor.bundle.js`, `apps/departmental_exams/tests_faculty_cases.py`, `CHANGE_LOG.md`, `TEACHERMATEPLUS_CONTEXT.md`, `HANDOFF.md`.
- Frontend validation: final `node --test compatibility.test.mjs` passed 28/28, zero failures/cancellations/skips (121.641s), including both rebuilt-bundle checks, all existing commands, narrow whitespace, recovery/fatal/Preview states and repeated Python canonical round trips. An earlier expanded run also passed 28/28 before the final selection-anchor rollback refinement. Initial 25-test run had one new whitespace-test failure; targeted diagnostic also failed, then passed 1/1 after the shared rule was applied before parsing. A temporary Python NBSP assertion used the wrong serialized representation; corrected expectation passed 1/1 without any sanitizer change.
- Backend validation: `python -B -m django test apps.departmental_exams.tests_faculty_cases apps.faculty_portal.tests_help_guide --no-color --verbosity 1` passed 69/69 in 443.214s, zero failures/errors/skips; disposable test database destroyed normally. The additional NBSP target was `python -B -m django test apps.departmental_exams.tests_faculty_cases.ScenarioContentTests.test_cell_boundary_whitespace_and_meaningful_editor_content_round_trip --no-color --verbosity 1`. Final evidence is 69 distinct Django plus 28 frontend tests; repetitions are not additional distinct coverage. Expected CSRF-negative-test warning and Git LF-to-CRLF notices only. Whitespace checks and full human-readable diff review passed; generated bundle verified by deterministic rebuild and executable bundle tests. Exactly ten approved paths differ, index empty; dependencies/notices, sanitizer and all migrations unchanged.
- Build/check commands: installed portable Node 24.21.0 at `C:\Users\Lenovo\AppData\Local\Temp\tmp-case-node-da076588c81b4445b21d1ee7c74151aa\node-v24.21.0-win-x64\node.exe`; frontend cwd `frontend/case-editor`; `node build.mjs` twice with equal SHA256 `E9F6AC56EF936F89D6933682B8B08938E3C8DE779F6F465223BCD9A19A4891DA` (411,063 bytes, 30 MIT package notices retained unchanged). `python -B -m django check --no-color` passed with zero issues; `python -B -m django makemigrations --check --dry-run --no-color` reported no changes; `python -B -m django migrate --plan --no-color` exited 0 against a unique temporary SQLite path. Every Django invocation used `DJANGO_SETTINGS_MODULE=config.settings.base`, `DJANGO_ENV=local`, `PYTHON_DOTENV_DISABLED=1`, dummy `DJANGO_SECRET_KEY=case-smoke-isolated-test-only`, `DB_ENGINE=django.db.backends.sqlite3`, unique temporary `DB_NAME` and `DJANGO_LOG_DIR`. No development database or repository logs used. The frontend's existing Python canonicalizer subprocess does not initialize Django/database/logging.
- Browser limitation: connected browser inventory is empty. No actual Firefox/Chromium caret, mouse/keyboard, responsive layout, CSP/network or Word clipboard acceptance was performed. No editor screenshot or actual failing clipboard sample was attached; synthetic fixtures are not the user's exact paste. Preserve the original staging cursor uncertainty and prior unresolved Linked MCQ validation predicate/standalone-POST incident.
- Gate: IMPLEMENTATION + FOCUSED VALIDATION PASS for automated scope. Next recommended gate is COMMIT PREPARATION, retaining real browser/Word acceptance as pending. No staging, commit, push, deployment, restart or non-test migration occurred. No repository logs, .env or secrets accessed; test databases/logs are isolated under unique temporary paths.

### Phase A Self-Hosted Tiptap Case Editor
- Date/gate/baseline: 2026-09-09 PHASE A IMPLEMENTATION + FOCUSED VALIDATION in `D:\codex-worktrees\TMP-Structured-Case-Rich-Editor-Phase2A`, branch `feat/structured-case-rich-editor-phase2a`, unchanged HEAD `ae98ba781e94f7629ef0d03dc62252616a53f633`; starting worktree/index were clean. Case narratives only; MCQ stems/choices, generation and printing remain unchanged.
- Completed: pinned self-hosted Tiptap 3.31.3 integration and labeled keyboard-operable toolbar for all requested text/list/alignment/indentation/table/history/clear-formatting commands. Paragraph/cell alignment remain independent. Upstream ProseMirror supplies selection, merge/split and table geometry operations. Additive finite sanitizer semantics preserve intentional editor paragraphs/breaks; unmarked legacy compaction and every existing safety limit remain unchanged, including 50 tables. Preview and Save retain server authority, existing routes and CSRF. Original source is not rewritten on opening; unsupported/lossy initialization disables Save and retains a recovery copy. Captions (read-only), row groups, scope, nested/merged/empty tables, list starts, Unicode and TMP LaTeX are covered. No content version or migration was introduced.
- Changed existing files: `apps/departmental_exams/scenario_content.py`, `apps/departmental_exams/tests_faculty_cases.py`, `apps/faculty_portal/help_guide.py`, `apps/faculty_portal/tests_help_guide.py`, `static/js/departmental_exam_case_editor.js`, `static/css/departmental_exam_case_editor.css`, `templates/departmental_exams/faculty/case_form.html`, `CHANGE_LOG.md`, `TEACHERMATEPLUS_CONTEXT.md`, `HANDOFF.md`. The existing shared Case stylesheet provides Preview/detail/workspace parity; no additional runtime module was changed.
- New files: `frontend/case-editor/{.gitignore,README.md,package.json,package-lock.json,build.mjs,extensions.js,compatibility.js,compatibility.test.mjs}` and `static/vendor/tiptap/3.31.3/{tmp-case-editor.bundle.js,THIRD_PARTY_NOTICES.txt}`. `node_modules` is isolated/ignored; no portable binaries are inside the repository.
- Dependencies/build: exact official versions verified before installation; all 16 direct Tiptap packages are 3.31.3, esbuild is 0.28.2, JSDOM is test-only 26.1.0. Official portable Node 24.21.0 was downloaded outside the repository and its ZIP SHA256 verified against Node's published checksum (`158F7685B44DE51F6C0DF1D153526CBCD3E1BC739A8DFC607721CEF75DE9E541`). Installation used isolated npm user/global config/cache paths and `npm install --ignore-scripts --no-audit --no-fund --registry=https://registry.npmjs.org`. `node build.mjs` verifies and retains all 30 shipped MIT license files. The final bundle is 406,020 bytes; two consecutive builds gave SHA256 `95E0DA4D183AC35464557510BC99EB88819368930022E382E4D4FC9C2E7E5F97`. No Pro/cloud/license-key/runtime-CDN dependency, injected editor CSS or site-wide CSP change.
- Isolated validation setup: every Django command used explicit `DJANGO_SETTINGS_MODULE=config.settings.base`, `DJANGO_ENV=local`, `PYTHON_DOTENV_DISABLED=1`, a dummy `DJANGO_SECRET_KEY`, `DB_ENGINE=django.db.backends.sqlite3`, and unique temporary SQLite/log paths. Only disposable test databases received test-runner migrations. Commands: `python -B -m django test apps.departmental_exams.tests_faculty_cases apps.faculty_portal.tests_help_guide --no-color --verbosity 1`; `node --test compatibility.test.mjs`; `python -B -m django check --no-color`; `python -B -m django makemigrations --check --dry-run --no-color`; `python -B -m django migrate --plan --no-color`; `git diff --check`; full source diff/scope review and deterministic rebuild.
- Additional regression command: `python -B -m django test apps.faculty_portal.tests_help_guide apps.departmental_exams.tests_stage5_views.Stage5FacultyViewTests.test_rendered_standalone_create_and_edit_revision_contract apps.departmental_exams.tests_stage5_views.Stage5FacultyViewTests.test_bootstrap_forms_render_bound_errors_checkboxes_and_csrf apps.departmental_exams.tests_stage5_views.Stage5FacultyViewTests.test_conflict_expiry_and_malformed_responses_use_safe_faculty_shell apps.departmental_exams.tests_stage5_views.Stage5FacultyViewTests.test_manual_html_and_link_shaped_content_stays_literal_in_workspace_and_delete apps.departmental_exams.tests_structured_exam_lifecycle.StructuredExamLifecycleTests.test_feature_off_preserves_automatic_flat_open_without_blueprint apps.departmental_exams.tests_structured_exam_lifecycle.StructuredExamLifecycleTests.test_valid_structure_allows_open_and_permanently_freezes_it apps.departmental_exams.tests_structured_exam_lifecycle.StructuredExamLifecycleTests.test_tenant_campus_department_and_direct_deny_boundaries_remain_fail_closed apps.departmental_exams.tests_stage6_blueprint_readiness.Stage6AuthorizationAndViewTests.test_feature_disable_direct_deny_assignment_loss_and_cross_tenant_fail_closed apps.departmental_exams.tests_stage6_blueprint_readiness.Stage6PlacementScenarioTests.test_invalid_scenario_membership_fails_closed --no-color --verbosity 1` passed 15/15 (6 help tests plus 9 selected regressions), zero failures/errors/skips.
- Final validation: complete Case/help rerun passed 68/68 (587.866s); final frontend suite passed 19/19 (104.361s), including the actual built bundle, toolbar/paste/Preview handlers and repeated Python round trips. Both final runs had zero failures/errors/skips. After build-only whitespace hygiene (escaped significant template-literal spaces and trimmed notice EOF), the final rebuilt-bundle test passed again (1/1); the source/runtime semantics were unchanged. Tracked and all ten new-file whitespace checks passed. Together with the additional run this is 83 Django test executions (77 distinct tests; six help tests repeated), plus 19 frontend tests. Django check reports zero issues, migration drift reports no changes, and isolated migration planning/diff checks passed. Initial runs exposed a fixed empty-table schema priority issue, Windows UTF-8 test-pipe issue, a test's hardcoded cell depth, and one new node-budget fixture that hit the character limit first; the fixture was corrected without changing safety limits. npm initially rejected a shared NUL config path; separate isolated config paths resolved it. The test-only `whatwg-encoding@3.1.1` deprecation warning, expected CSRF-negative-test warning and Git LF-to-CRLF notices are not browser evidence.
- Compatibility/size evidence: detailed 14-table Word-style fixture raw/normalized/canonical lengths are 52,533/27,562/27,562 characters; 25-table fixture lengths are 94,003/49,276/49,276. Three editor/Python-canonicalize/reopen cycles preserve semantic content and idempotence; the 25-table fixture has only 724 canonical characters of headroom. Exactly 50 simple tables remain within budgets; 51-table, excessive node/depth/size/grid rejection and safe rejected-Save recovery remain covered. The table cap never overrides total content limits.
- Remaining risks/exact next steps: the browser-control skill found no connected browser, and its troubleshooting resource was unavailable. JSDOM command/DOM/bundle checks are not real Firefox/Chromium acceptance. Before browser acceptance, test all toolbar actions, mouse/keyboard selection, focus/announcements, justified rich-block whitespace, network/CSP behavior, repeated Save/reopen/Preview/detail and actual Word clipboard content on Firefox plus Chromium. Captions are read-only; grids requiring automatic repair fail closed; unsupported Word numbering rejects. No PDF-converted Word fidelity is promised. MariaDB concurrency, full repository regressions and staging data were not exercised. Preserve the prior unresolved staging Linked MCQ predicate/standalone-POST incident and pending real Word retest. After the automated gate is green, next recommended gate is COMMIT PREPARATION, with these browser/Word retests explicitly still open before deployment.
- Non-actions: no staging, commit, push, deployment, service restart, non-test migration, schema/migration edit (including 0025), paid dependency, system-wide runtime installation, sub-agent, secret/.env access or repository-log access occurred. All changes are unstaged in the authorized worktree.

### Phase 2A Linked MCQ Form Validation and Case Context
- Date/gate/baseline: 2026-09-09 IMPLEMENTATION + FOCUSED VALIDATION in `D:\codex-worktrees\TMP-Structured-Case-Rich-Editor-Phase2A`, branch `feat/structured-case-rich-editor-phase2a`, unchanged HEAD `2f5192de60e353c2a666148f24bdc9408bbbc69d`; starting worktree/index were clean.
- Confirmed finding: an authoritative service `ValidationError` was masked by the generic HTTP 400 page; normalized duplicate choices reproduced it. A fresh rendered linked form succeeded during diagnosis. The exact staging rejection predicate and why the observed POST used the standalone URL remain unknown.
- Completed: question Create/Edit binds only known author-facing service messages to fields or non-field errors and redisplays escaped submitted content and owned Case context; unknown internal diagnostics receive a generic form error. Contribution conflicts remain HTTP 409. Dynamic linked Case IDs must be present and equal server context; fixed sections must match, standalone Case-ID injection rejects, and mutation services receive the server-resolved owned Case ID. Explicit actions distinguish linked creation, standalone creation, and editing. Blank create question revisions remain valid; blank/stale edit revisions remain conflicts. Shared error handling and other mutation routes were not changed.
- Changed files (approved eight only): `apps/departmental_exams/faculty_views.py`; `apps/departmental_exams/contribution_forms.py`; `templates/departmental_exams/faculty/question_form.html`; `apps/departmental_exams/tests_faculty_cases.py`; `apps/departmental_exams/tests_stage5_views.py`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; `HANDOFF.md`.
- Validation PASS: complete Faculty Cases and Stage 5 view modules passed 89/89 (258.553s); additional manual-question service, DOCX row-form, and reopened-Draft regressions passed 24/24 (101.654s). Total: 113 passing tests, zero final failures/errors/skips. All Django commands used `python -B -m django`, explicit `config.settings.base`, dotenv disabled, a dummy test-only secret, unique temporary SQLite identity, and external temporary logs; test migrations ran only in disposable databases.
- Complete-module command: `python -B -m django test apps.departmental_exams.tests_faculty_cases apps.departmental_exams.tests_stage5_views --no-color --verbosity 1`.
- Additional-regression command: `python -B -m django test apps.departmental_exams.tests_stage5_contributions.Stage5ManualQuestionTests apps.departmental_exams.tests_docx_import.DOCXRowFormTests apps.departmental_exams.tests_reopened_draft_access.ReopenedDraftAccessTests.test_b_reopened_partial_draft_allows_add_edit_delete_and_reorder apps.departmental_exams.tests_reopened_draft_access.ReopenedDraftAccessTests.test_j_owner_tenant_campus_and_direct_deny_boundaries_remain apps.departmental_exams.tests_reopened_draft_access.ReopenedDraftAccessTests.test_e_reopened_docx_preview_edit_and_confirmation_preserve_provenance --no-color --verbosity 2`.
- Checks: `python -B -m django check --no-color` passed with zero issues; `python -B -m django makemigrations --check --dry-run --no-color` reported no changes; `python -B -m django migrate --plan --no-color` exited zero against the isolated database identity. `git diff --check` passed; full diff/stat reviewed within the eight-file scope; index remains empty.
- Test development: the initial 89-test run reported seven failures and an intermediate four-test diagnostic run reported two subtest failures. New test helpers were corrected to submit rendered uppercase answer values and model browser textarea opening-newline handling; CSRF token remasking is excluded from value-equality assertions. No runtime workaround was needed. Final runs pass. The deliberate missing-CSRF request emits one expected warning per applicable run; Git reports LF-to-CRLF conversion notices, not whitespace errors.
- Migration/protections: no model/schema change or migration is required; migration `0025` and the rich-content sanitizer/50-table limits are unchanged. Ownership, tenant/campus, RBAC/direct-DENY, lifecycle, quota, CSRF, revision, and Case-membership protections remain authoritative. Stage 6 isolation, Automatic Generation, and printing were not modified.
- Pending/risks: actual staging Word clipboard and Add Linked MCQ browser retests remain pending. Do not claim the staging validation predicate or standalone-route discrepancy is conclusively resolved. Full-repository tests and external-RDBMS concurrency remain outside focused local validation.
- Exact next steps: COMMIT PREPARATION under separate authorization; preserve the validated eight-file diff and keep staging-browser retests pending through the separate publication/deployment gates.
- Non-actions: no staging, commit, push, non-test migration, deployment, restart, destructive Git operation, secret access, or repository log access/modification.

### Phase 2A Faculty Case 50-Table Bound
- Date/gate/baseline: 2026-09-08 IMPLEMENTATION + FOCUSED VALIDATION in `D:\codex-worktrees\TMP-Structured-Case-Rich-Editor-Phase2A`, branch `feat/structured-case-rich-editor-phase2a`, unchanged HEAD `374ad171e3080590d13cdeef48b70d87950ac700`; starting worktree/index were clean.
- Completed: increased only `MAX_TABLES` from 10 to 50, retaining the generated validation message. Nested tables share the document total; suppressed tables are excluded and surviving empty tables count. This is a technical bound, not an academic maximum. All other safety limits, authoritative canonicalization, safe rejected-Save recovery, ownership/tenant/campus/RBAC/lifecycle boundaries, Stage 6 isolation, Automatic Generation, and printing remain unchanged. Faculty help now explains the 50-table allowance and continuing overall content limits.
- Changed files (approved six only): `apps/departmental_exams/scenario_content.py`; `apps/departmental_exams/tests_faculty_cases.py`; `apps/faculty_portal/help_guide.py`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; `HANDOFF.md`.
- Regression coverage: realistic Word-style 14/25-table fixtures preserve cell text, geometry, merged cells, paragraph separation, alignment, Unicode, and idempotence; 50/51 flat and nested table boundaries; suppressed/empty tables; unchanged node and per-table row rejection; authenticated Preview/Create/Edit canonical consistency; and safe formatted 51-table rejected-Save redisplay with recoverable hidden source.
- Validation PASS: `python -B -m django test apps.departmental_exams.tests_faculty_cases apps.faculty_portal.tests_help_guide --no-color --verbosity 2` passed 57/57 in 400.493s (52 complete Phase 2A sanitizer/workflow tests, including ownership/lifecycle/Stage 6 isolation, and five Faculty help-guide tests; zero failures/errors/skips). The preliminary `python -B -m django test apps.departmental_exams.tests_faculty_cases.ScenarioContentTests --no-color --verbosity 1` passed 31/31 in 3.002s; these overlap the final run and are not additional unique tests. `python -B -m django check --no-color` passed with zero issues; `python -B -m django makemigrations --check --dry-run --no-color` reported `No changes detected`; `python -B -m django migrate --plan --no-color` exited zero. `git diff --check` passed with LF-to-CRLF notices only; full diff and `git diff --stat` were reviewed. No Django test/check warnings were reported.
- Isolation: every Django command used `DJANGO_SETTINGS_MODULE=config.settings.base`, `DJANGO_ENV=local`, `PYTHON_DOTENV_DISABLED=1`, an explicit dummy test-only secret, `DB_ENGINE=django.db.backends.sqlite3`, a unique temporary `DB_NAME`, and external temporary `DJANGO_LOG_DIR`. The workflow run applied migrations only to its disposable in-memory test database and destroyed it afterward; `manage.py` was not used.
- Migration: no model/schema changes or new migration; `0025_faculty_case_rich_content.py` is unchanged. No non-test migration was applied.
- Pending/risks: original authenticated staging Word clipboard retest remains pending; automated synthetic fixtures do not establish acceptance of that unavailable payload. Cases within 50 tables can still exceed the unchanged overall budgets. Full repository tests, standalone/browser JavaScript, and external-RDBMS concurrency are outside this focused run.
- Exact next steps: COMMIT PREPARATION under separate authorization. No focused automated blocker remains. Later repeat the original staging Word paste/Preview/Save workflow under its own authorized gate; do not interpret this local result as a real-clipboard browser pass.
- Non-actions: no staging, commit, push, non-test migration application, deployment, restart, destructive Git operation, secret access, or repository log access/modification.

### Phase 2A Staging Word-Paste Blocker Remediation
- Date/gate/baseline: 2026-09-06 narrow implementation/remediation in isolated `D:\codex-worktrees\TMP-Structured-Case-Rich-Editor-Phase2A`, branch `feat/structured-case-rich-editor-phase2a`, starting HEAD `00d94bb9eb2b0761cc2922deb1c02dd964377dea`; starting worktree/index were clean.
- Completed: the server now sanitizes Word HTML, deterministically compacts non-meaningful empty/NBSP/break-only paragraphs, repeated NBSP/break runs, repaired Word block-wrapper empties, and sole meaningful cell paragraphs, then enforces the unchanged canonical limits. Multiple meaningful cell paragraphs, table geometry/rowspan/colspan, alignment, headings/lists/emphasis/sup/sub, Unicode/₱, and LaTeX remain intact. Client paste mirrors the cosmetic cleanup but is not authoritative. Rejected Save redisplay receives a separately sanitizer-produced safe editor value; unrenderable source remains escaped in a non-executable recovery control and is not injected into the contenteditable. Preview and Save still share `canonicalize_scenario_content()`.
- Measured fixture: five Word-style accounting tables, 80 rows, 638 cells, one rowspan, and one colspan measured 3,923 raw elements, 3,285 normalized/sanitized elements, and 733 canonical elements. It saves successfully, is idempotent, and leaves `MAX_NODES = 2000`; genuinely meaningful content over 2,000 elements still rejects.
- Changed files: `apps/departmental_exams/scenario_content.py`; `static/js/departmental_exam_case_editor.js`; `apps/departmental_exams/faculty_views.py`; `templates/departmental_exams/faculty/case_form.html`; `apps/departmental_exams/tests_faculty_cases.py`; `apps/faculty_portal/help_guide.py`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; and this handoff.
- Validation: sanitizer/compaction block 27/27; complete Phase 2A module 46/46; Stage 5 contribution/views plus structured lifecycle/equivalency/features 191/193 with two expected SQLite concurrency skips; Stage 6 blueprint/generation/lifecycle/reporting/audit 73/73; Faculty help guide 5/5. Unique combined result: 317 discovered, 315 passed, zero failures/errors, two expected skips. `python -B -m django check` reported zero issues; `makemigrations --check --dry-run` reported `No changes detected`; `migrate --plan` exited zero and retained `departmental_exams.0023` then `0024` then unchanged `0025`; `git diff --check` exited zero with Windows line-ending notices only. Every Django command imported `config.settings.base` directly with an explicit temporary SQLite identity, dummy check-only secret, and external temporary log directory; `manage.py` was not used.
- Pending/risks: no known automated source blocker remains in this narrow remediation. Node.js is unavailable, so client JavaScript was manually inspected but not executed in a standalone runtime. The exact staging AE313 clipboard payload and authenticated real-browser paste/Preview/Save/cache behavior remain to be rerun; automated Django rendered-form coverage is not a claim of live browser evidence.
- Exact next steps: perform an independent final diff/security review, then repeat the original authenticated staging Word-paste workflow and confirm the current static asset is served before any separately authorized commit/publication gate.
- Non-actions: no commit, push, deployment, normal migration/application, restart, Automatic Generation/Set A/B/questionnaire/DOCX Case change, `.env`/secret access, or repository log access/modification occurred. Migration `0025_faculty_case_rich_content.py` is unchanged.

### Structured Faculty Case Rich Editor Phase 2A
- Date/gate/baseline: 2026-09-06 Phase 2A implementation followed by three blocker-only remediation gates in isolated `D:\codex-worktrees\TMP-Structured-Case-Rich-Editor-Phase2A`, branch `feat/structured-case-rich-editor-phase2a`, exact unchanged HEAD/base `e3d2f7a1169323d1e99cae7021db18df5fb1729a`. The latest gate's starting checks confirmed the existing 29-path payload was unchanged. The protected dirty `D:\teachermateplus` checkout was not modified.
- Completed remediation: `FacultyCaseForm.stimulus` is a real `HiddenInput`, so the rendered form contains exactly one non-visible `name="stimulus"` control with `data-case-source` for Preview/Save synchronization. The sanitizer now keeps tag-identified structural suppression boundaries: matched non-void active containers and all nested content are removed, while any mismatched closing tag inside suppression fails closed and cannot expose later lexically contained content. Self-closing syntax on every non-void drop-with-content root is explicitly rejected as malformed unsupported active markup rather than being treated as an immediate safe close; legitimate HTML void syntax retains its prior normalization behavior and images remain rejected. Each outer/nested table has independent effective-grid state that carries active rowspan occupancy into every later row, reserves complete colspan slot ranges, rejects overlaps, and applies the 20-column maximum to inherited plus new occupancy. Legacy Admin/Reviewer Stage 6 listing, scenario update/delete/member replacement, readiness, and generation-input locking select only `ExamScenario(contribution IS NULL)`, while legacy null-contribution scenario CRUD remains functional. Structured Final Submission locks the union of owned-Case forward members and contribution-Question reverse memberships and rejects any reverse relation outside the contribution-owned Case set.
- Latest sanitizer-gate files: `apps/departmental_exams/scenario_content.py`; `apps/departmental_exams/tests_faculty_cases.py`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; and this handoff. The earlier remediation also changed `apps/departmental_exams/contribution_forms.py`, `faculty_case_services.py`, `blueprint_services.py`, `stage6_views.py`, `generation_readiness.py`, `generation_services.py`, `apps/admin_portal/help_guide.py`, and `apps/faculty_portal/help_guide.py`. The complete current Phase 2A payload remains 29 paths: 17 modified tracked files and 12 untracked files. Every tracked diff and every untracked file was explicitly inspected.
- Validation: the sanitizer/security `ScenarioContentTests` passed 24/24, including exact self-closing rejection for `form`, `iframe`, `object`, `script`, `style`, `button`, `select`, `textarea`, and `option`; self-closing void-element preservation/idempotence; image rejection; the exact form-boundary reproduction; mismatched nested closes; nested/matched suppressed containers; script/style/iframe/object/embed/SVG behavior; unsafe attributes/URLs; Word/VML/OMML/images/comments; raw/canonical length; node/depth/table/grid limits; inherited rowspan/colspan grids; nested table isolation; bounded spans; and idempotence. The definitive Phase 2A module passed 40/40 with zero failures, errors, or skips. The adjacent matrix passed Stage 5/help 108/108, structured lifecycle/equivalency/features 107/109 with two expected SQLite skips for MariaDB/InnoDB-only structured lock scheduling, and Stage 6 blueprint/generation/lifecycle/reporting/audit 73/73. Combined unique final evidence is 330 discovered, 328 passed, zero failed/errors, and two expected skips. `python -B -m django check --no-color` reported zero issues; `makemigrations --check --dry-run` reported `No changes detected`; `git diff --check` exited zero with Windows line-ending notices only; and read-only `migrate --plan` exited zero and explicitly ordered `departmental_exams.0024` then `0025`. Every Django command imported `config.settings.base` directly with an explicit temporary SQLite identity, dummy check-only secret, and external temporary log directory; `manage.py` was never used.
- Migration: `0025_faculty_case_rich_content.py` is unchanged by remediation; no additional migration is required. It was applied only to disposable Django test databases, never a normal Windows database.
- Pending/risks/exact next step: no known source blocker remains in the six remediated review findings, including the three latest sanitizer blockers. Node.js is unavailable, so no standalone JavaScript execution was possible. Authenticated browser Word paste, Preview/Save synchronization, KaTeX, merged/nested-table presentation, and responsive layout; the full repository suite beyond the 330 requested tests; and MariaDB/InnoDB execution remain unverified. Perform those runtime checks and an independent final diff/security review before any separately authorized commit/publication gate.
- Non-actions: no staging, commit, push, deployment, normal-database migration, restart, reset, clean, stash, rebase, amend, revert, force operation, `.env`/secret access, or repository log access/modification occurred. Automatic Generation behavior, Set A/B algorithms/output, questionnaire printing, and DOCX Case import were not changed.

### Release Center Form Action Shadowing Hotfix
- Date/gate/baseline: 2026-09-05 focused implementation/remediation and validation in clean isolated `D:\codex-worktrees\TMP-Release-Center-Form-Action-Hotfix`, branch `fix/release-center-form-action-shadowing`, exact unchanged HEAD/base `ad5c54a3c96a9d2c39ff3a2bf8dfe71b5f5f8b8f`. The protected dirty `D:\teachermateplus` checkout was not modified.
- Completed/root cause: the shared progressive AJAX handler used `form.action`, but every Release Center form intentionally contains a hidden POST control named `action`; browser named-control lookup therefore returned the `HTMLInputElement`, which stringified into the invalid `[object HTMLInputElement]` request path. The handler now resolves `const requestUrl = form.getAttribute("action") || window.location.href;` and fetches that URL. Questionnaire and Answer Key bulk/individual release and revoke forms retain their hidden backend action names and use the same delegated handler; routes, services, lifecycle/equivalency, exact-revision, tenant/campus/RBAC/direct-DENY, confidentiality, Who Viewed, and normal POST/redirect fallback behavior are unchanged.
- Changed files: `static/js/departmental_exam_release_center.js`; new `apps/departmental_exams/tests_release_center_ajax.py`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; and this handoff. No form, template, route, backend, model, service, lifecycle, equivalency, migration, guide, log, or secret file changed.
- Regression/validation: no JavaScript runtime test harness exists in the repository, so the new strongest practical Django `SimpleTestCase` source contract proves the handler uses `getAttribute("action")` plus the current-page fallback, rejects the shadowable `form.action` fetch pattern, and verifies every Questionnaire/Answer Key AJAX form retains a hidden named `action` control covering all six operation types. The focused module passed 2/2. The combined focused plus complete release run discovered 85 tests: AJAX 2/2 passed, Questionnaire 36/37 passed with the previously documented unchanged `test_summary_displays_actual_persisted_set_and_difficulty_counts` failure, and Answer Key 46/46 passed; total 84 passes, one failure, zero errors, zero skips in 1659.028s. The emitted forced `TemplateDoesNotExist` Answer Key traceback is an intentional passing no-view-audit regression. `git diff --check` exited 0 with only Windows LF-to-CRLF notices; `python -B manage.py check --no-color` reported zero issues; and `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`. Django logging was redirected to external temporary directories for every command; protected repository logs remained untouched.
- Pending/risks/exact next step: the hotfix and focused regression are complete, but no authenticated browser/JavaScript runtime smoke or full repository suite was run, and the broader requested matrix remains technically non-green because of its unchanged questionnaire baseline failure. Perform independent diff review and an authenticated browser network smoke proving a hidden `action` form POST targets the Release Center URL before any separately authorized commit/publication gate.
- Non-actions: no staging, commit, push, deployment, migration creation/application, restart, reset, clean, stash, rebase, amend, revert, force operation, structured-lifecycle change, database access outside disposable Django test databases, protected-log change, or secret/credential/environment-file access occurred.

### Structured Lifecycle Configurable Features UI Remediation
- Date/gate/baseline: 2026-09-05 implementation/remediation only in `D:\codex-worktrees\TMP-Structured-Exam-Lifecycle-Phase1`, branch `codex/tmp-structured-exam-lifecycle-phase1`, exact unchanged HEAD `3481f04c87e28ae26eb5c87a687eeb8d8eade983`.
- Completed: rendered the existing `departmental_exam_structured_lifecycle_enabled` Boolean field, label, and help text in the Departmental Exam Builder card on Configuration Management / Configurable Features, between the existing builder and Word-import switches. No backend form, view, service, lifecycle, Automatic Generation, Case/Scenario authoring, or migration behavior changed. Focused GET coverage now verifies the exact label and field name, both neighboring controls, and the field's default-OFF initial state.
- Changed files: `templates/admin_portal/tools/configurable_features.html`, `apps/departmental_exams/tests_feature_flag.py`, and this handoff. Existing behavior documentation already describes the default-OFF feature, so no product guide, context, or change-log update was required for this rendering correction.
- Validation: `apps.departmental_exams.tests_feature_flag` passed 10/10; its real Configurable Features GET is the focused automated page smoke. `python -B manage.py check --no-color` reported zero issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`; and `git diff --check` exited zero with only Windows LF-to-CRLF notices. No authenticated browser smoke or full repository suite was run.
- Pending/risks/exact next step: implementation and focused validation are complete. Perform an independent review and authenticated browser smoke before any separately authorized commit/publication gate.
- Non-actions: no staging, commit, push, deployment, migration creation/application, service restart, lifecycle/Automatic Generation/Case editor change, protected-log modification, or secret/credential/environment-file access occurred.

### Structured Case/Scenario Exam Lifecycle Phase 1
- Date/gate/baseline: 2026-09-04 Phase 1 implementation followed by 2026-09-05 independent review and remaining Blocker 1 ownership remediation in clean isolated `D:\codex-worktrees\TMP-Structured-Exam-Lifecycle-Phase1`, branch `codex/tmp-structured-exam-lifecycle-phase1`, exact unchanged HEAD/local `origin/main` baseline `5ca27ac289d28eeb0751a3153dd9a7a7ffd0a4e4`. The protected dirty `D:\teachermateplus` checkout was not modified.
- Completed: retained the default-off `FEATURE_DEPARTMENTAL_EXAM_STRUCTURED_LIFECYCLE_ENABLED` Phase 1 behavior and closed the authoritative-ownership defects. `ExamBlueprint.save()` derives each protected Django field's normal name and concrete attname, so `cycle_course_id`, `structure_frozen_by_id`, `created_by_id`, and `updated_by_id` cannot bypass the same frozen ownership/evidence checks as their field names while legitimate non-structure saves and the authorized freeze scope still work. Equivalency create/replace now locks the final proposed member set and rejects either multiple independently configured blueprints or a sole blueprint not owned by the proposed authoritative primary, without deleting, reassigning, cloning, merging, or silently changing the primary. Zero-blueprint legacy units and a sole proposed-primary-owned blueprint remain allowed. Structured Open locks every member-owned blueprint and fails closed with an administrative-reconciliation error unless exactly one blueprint exists and belongs to the authoritative primary, protecting legacy/preexisting inconsistent data. The earlier frozen-ownership, cycle-final-count propagation, QuerySet/`bulk_update`, cycle-first locking, and stale Open-vs-mutation remediations remain unchanged, as do tenant/campus/responsible-department/RBAC/direct-DENY, feature-OFF Manual/Automatic behavior, audit, generation, and confidentiality.
- Concurrency test design: two real `TransactionTestCase` cases run only when Django confirms a MariaDB backend. Separate database connections and explicit `threading.Event` barriers load a stale blueprint or section in worker B, hold worker A's Open transaction after it has written freeze evidence but before commit, prove B has started but cannot complete while A holds the cycle-first locks, then release A and assert B re-reads the committed frozen row and raises `ValidationError` without persisting its stale mutation. SQLite skips both because it cannot prove InnoDB row-lock scheduling. MariaDB concurrency test implemented but not executed in this environment.
- Changed files: the complete Phase 1 payload remains exactly 21 files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/forms.py`; `apps/admin_portal/help_guide.py`; `apps/admin_portal/tests_help_guide.py`; `apps/admin_portal/views.py`; `apps/core/services/features.py`; `apps/departmental_exams/blueprint_services.py`; `apps/departmental_exams/exam_units.py`; `apps/departmental_exams/models.py`; `apps/departmental_exams/services.py`; `apps/departmental_exams/stage6_views.py`; `apps/departmental_exams/tests_feature_flag.py`; new `apps/departmental_exams/tests_structured_exam_lifecycle.py`; `apps/departmental_exams/views.py`; new `apps/departmental_exams/migrations/0024_structured_exam_lifecycle_freeze.py`; `templates/admin_portal/guide.html`; and the three Admin exam templates `assigned_course_examination_list.html`, `blueprint_configuration.html`, and `course_configuration.html`. The final sole-blueprint/proposed-primary remediation changed only five of those files: `CHANGE_LOG.md`, `HANDOFF.md`, `TEACHERMATEPLUS_CONTEXT.md`, `apps/departmental_exams/exam_units.py`, and `apps/departmental_exams/tests_structured_exam_lifecycle.py`.
- Validation: the final combined matrix discovered 128 tests: structured lifecycle 32 discovered with 30 passes and two expected SQLite skips; complete exam-course equivalency 67/67; complete Stage 4 cycle-defaults 19/19; and feature flags 10/10. Total: 126 passed, zero failed/errors, two expected skips. Three final regressions prove that create rejects a sole alias-owned blueprint without mutation, primary replacement preserves the existing primary/memberships/blueprint ownership on rejection, and a sole proposed-primary-owned blueprint remains allowed; the existing zero-blueprint and multiple-blueprint cases also pass. `python -B manage.py check --no-color` reported zero issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`; read-only `python -B manage.py migrate --plan --no-color` exited 0 with an 834-line plan; disposable in-memory test databases applied `0024` successfully; and `git diff --check` exited 0 with only LF-to-CRLF notices. No authenticated browser smoke, full repository suite, or actual MariaDB execution was run.
- Migration/status: `departmental_exams.0024_structured_exam_lifecycle_freeze` was not changed by remediation. It remains the additive nullable freeze-evidence migration with no data migration, seed, backfill, ownership rewrite, or legacy-row mutation. No migration was applied to the isolated checkout's normal database; `showmigrations departmental_exams` reports `0001` through `0024` unapplied there. Test database migration does not change that status.
- Pending/risks/exact next step: no known Phase 1 source blocker remains in the authorized ownership scope. Execute the two implemented concurrency cases against an authorized temporary MariaDB/InnoDB test configuration and perform authenticated browser smoke before any separately authorized staging/commit/publication gate. Phase 2 authoring/generation/printing remains intentionally absent. Do not stage, commit, push, deploy, migrate a normal database, or restart under this handoff.
- Non-actions: no staging, commit, push, deployment, normal-database migration, restart, reset, clean, stash, rebase, amend, revert, force operation, secret/environment-file access, protected-log modification, or unrelated worktree change occurred.

### Faculty Summary Print Layout and Campus Scope Remediation
- Date/gate/baseline: 2026-09-04 remediation and focused validation in isolated `D:\codex-worktrees\TMP-Draft-Contribution-Print-Report`, branch `fix/faculty-summary-print-layout`, created cleanly from refreshed `origin/main` baseline `e3eda3337122e6bdb2f10060b0fabde1d20f6879`. The protected `D:\teachermateplus` checkout was not modified.
- Completed: the Draft Summary by Faculty uses a balanced three-column ordered-list layout on desktop/print, responsive two-/one-column fallbacks, stable contiguous Django numbering, and non-splitting entries. The Faculty Submission Summary no longer filters by or displays the viewer's single default campus; it filters current source/offering evidence through the viewer's full authorized campus set, retains authoritative course/RBAC/direct-DENY scope, and displays each contributor's configured Default Campus or `Not set` per row.
- Changed files (12): `CHANGE_LOG.md`, `HANDOFF.md`, `TEACHERMATEPLUS_CONTEXT.md`, `apps/admin_portal/help_guide.py`, `apps/admin_portal/tests_help_guide.py`, `apps/departmental_exams/contribution_selectors.py`, `apps/departmental_exams/monitoring_views.py`, `apps/departmental_exams/tests_contributor_monitoring_draft_print.py`, `apps/departmental_exams/tests_contributor_monitoring_faculty_submission_print.py`, `templates/admin_portal/guide.html`, `templates/departmental_exams/admin/contributor_monitoring_draft_print.html`, and `templates/departmental_exams/admin/contributor_monitoring_faculty_submission_print.html`.
- Validation: final focused tests passed 38/38 with 0 failures, 0 errors, and 0 skips: affected Draft/Faculty Summary reports 12/12, adjacent Contributor Monitoring print/scope regressions 7/7, and Admin Help Guide 19/19. `git diff --check` passed with only Windows LF-to-CRLF checkout notices; `python -B manage.py check --no-color` reported no issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported no changes; and `python -B manage.py migrate --plan --no-color` exited 0 and only listed the isolated empty database's unapplied baseline migration plan without applying it. The generated zero-byte ignored `db.sqlite3` placeholder was removed.
- Pending/risks/next step: implementation and focused validation are complete; the next separately authorized gate may review/prepare the exact 12-file remediation. Authenticated browser print preview, physical printing, the full suite, and external-RDBMS testing remain outside this focused gate.
- Non-actions: no staging, commit, push, main promotion, deployment, migration application, service restart, reset, clean, stash, amend, rebase, revert, force-push, protected-log/database access, or secret/environment access occurred.

### Faculty Contribution Summary Reports and Sequential Numbering
- Date/gate/baseline: 2026-09-04 implementation and focused validation in isolated `D:\codex-worktrees\TMP-Draft-Contribution-Print-Report`, new branch `feat/faculty-contribution-summary-reports`, created cleanly from refreshed `origin/main` and approved baseline `8d7461a9242b0c21034ea4a550f8695c1ddfe134`. The protected `D:\teachermateplus` checkout was not modified or inspected beyond the supplied context.
- Completed: the existing Draft Contribution Report now numbers already-sorted course headings and adds a numbered Draft Summary by Faculty with distinct DRAFT course-contribution counts and case-insensitive last-name/first-name ordering. The new selected-cycle Faculty Submission Summary uses the effective request-scope Default Campus, current scope-matched eligibility sources intersected with course offering snapshots, and the existing monitoring selector/RBAC/direct-DENY boundary; it sequentially numbers faculty and reports distinct Draft, Submitted, and total Courses counts. Exempt, other-campus/cycle/tenant, unauthorized, duplicate-source, and confidential question/answer data remain excluded. No model or migration change is required.
- Changed files (13): `CHANGE_LOG.md`, `HANDOFF.md`, `TEACHERMATEPLUS_CONTEXT.md`, `apps/admin_portal/help_guide.py`, `apps/admin_portal/tests_help_guide.py`, `apps/departmental_exams/monitoring_views.py`, `apps/departmental_exams/tests_contributor_monitoring_draft_print.py`, `apps/departmental_exams/tests_contributor_monitoring_faculty_submission_print.py`, `apps/departmental_exams/urls.py`, `templates/admin_portal/guide.html`, `templates/departmental_exams/admin/contributor_monitoring.html`, `templates/departmental_exams/admin/contributor_monitoring_draft_print.html`, and `templates/departmental_exams/admin/contributor_monitoring_faculty_submission_print.html`.
- Validation: final focused modules passed 37/37 with 0 failures, 0 errors, and 0 skips: affected reports 11/11, Admin help guide 19/19, and adjacent contributor-monitoring print/scope regression 7/7. Earlier development attempts found two fixture errors, one stale-name assertion failure, and one help-topic lookup failure; all were corrected before the final green runs (78 total executions across attempts: 74 passes, 2 failures, 2 errors, 0 skips). `git diff --check` passed with only Windows LF-to-CRLF checkout notices; `python -B manage.py check --no-color` reported no issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported no changes; and `python -B manage.py migrate --plan --no-color` exited 0 and listed the isolated empty database's unapplied baseline migration plan without applying it. The complete tracked and untracked diff was inspected; no migration, database, log, secret, credential, environment, or unrelated parallel-work file is present.
- Pending/risks/next step: implementation and focused validation are complete; the next separately authorized gate may prepare the exact 13-file change for review/commit. Authenticated browser print-preview, physical printing, the full suite, and external-RDBMS testing remain outside this focused gate.
- Non-actions: no staging, commit, push, main promotion, deployment, migration application, service restart, reset, clean, stash, amend, rebase, revert, force-push, protected-log access, or secret/environment access occurred.

### Draft Contribution Print Report Mainline Port
- Date/gate/baseline: 2026-09-04 clean-mainline port and focused validation in isolated `D:\codex-worktrees\TMP-Draft-Contribution-Print-Report`, branch `feat/draft-contribution-print-report`, created exactly from refreshed `origin/main` `56a19668005132b42300eb1ba351e7594f36ce0f`. The protected dirty `D:\teachermateplus` checkout was inspected read-only at `765acd5184c1321dfe64d85ed8d4721c4155ce24`.
- Completed: added a selected-cycle **Print All Draft Contributions** action and GET-only report. It reuses the current mainline monitoring selector with an optional DRAFT prefetch filter, keeps confirmed-import progress and current Manual/Automatic permission behavior, filters Included courses and DRAFT contributions server-side, groups by course, renders the five required columns, configured NCBA identity/address and logo, Asia/Manila time, effective deadline, print CSS, and the required empty state. It exposes no question or answer content. No model or migration change is required.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/departmental_exams/contribution_selectors.py`; `apps/departmental_exams/monitoring_views.py`; `apps/departmental_exams/urls.py`; new `apps/departmental_exams/tests_contributor_monitoring_draft_print.py`; `templates/admin_portal/guide.html`; `templates/departmental_exams/admin/contributor_monitoring.html`; and new `templates/departmental_exams/admin/contributor_monitoring_draft_print.html`.
- Validation: the new report module passed 5/5, then passed 5/5 again after explicit course ordering; the existing Contributor Monitoring print module plus the combined-scope/cycle regression passed 7/7; and the complete Admin Help Guide module passed 19/19. Total execution was 36/36 passes (31 distinct tests), zero failures/skips/warnings. `git diff --check` exited zero with checkout line-ending notices only; `python -B manage.py check --no-color` reported zero issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`; and read-only `python -B manage.py migrate --plan --no-color` exited zero and listed the isolated worktree's unapplied baseline migration chain. Every Django command used an external temporary log directory. The complete diff was inspected and no unrelated implementation or confidential field exposure was found.
- Pending/risks/next step: authenticated browser print-preview smoke, the full repository suite, and external-RDBMS testing were intentionally not run under this focused gate. Next perform an independent read-only review/browser smoke before any separately authorized commit or publication action. Do not commit, push, deploy, migrate, or restart without separate authorization.
- Non-actions: no staging, commit, push, deployment, migration application, service restart, reset, clean, stash, amend, rebase, revert, force-push, secret access, or protected-log inspection occurred.

### Bulk Answer Key Release UX
- Date/gate/baseline: 2026-09-03 implementation/remediation only in clean isolated `D:\codex-worktrees\TMP-Bulk-Answer-Key-Release-UX`, branch `feat/bulk-answer-key-release-ux`, starting exact HEAD/current `origin/main`/approved deployed baseline `856c5a955087baf27765bbecf30a24583a4cdfd8`. The protected dirty `D:\teachermateplus` checkout remained on `feat/departmental-exam-builder-stage5-8` at `765acd5184c1321dfe64d85ed8d4721c4155ce24` and was not modified.
- Completed: converted the shared URL into a compact Bootstrap Release Center with separate Questionnaire Releases and Answer Key Releases tabs, a bounded-height bulk table, default-collapsed individual Answer Key controls/history, and the existing NCBA `#0f5132` / `#13795b` / `#f4b942` dark-green/gold palette. Bulk Answer Key rows expose only authorized rendered metadata for case-insensitive primary/equivalent course code/title, authoritative responsible/Exam Department, examination-unit campus-union, and live derived status filters. Select All Visible changes only visible, filtered, server-eligible, enabled checkboxes. Changing Search Course, Department, Campus, or Release Status automatically deselects rows that become hidden or unavailable, preserves still-visible manual selections, and recalculates selected/visible/checked/indeterminate state; AJAX restoration excludes disabled values and current filters plus a pre-submit guard prune unavailable selections.
- AJAX/preservation: existing CSRF-protected Questionnaire single/bulk release, Questionnaire revoke, Answer Key single/bulk release, and Answer Key revoke forms still call the unchanged authoritative services. Progressive AJAX returns safe message/section/course IDs only, refreshes the bulk status area plus affected course card, preserves scroll/tab/filter context, disables duplicate submission during processing, and keeps real-form POST/redirect fallback. RBAC/direct-DENY, tenant/campus/equivalency scope, exact-current revision binding, attestation, windows, atomicity, idempotency, immutable history, confidentiality, and Who Viewed semantics are unchanged; Admin AJAX creates no Faculty Viewed event.
- Tests/remediation check: focused Answer Key release tests cover rendered filter metadata, filter-change pruning of hidden/ineligible/disabled selections, preservation/counting of visible selections, checked/indeterminate recalculation, pre-submit pruning, AJAX restoration safety, single/bulk success, validation/stale/direct-DENY failure, non-AJAX fallback, authoritative revoke refresh, R4-to-R5 current-row isolation, no Faculty Viewed audit, Questionnaire AJAX release/revoke, and JSON confidentiality. The single affected source-contract test and matching Admin guide test each passed `1/1`; the broader added Answer Key tests and browser smoke were not run under this narrow remediation gate.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/admin_portal/tests_help_guide.py`; `apps/departmental_exams/stage6_views.py`; `apps/departmental_exams/tests_answer_key_release.py`; `static/js/departmental_exam_release_center.js`; `templates/admin_portal/guide.html`; `templates/departmental_exams/admin/answer_key_viewers.html`; `templates/departmental_exams/admin/questionnaire_print_release.html`; and new `_questionnaire_release_pane.html` / `_answer_key_release_pane.html` partials. No model or migration file changed; no migration is required.
- Validation performed: `python -B manage.py test apps.departmental_exams.tests_answer_key_release.AnswerKeyReleaseTests.test_release_center_filter_selection_and_ajax_source_contract --no-color` passed `1/1`; `python -B manage.py test apps.admin_portal.tests_help_guide.AdminHelpGuideTests.test_departmental_exam_output_guides_cover_release_and_audit_operations --no-color` passed `1/1` after aligning its pre-existing `responsible/Exam Department` wording contract; and `python -B manage.py check --no-color` passed with zero issues. Each used a temporary external log directory. The earlier implementation checks also compiled all three release templates and parsed every modified Python file. The tracked `logs/system.log` remained untouched. No local JavaScript runtime or parser was available, so JavaScript execution was not run. Formal broader tests, authenticated browser behavior, and migrations were intentionally not run.
- Pending/risks: source contracts do not replace authenticated browser evidence for filter combinations, visible-only Select All, responsive tabs, focus/scroll preservation, AJAX network/error behavior, or Bootstrap tab activation. Formal focused/adjacent regression validation and browser smoke remain a separate gate.
- Exact next steps: under a separately authorized validation gate, run the new focused Answer Key and Admin help-guide tests, approved adjacent Questionnaire/equivalency regressions, then authenticated desktop/mobile browser smoke for filters, visible-only Select All, single/bulk release/revoke, failure feedback, fallback, tab/filter/focus/scroll preservation, and Who Viewed return navigation. Do not stage, commit, push, migrate, deploy, or restart without separate authorization.
...
- Defense/preservation: locked bulk service validation independently rejects superseded, non-current, non-Generated, and secondary-owned equivalency revisions. The outer transaction, deterministic locking, exact course/revision binding, complete Set A/B checks, tenant/full member-campus/RBAC/direct-DENY enforcement, replacement/history audits, CSRF, and confidentiality controls remain unchanged. Individual exact-revision release and historical output workflows remain explicitly historical-capable.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/admin_portal/tests_help_guide.py`; `apps/departmental_exams/questionnaire_printing.py`; `apps/departmental_exams/stage6_views.py`; `apps/departmental_exams/tests_questionnaire_print_release.py`; `templates/admin_portal/guide.html`; and `templates/departmental_exams/admin/questionnaire_print_release.html`.
- Validation: final bulk/current-only/forged-request/atomic rollback/equivalency/RBAC/DOM-contract subset passed 18/18 in 369.934s; Admin help-guide contracts passed 19/19 in 80.284s. The required complete Questionnaire Print Release module ran 37 tests in 703.007s: 36 passed and one unrelated Automatic Summary assertion failed; the exact failing method `test_summary_displays_actual_persisted_set_and_difficulty_counts` reproduced alone (1 failure in 22.038s) because baseline output contains newer actual-count fields than its stale exact tuple. This session did not change that summary implementation or assertion, and the four-requirement scope was preserved. `git diff --check` exited zero with LF-to-CRLF checkout notices only; `python manage.py check --no-color` reported zero issues; `makemigrations --check --dry-run --no-color` reported `No changes detected`; and read-only `migrate --plan --no-color` exited zero and listed the isolated worktree's existing unapplied migration chain. No migration was created or applied.
- Pending/risks/next step: no browser-JavaScript harness was available in the focused suite, so Select All click/partial/disabled-row behavior has rendered DOM/source-contract coverage but still requires authenticated staging browser smoke. Also independently reconcile the pre-existing Automatic Summary stale tuple in a separately authorized scope; do not fold it into this fix. Next perform a read-only diff/security review and staging browser smoke before any separately authorized commit/publication/deployment gate. Full repository, external-RDBMS concurrency, deployment, and production smoke were not performed.
- Non-actions: no staging, commit, push, deployment, migration application, service restart, production/staging/training access, reset, clean, stash, amend, rebase, revert, force-push, unrelated refactor, secret access, or `logs/system.log` modification occurred.

### Automatic Assignment Solver Performance Fix
- Date/gate/baseline: 2026-08-30 implementation/remediation plus validation in clean isolated `D:\codex-worktrees\TMP-DEB-Assignment-Solver-Performance-Fix`, branch `codex/tmp-deb-assignment-solver-performance-fix`, exact unchanged HEAD and `origin/main` `d0a1f105bb65da382641368576e4cd71b1fb170f`. The protected dirty `D:\teachermateplus` checkout remained on `feat/departmental-exam-builder-stage5-8` at `765acd5184c1321dfe64d85ed8d4721c4155ce24` and was not modified.
- Root cause/completed: the hard phase invoked the full ranked Hungarian `assignment_for()` for every overlap/campus allocation. Its local `row_cache` was recreated per invocation, so it did not reuse identical HMAC-ranked rows across the 1,584-1,800 calls seen in production profiles. Hard feasibility now consumes one configured state and runs exact capacitated max flow on the campus-to-logical-group graph for each candidate, which is polynomial and equivalent to Hall's condition without enumerating `2^campuses` subsets. Candidate order is unchanged; only the first feasible allocation runs the existing ranked Hungarian path. Solver-scope HMAC memoization keys the complete varying inputs (`overlap`, `block_id`, `selection_state`) while secret/domain/base context remain fixed. Difficulty optimization, proportional/contributor metrics, deterministic HMAC selection, hard-valid exhaustion fallback, Manual/scenario behavior, generation/audit/fingerprint paths, and the 300-second timeout are unchanged.
- Performance/correctness evidence: the deterministic 150-row pathological fixtures reproduce 101/102 unique identities, 1,584 baseline `assignment_for` calls, 678,720/678,714 HMAC calls, overlap 20, and independent-review HEAD runtimes of 79.713s/81.516s. After polynomial remediation they retain the exact baseline Set A/B SHA-256 hashes, overlap 20, and 1,584 budgeted feasibility states while using 1 ranked assignment, 301/300 HMAC calls, and 0.722s/0.749s. The exhaustive small oracle compares all 31 two-campus capability multisets; new three/four-campus brute-force cases prove combined-subset Hall deficits; 16/20-campus `max_states=1` fixtures finish in 0.016s/0.015s with exactly one state/check/ranked witness instead of exponential preprocessing. Existing FM311-like/FM322-like/GE213-like outputs retain exact selection hashes and state counts, and IS313-like difficulty proof plus one-state optimization exhaustion remain green.
- Changed files: `apps/departmental_exams/generation_algorithms.py`; `apps/departmental_exams/tests_stage6_algorithms.py`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; and `HANDOFF.md`. No model, migration, settings, timeout, generation service, audit schema/path, UI, URL, guide, log, or secret file changed.
- Validation: remediation production-shape class 14/14 in 2.804s; complete Stage 6 algorithms 38/38 in 4.380s; complete Automatic Workflow 62/62 in 404.909s; complete Automatic Generation Audit 8/8 in 77.713s. Zero failures/skips. Workflow emitted only expected timeout/RuntimeError isolation log lines. `git diff --check` exited zero with LF-to-CRLF checkout notices only; `manage.py check --no-color` reported zero issues; `makemigrations --check --dry-run --no-color` reported `No changes detected`; read-only `migrate --plan --no-color` exited zero and listed the isolated worktree's full unapplied project history because it has no initialized local database. No migration was created or applied.
- Pending/risks/next step: perform an independent read-only review of the polynomial remediation plus production-clone/external-RDBMS timing before any separately authorized commit/publication/deployment gate. The local pathological fixture reproduces the production call shape but is anonymized/synthetic; it does not prove production runtime. Full repository tests and authenticated browser smoke were not run; no UI changed. Preserve all earlier unresolved handoff items below.
- Non-actions: no staging, commit, push, deployment, production/staging/training access, migration application, service restart, reset, clean, stash, amend, rebase, revert, force-push, secret access, generated-record rewrite, or `logs/system.log` modification occurred.

### Departmental Exam Reopened Draft Access Fix
- Date/gate/baseline: 2026-08-29 through 2026-08-30 implementation, independent-review blocker remediation, and focused/broad validation in isolated `D:\codex-worktrees\TMP-DEB-Reopened-Draft-Access-Fix`, detached at exact unchanged HEAD and `origin/main` `6f93377903e1a842e1a33a5ef63dea6f923e6659`. The protected dirty `D:\teachermateplus` checkout was not modified.
- Completed: centralized an explicit reopened-existing-Draft authority in `ContributionAuthorizationService`. It requires the authenticated contribution owner, active tenant/campus/Faculty Portal permission, Included Automatic Generation scope, Open cycle/configuration, a future effective reopened deadline, the original 50-75 snapshotted quota, one retained exact-scope source with durable source-specific `eligibility_proven_at`, and no current `ExamGenerationRevision` across the complete resolved equivalency unit. Ordinary live eligibility remains the primary path. Authoritative roster evaluation sets the proof only when that exact source is eligible and never clears it on later invalidation; never-eligible sources cannot authorize reopen.
- Workflow/UI: authoritative service checks now receive the actor across manual add/edit/delete/reorder, CSV preview/confirm, DOCX preview/edit/confirm, and Final Submission. Faculty listing/workspace render `Reopened Draft` and the deadline while preserving truthful historical assignment and blocked-roster evidence. Expiry immediately restores read-only/deny behavior; Submitted rows remain immutable; current generation in any equivalent member blocks mutation.
- Changed files: the original 15-file candidate plus `apps/departmental_exams/models.py` and new migration `apps/departmental_exams/migrations/0023_faculty_source_eligibility_proof.py`. No URL, setting, generated-exam, assignment, release, Set A/B, or audit-event schema changed.
- Migration/backfill: `0023` adds nullable/no-default `FacultyContributionEligibilitySource.eligibility_proven_at`, then stamps only NULL, non-current sources created after migration `0006` completed and carrying the strict persisted transition `invalidated_at > created_at`. Repository history shows the sole post-`0006` application writer set `is_current=True` only from the complete authoritative inventory, while create-already-invalid rows have `invalidated_at <= created_at`; `0023` records `invalidated_at` as the conservative proof boundary. The structurally evaluated `0006` rows and all ambiguous histories remain NULL. There is no contribution-level or blanket-current backfill.
- Validation: expanded reopened-Draft/provenance/migration matrix passed 20/20; Stage 5 contribution/view/roster/eligibility/CSV/resume/DOCX group passed 189/189; Automatic Open/Restore/workflow/equivalency group passed 140/140; Admin/Faculty help-guide contracts passed 24/24. Total distinct tests: 373/373. All used temporary Django test databases and separately verified/removed temporary log directories. `git diff --check`, `manage.py check --no-color`, `makemigrations --check --dry-run --no-color`, and read-only `migrate --plan --no-color` exited zero; the plan shows `0023` AddField plus the narrow Raw Python proof operation. No migration was applied.
- Pending/risks/next step: perform an independent read-only re-review and authenticated browser smoke before any separately authorized migration/commit/publication gate. Production sources that originated in `0006` or lack the strict transition remain fail-closed and require a separately governed, source-specific evidence review/remediation; do not authorize them from row/contribution existence. External-RDBMS concurrency/lock timing, full repository tests, deployment, and production smoke were not performed.
- Non-actions: no staging, commit, push, deployment, migration application, service restart, production/staging/training access, reset, clean, stash, amend, rebase, revert, force-push, secret access, or `logs/system.log` modification occurred.

### Automatic Generator Performance and Preferred-Difficulty Hotfix
- Date/gate/baseline: 2026-08-29 implementation/remediation plus focused validation in clean isolated `D:\teachermateplus-automatic-generator-performance-difficulty-soft`, branch `fix/automatic-generator-performance-difficulty-soft`, exact unchanged HEAD/baseline `ed7d0828359d1efb0c16dbf19dc0f2ad0c1d8ca4` from freshly fetched `origin/main`. The protected dirty `D:\teachermateplus` checkout was baselined first and not modified.
- Completed solver remediation: Automatic generation removes difficulty from hard margins and proves minimum overlap from total logical identities plus per-campus logical capacity. Its dedicated tri-state fast path lazily enumerates only capacity-feasible shared-campus allocations, charges every Hungarian assignment candidate to the configured state budget, returns proved infeasible without repeating generic recursion, and preserves a hard-feasible selection when only difficulty refinement exhausts. Soft refinement now exhaustively visits every completed difficulty-count vector, lower-deviation count pair, complete campus-by-difficulty table, table pair, shared-cell allocation, and assignment witness until minimum combined Set A/B deviation is proved or the same real state budget is exhausted. The former 64-table, 128-pair, and 24-iteration correctness caps are absent. The independent-review counterexample now returns deviation 2 with proof under `max_states=100000`; `max_states=1` retains a hard-valid result with `optimization_limit_hit=True` and no optimality claim. FM322, GE213, FM311, and IS313 solver timings are 1.596311s, 1.834922s, 2.045681s, and 0.417411s respectively, all far below the unchanged 300-second per-course timeout.
- Workflow/policy: Automatic readiness emits `PREFERRED_DIFFICULTY_UNAVAILABLE` instead of a difficulty-only blocker; exact item count, campus allocation, logical uniqueness, minimum overlap, equivalency, lifecycle, fingerprint/revision, tenant/campus/RBAC, and source evidence remain hard. Locked `build_problem()` carries its authoritative Automatic selection into `generate()`, removing the redundant third solver call while retaining the pre-lock stale-safe solve and locked revalidation/solve. Automatic `GeneratedExamSet.difficulty_quotas_snapshot` stores actual achieved counts; the generation audit event stores target, actual Set A/B counts, deviation, target-met, optimality-proved, and optimization-limit evidence. Summary and exact-revision audit say “closest feasible” only when proof exists; on budget exhaustion they state that the preferred target could not be fully optimized within the processing budget and the best valid difficulty mix found was used. Snapshot-versus-item corruption remains a failure. A current GENERATED course returns before readiness/solver work, and regression snapshots prove its revision, sets, and items remain unchanged.
- Faculty policy: Final Submission still requires the exact valid question count, current lifecycle/assignment/revision, and fully valid payload, but a 30/50/20 mismatch is warning-only. Workspace/confirmation pages show preferred target versus actual counts, and the immutable submission audit records actual counts, target, and target-met status. Duplicate/originality policy, CSV/DOCX behavior, and cross-faculty duplicate handling were not changed.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/departmental_exams/approval_services.py`; `automatic_generation_audit.py`; `automatic_generation_readiness.py`; `automatic_workflow.py`; `contribution_services.py`; `generation_algorithms.py`; `generation_readiness.py`; `generation_services.py`; `tests_automatic_generation_audit.py`; `tests_automatic_workflow.py`; `tests_stage5_contributions.py`; `tests_stage5_views.py`; `tests_stage6_algorithms.py`; `apps/faculty_portal/help_guide.py`; `templates/departmental_exams/admin/automatic_generation_summary.html`; and Faculty `contribution_submit.html` / `contribution_workspace.html`. No model, migration, settings, timeout-isolation implementation, URL, log, or secret file changed.
- Validation: after all budget-accounting changes, the final full algorithm module passed 34/34 in 2.781s, and the two database-backed shortage/exhaustion reporting tests passed 2/2 in 43.340s. The two focused remediation tests separately passed 2/2 in 0.002s. The exact small regression independently brute-forces every hard-valid Set A/B combination and confirms the solver's proved deviation 2, while the one-state variant retains a hard-valid result with optimization exhaustion. The complete production-shape class passed 10/10 in 6.325s with the conservative exact timings above. The consolidated current-generation, Automatic soft-difficulty, budget-exhaustion, IS316/IS317, Manual-preservation, timeout-isolation, summary-query, Faculty submission/view, and audit subset passed 20/20 in 292.219s with zero failures/skips; isolated audit regression also passed 1/1 in 45.046s. Earlier pre-remediation validation remains historical only: complete `tests_automatic_workflow` passed 59/59 in 1303.737s, dedicated spawned-process rollback/parent-reuse/next-course isolation passed 1/1 in 21.613s under `config.settings.spawn_test`, seven equivalency-primary checks plus complete Stage 6 generation passed 22/22 in 325.617s, and Admin/Faculty help guides passed 24/24 in 58.847s.
- Required gates: final `git diff --check` exited zero with LF-to-CRLF checkout notices only; `python manage.py check --no-color` reported zero issues; `python manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`; and read-only `python manage.py migrate --plan --no-color` exited zero and listed the isolated worktree's pre-existing unapplied baseline chain. No migration was created or applied.
- Pending/risks/next step: authenticated browser smoke for Faculty warning/submit and Admin generated-summary rendering, external-RDBMS/concurrency timing, independent security/performance re-review, and the full repository suite remain pending. The exhaustive optimizer is bounded by the shared configured state limit and explicitly distinguishes proved minimum deviation from best hard-valid output retained after exhaustion; the 300-second process timeout remains the final safety net. Next perform an independent read-only diff review and browser/external-RDBMS smoke before any separately authorized commit/publication/deployment gate. Existing production generated examinations remain untouched and must not be rewritten.
- Non-actions: no staging, commit, push, deployment, production/staging/training access, migration application, service restart, reset, clean, stash, rebase, revert, amend, force-push, duplicate/originality workflow implementation, secret access, generated-exam rewrite, or `logs/system.log` modification occurred.

### Automatic Generation Per-Course Timeout Isolation
- Date/gate/baseline: 2026-08-28 to 2026-08-29 implementation/remediation in isolated `D:\teachermateplus-automatic-generation-course-isolation`, branch `fix/automatic-generation-course-isolation`, exact unchanged HEAD/baseline `fd4b88cc79b4e11569db53b916c0ca2724b8ef7a` from freshly fetched `origin/main`. The protected dirty `D:\teachermateplus` worktree was baselined first and not modified.
- Completed: `process_departmental_exam_deadlines` now resolves examination-unit primaries once and gives each primary a fresh `spawn` child. `DEPARTMENTAL_EXAM_AUTOMATIC_COURSE_TIMEOUT_SECONDS` is a required-positive setting with default `300`; timeout cleanup sends terminate, waits up to five seconds, force-kills if still alive, waits again, verifies exit, closes process/pipe handles, persists `ERROR` / `AUTOMATIC_PROCESSING_TIMEOUT`, and continues. The command reports content-safe per-course status/code/revision plus `timeouts=N`; current-generation protection, authoritative exact solver validation, and equivalency primary ownership remain unchanged. Deadline close is still its own transaction; killed in-progress generation relies on the existing `ExamGenerationService.generate()` atomic transaction and database connection-loss rollback, so no partial revision or Set A/B commits. The independent-review C blocker is remediated by a real `spawn`/database `TransactionTestCase`: a child-local barrier pauses inside the open generation transaction after it observes one current revision, two sets, 100 items, one source-audit row, and 50 source-question snapshots; production timeout cleanup terminates that child, every generation row rolls back, the same parent connection remains usable, and the next real spawned course commits successfully. No production code changed for this remediation.
- Lifecycle/summary: CLOSED `BLOCKED`, `ERROR`, or `SKIPPED` rows with a processed timestamp are not retried every five minutes. Governed Reopen Contributions and approved Open-cycle Restore already clear status/code/time and restore eligibility. Timeout rows retain the existing status schema and show a distinct reason/recommendation instead of QUESTION_SHORTAGES, FEASIBILITY_LIMIT, ordinary PROCESSING_ERROR, or pending state. No model or migration changed.
- Changed files: `.env.example`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; new `apps/departmental_exams/automatic_processing_isolation.py`; new test-only `apps/departmental_exams/automatic_processing_spawn_test_support.py`; new `apps/departmental_exams/tests_automatic_processing_isolation.py`; `apps/departmental_exams/automatic_workflow.py`; `apps/departmental_exams/management/commands/process_departmental_exam_deadlines.py`; `apps/departmental_exams/tests_automatic_workflow.py`; `config/settings/base.py`; new test-only `config/settings/spawn_test.py`; `docs/DEPLOYMENT_UBUNTU.md`; `templates/admin_portal/guide.html`; and this handoff.
- Validation: remediation rerun: the new real spawn/database rollback/parent-reuse/next-course test passed 1/1 in 8.731s under `--settings=config.settings.spawn_test`; the existing timeout/isolation matrix passed 7/7 in 32.950s; and the relevant generated/current/shortage/feasibility/close/reopen/Restore/equivalency/tenant/feature subset passed 10/10 in 65.068s. Earlier validation remains: focused matrix 17/17, full Open/Restore 11/11, Admin Help Guide 19/19, and complete Automatic Workflow 58/59 with the sole unchanged pre-existing stale wording assertion proven at `origin/main`. After remediation, `git diff --check` passed with checkout line-ending notices only; `manage.py check --no-color` reported zero issues; `makemigrations --check --dry-run --no-color` reported `No changes detected`; and read-only `migrate --plan --no-color` exited zero, listing the worktree's full unapplied baseline chain because its ignored `db.sqlite3` is zero bytes. No migration was applied. The dedicated spawn settings are required only for SQLite because Django's default in-memory test database is not visible to a fresh spawn interpreter; configured external test RDBMS runs use their normal test database.
- Pending/risks/next step: perform a focused independent read-only re-review, then separately authorize an Ubuntu/external-RDBMS staging smoke that deliberately exceeds a short per-course setting, verifies the child PID is gone, confirms no partial revision/sets, confirms the next real course completes, and checks the Summary/reopen retry flow. Production cron is intentionally disabled and must remain disabled until the exact remediation is deployed and that smoke is accepted. Browser UI smoke, MariaDB connection-loss rollback observation, full repository tests, commit, push, deployment, service restart, and cron re-enable were not performed.
- Non-actions: no commit, push, deployment, production/staging/training access, service restart, non-test migration, reset, clean, stash, rebase, revert, amend, force-push, secret access, or `logs/system.log` modification occurred.

### Automatic Generation Summary 504 Remediation
- Date/gate/baseline: 2026-08-28 implementation/remediation only in isolated `D:\teachermateplus-automatic-summary-post-generation-timeout`, branch `fix/automatic-summary-post-generation-timeout`, exact clean baseline/current HEAD `59c64261987d8ab02c9ab7f75b2e14f65ff9f74f` from freshly fetched `origin/main`. The protected dirty `D:\teachermateplus` checkout remained on `feat/departmental-exam-builder-stage5-8` at `765acd5184c1321dfe64d85ed8d4721c4155ce24` and was not modified.
- Completed: `AutomaticGenerationSummaryService` now fetches only current GENERATED revisions and their persisted sets/items/source-audit snapshots. Generated rows derive actual Set A/B counts, audit pool metrics, warnings that can be established from persisted/current aggregate evidence, revision metrics, and the regeneration fingerprint from those records; the GET never rebuilds a generated course's `GenerationProblem`. Persisted automatic-processing statuses/codes render BLOCKED/ERROR/equivalent processed rows without live readiness. A residual unprocessed Closed row uses only `evaluate_automatic_pool(exact_feasibility=False)`. Equivalency units are cached/rebound to prefetched cycle rows and continue to render the primary once.
- Regeneration boundary: the POST/action and `ExamGenerationService.generate()` are unchanged. They still lock generation inputs, call authoritative solver-enabled `Stage6ReadinessService.build_problem()`, compare the submitted current revision/fingerprint with locked live inputs, and run `solve_automatic_identity_aware_two_sets()` before superseding or creating generation records.
- Changed files: `apps/departmental_exams/automatic_workflow.py`; `apps/departmental_exams/tests_automatic_workflow.py`; `apps/departmental_exams/tests_exam_course_equivalency.py`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; and `HANDOFF.md`. No model, migration, generation service, solver algorithm, view, URL, template, log, or secret file changed.
- Tests added/changed: generated-summary build/solver bypass through the real GET; persisted source-audit metrics and current-revision fingerprint; authoritative regeneration readiness/solver invocation; persisted BLOCKED and ERROR bypass; residual `exact_feasibility=False`; equivalency primary-once with zero exact calls; current-only revision prefetch; and bounded query growth for eight generated courses with exact solver count zero. Per the current gate, these tests were **NOT RUN**. `python manage.py check`, migration commands, smoke tests, and the Django suite were also **NOT RUN**. Diff inspection and `git diff --check` found no whitespace errors (only checkout line-ending notices); this is not execution validation.
- Migration status: No migrations are required. No migration was created or applied.
- Pending/risks/next step: run the separately authorized focused validation gate, including the new regressions, existing Automatic workflow/equivalency/generation compatibility, `manage.py check`, migration-drift inspection, and authenticated browser timing against production-shaped data. Then perform an independent read-only security/performance review before any separately authorized staging/commit/publication gate. Production MariaDB/Gunicorn timing remains unverified.
- Non-actions: no staging, commit, push, migration application, deployment, service restart, reset, clean, stash, amend, rebase, revert, force-push, log modification, secret access, or production/staging/training action occurred. Unrelated work was preserved.

### Automatic Generation Readiness Performance Hotfix
- Date/gate/baseline: 2026-08-27 to 2026-08-28 implementation/remediation plus focused validation in isolated `D:\teachermateplus-automatic-readiness-performance`, branch `fix/automatic-readiness-performance`, exact unchanged baseline HEAD `cda7fe5871a29ec5a6ab684ea01140d178738c11` from local `origin/main`. The protected dirty `D:\teachermateplus` checkout was inspected first at `765acd5184c1321dfe64d85ed8d4721c4155ce24` on `feat/departmental-exam-builder-stage5-8` and was not modified by this session. No commit, push, deployment, service restart, normal-database migration application, staging, reset, clean, stash, amend, rebase, revert, force-push, log edit, or secret access occurred.
- Completed remediation: normal Automatic Generation Readiness screen and print GETs no longer invoke exact Set A/B feasibility. `Stage6ReadinessService.evaluate_automatic_pool()` retains its solver-enabled default for authoritative callers but now accepts an explicitly report-only `exact_feasibility=False` boundary; that boundary preserves Submitted/usable/invalid counts, normalized logical duplicate grouping, participating-campus representation/policy, and total/campus/30-50-20 difficulty aggregate shortages, then stops before feasibility vector construction and `solve_automatic_identity_aware_two_sets()`. `generation_algorithms.py` is unchanged.
- Cheap blockers/status: the report now classifies EXEMPT and GENERATED first, then cycle/configuration/quota/deadline/workflow/processing/roster/contributor/unresolved Blocked Draft conditions before pool work where they are decisive. Aggregate-sufficient future-deadline rows remain `WAITING FOR DEADLINE`; aggregate-sufficient due rows are `EXACT FEASIBILITY PENDING` with `Aggregate requirements met \u2014 exact generation feasibility pending.` They are never labeled exact Ready. Aggregate shortages remain `NEEDS QUESTIONS` or `BLOCKED` with existing concrete actions; AVAILABLE_WITH_WARNING/STRICT and equivalency-unit behavior remain intact.
- Authoritative generation: ordinary solver-enabled `build_problem()` is still the default and `ExamGenerationService.generate()` still re-locks/revalidates inputs and invokes the unchanged exact automatic solver before Set A/Set B creation. Final count, campus/difficulty allocation, normalized uniqueness, overlap, deterministic selection, contributor balancing, equivalency, RBAC/tenant/campus isolation, and lifecycle/reopen controls were not weakened.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/departmental_exams/automatic_generation_readiness.py`; `apps/departmental_exams/generation_readiness.py`; `apps/departmental_exams/tests_automatic_generation_readiness_report.py`; `apps/departmental_exams/tests_exam_course_equivalency.py`; and `templates/admin_portal/guide.html`. No model, migration, template for the report itself, generation service, workflow service, or algorithm file changed.
- Validation: finalized targeted hotfix/authoritative-generation coverage passed 8/8 in 189.112s. The finalized Automatic Generation Readiness module passed 38/38 in 934.240s. Admin Help Guide passed 19/19 in 81.598s. The requested broader Stage 6 algorithms, Stage 6 generation, Automatic workflow, and equivalency run completed 156 tests in 2893.384s: 155 passed and one unchanged pre-existing/stale wording assertion failed in `test_query_bound_for_multiple_draft_courses_and_campuses` (`Open contributions / complete course configuration.` expected versus current `Complete the course configuration and open contributions before automatic generation can proceed.`); both `automatic_workflow.py` and its test file are byte-unchanged relative to `origin/main`. This broad run is not reported as green. An earlier in-flight broad run was deliberately interrupted after final test/code changes and is not credited. `python manage.py check --no-color` reported zero issues; `makemigrations --check --dry-run --no-color` reported `No changes detected`; read-only `migrate --plan --no-color` exited 0 with the fresh worktree's 826-line full unapplied baseline chain and applied nothing. Its ignored zero-byte `db.sqlite3` artifact was verified and removed. Final diff/status checks must be rerun after this handoff edit.
- Pending/risks/next step: authenticated browser screen/print smoke, production-shaped MariaDB timing/concurrency, Gunicorn/Nginx load testing, and the full repository suite were not run. Aggregate validation/fingerprinting remains linear per otherwise-eligible visible unit, so production latency should still be measured even though combinatorial search is removed. The unchanged stale Automatic-summary wording test remains a separate test-maintenance issue. Next perform an independent read-only diff/security/performance review and production-shaped request timing before any separately authorized staging/commit/publication gate.

### DOCX Real-Word OPC Compatibility and Explicit Step 2 Difficulty Selection
- Date/gate/baseline: 2026-08-27 implementation, focused remediation, and validation in `D:\\codex-worktrees\\TMP-DOCX-Import-MVP-Implementation`, branch `feat/docx-question-import-mvp`, unchanged HEAD `a1509fd8746633c26db075d742509b90d532e3e7`. No staging, commit, push, migration application, deployment, service restart, reset, clean, stash, amend, rebase, revert, or force-push occurred.
- Completed: corrected a relationship-validation false positive that treated the standard OPC core-properties URI (`.../package/2006/relationships/metadata/core-properties`) as an embedded package merely because the complete URI contained `package`. Screening now applies only to the final relationship type name, so normal Word metadata is accepted while an actual `/relationships/package` relationship and the other unsupported kinds remain rejected. Added adversarial coverage for both outcomes.
- Difficulty contract: `Difficulty:` may be present or absent in the uploaded DOCX. An omitted value reaches the owner-only editable Step 2 preview as a field error and remains empty rather than receiving an inferred default. The DOCX-only staged-row form now renders a required blank `Select difficulty` choice first, does not implicitly select Easy, rejects a blank POST, and requires an explicit Easy, Moderate, or Difficult selection before authoritative validation permits import. Confirmation remains blocked while the row is invalid; existing DOCX difficulty values, shared CSV behavior, and Final Submission difficulty rules are unchanged. Upload and help guidance explain this behavior.
- Real-file result: direct parsing of `C:\\Users\\Lenovo\\Downloads\\CMC MIDTERM.docx` now reports no file rejection and detects 50 rows. All 50 are currently invalid staged rows because the source file has no per-question `Answer:` or `Difficulty:` paragraphs; it also has incomplete typed choices (including question 5 missing B, question 6 missing C, and 29 questions with no detected choices). These source-content issues are correctable in Step 2 or by editing and re-uploading the Word file; they are distinct from the fixed OPC false rejection.
- Changed files: `apps/departmental_exams/contribution_forms.py`; `apps/departmental_exams/docx_import.py`; `apps/departmental_exams/tests_docx_import.py`; `templates/departmental_exams/faculty/docx_upload.html`; `apps/faculty_portal/help_guide.py`; `CHANGE_LOG.md`; `TEACHERMATEPLUS_CONTEXT.md`; and `HANDOFF.md`. No shared CSV implementation, model, or migration file changed.
- Validation: the remediated DOCX suite passed 48/48 in 32.690s, including the blank selected option, no implicit Easy selection, blank POST rejection, invalid-row confirmation denial, all three explicit valid values, and preservation of explicit DOCX difficulty. `python manage.py check --no-color` reported no issues, `python manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`, and `git diff --check` passed with only the existing LF-to-CRLF checkout warnings. CSV was intentionally not rerun because no shared CSV code changed; the preceding delta validation remains 57/57 CSV/resume and 5/5 Faculty help. The direct CMC parser smoke remains 50 detected, 0 valid, and 50 editable invalid rows without a package-level rejection.
- Pending/risks/next step: authenticated browser upload/review smoke remains pending. The CMC source still requires missing answers, difficulties, and incomplete choices before import. Next, independently review the eight-file unstaged delta from `a1509fd8746633c26db075d742509b90d532e3e7`; selective staging, commit, and push require separate authorization.

### DOCX Question Import MVP, Text Integrity Remediation, and Progress Indicator
- Date/gate/baseline: 2026-08-26 implementation plus focused validation in clean isolated worktree `D:\\codex-worktrees\\TMP-DOCX-Import-MVP-Implementation`, branch `feat/docx-question-import-mvp`, unchanged HEAD/base `2499adf35d890cf81d55cf3799abe5917bd33692`. The protected dirty `D:\\teachermateplus` checkout was baselined at `7b53ba85a7f38e445b772bf009403b2d06419233` and was not modified. No staging, commit, push, normal-database migration, deployment, restart, log edit, or secret access occurred.
- Completed: added tenant-configurable, default-OFF `FEATURE_DEPARTMENTAL_EXAM_DOCX_IMPORT_ENABLED`; faculty DOCX upload, durable preview, row editing, validation, duplicate warnings, resumable chunk confirmation/status, replay protection, and shared CSV/DOCX question persistence. DOCX batches preserve tenant/campus/owner/contribution scope, deadline/lifecycle/roster/INCLUDED/quota/current-revision checks, create `Question` rows with `entry_method=DOCX`, advance the contribution revision exactly once, purge staged rows, and leave the contribution Draft for the existing Final Submit workflow.
- Parser/security: in-memory `.docx`/OPC parsing uses `defusedxml`, a 2 MB upload cap, ZIP member/count/uncompressed-size/compression-ratio bounds, canonical safe member paths, external/unsafe relationship rejection, and no office subprocess or resource extraction. It accepts only ordinary typed paragraphs in the MVP grammar and rejects macro-enabled content, external/unsafe relationships, ActiveX/OLE/embedded packages, media, charts, headers/footers, notes/comments, altChunk, tables, automatic lists, drawings/text boxes, math, and tracked changes. The independent-review text-integrity blockers are remediated: no-break and soft hyphens map to their exact Unicode characters, meaningful blank paragraphs and continuation indentation are preserved, structural separator blanks remain non-content, and font-dependent `w:sym`, generated fields/references, ruby/bidirectional/alternate-content structures, plus any unrecognized run child reject the complete file instead of disappearing. Existing sanitation, shared payload validation, and normalized duplicate detection remain authoritative; raw DOCX bytes are not persisted.
- Progress UX: the DOCX form now progressively enhances to show accessible upload/processing status. Browser upload events drive real percentages; after upload it switches to an animated indeterminate processing state with honest stage labels. Only the submit control and file input are disabled, repeat submits are blocked, failures restore the form, success redirects to the durable preview, and no-JavaScript form POST remains functional. Existing CSV behavior is retained and no fake backend percentage or new background subsystem was added.
- Migration impact: new unapplied `departmental_exams.0022_docx_question_import` adds `QuestionImportBatch.source_format`, permits `Question.entry_method=DOCX`, and updates the two related DB checks. Existing batches default to CSV. There is no custom data migration, seed, question rewrite, or normal-database application.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/forms.py`; `apps/admin_portal/views.py`; `apps/core/services/features.py`; `apps/departmental_exams/contribution_forms.py`; `apps/departmental_exams/csv_import.py`; new `apps/departmental_exams/docx_import.py`; `faculty_views.py`; `models.py`; `urls.py`; new migration `0022_docx_question_import.py`; new `tests_docx_import.py`; `apps/faculty_portal/help_guide.py`; `requirements/base.txt`; `static/js/departmental_exam_csv_import.js`; `templates/admin_portal/guide.html`; `templates/admin_portal/tools/configurable_features.html`; `templates/departmental_exams/faculty/contribution_workspace.html`; and new `docx_upload.html`, `docx_preview.html`, and `docx_row_edit.html`. The text-integrity remediation changed only `docx_import.py`, `tests_docx_import.py`, `CHANGE_LOG.md`, `TEACHERMATEPLUS_CONTEXT.md`, and this handoff; the rest of the 23-path implementation inventory was preserved.
- Validation: remediation-focused parser tests passed 16/16 in 0.028s; the required full DOCX suite passed 23/23 in 25.842s; and the required CSV/resumability regression pair passed 57/57 in 210.317s, all with zero skips. `git diff --check`, `python manage.py check --no-color`, `python manage.py makemigrations --check --dry-run --no-color`, and read-only `python manage.py migrate departmental_exams --plan --no-color` all exited zero; no model change was detected and no migration was applied. Because the isolated default SQLite file has no migration state, the plan lists the full unapplied chain and ends with the unchanged five `0022` operations. The duplicate-member security fixture emits one expected Python `zipfile` warning. Earlier MVP validation also included combined DOCX/progress, feature/help, and Faculty help-guide runs; no timed-out run is credited.
- Pending/risks/next step: authenticated browser upload/redirect and accessibility smoke, actual Word-authored file sampling, MariaDB/production-RDBMS validation, JavaScript parser/linter validation, broad full-repository tests, and independent read-only re-review of this remediation remain outstanding. Node.js is unavailable in this environment, so `node --check` did not run. The MVP intentionally rejects font-dependent symbols and complex Word structures and does not claim antivirus scanning. Next perform the independent read-only remediation review and browser smoke; only then consider separately authorized migration, selective staging, commit, and publication gates.
...
- Pipeline/consistency: sanitation runs after strict structural CSV parsing and before validation, fingerprinting, duplicate detection, and durable preview persistence. Changed rows receive one content-safe generic warning. The resumable chunk path consumes and revalidates the same staged sanitized payload; existing question text is sanitized only in a duplicate-comparison copy, and original upload bytes still determine `file_sha256`.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/departmental_exams/csv_import.py`; `apps/departmental_exams/tests_stage5_csv.py`; `apps/departmental_exams/tests_stage5_csv_resume.py`; and `apps/faculty_portal/help_guide.py`. No model or migration file changed.
- Validation: the complete modified CSV modules passed 57/57 in 208.062s with zero failures/errors/skips; the targeted new sanitation slice passed 9/9 in 32.466s; and Faculty Help Guide tests passed 5/5 in 4.457s. `python -B manage.py check` reported zero issues, `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`, and `git diff --check` exited zero with line-ending notices only. No normal database migration was applied.
- Pending/risks/next step: authenticated browser smoke, MariaDB, and the full project suite were not run. Next gate is Validation, including an independent focused diff/security review and browser preview smoke if available. Commit, push, deployment, restart, and migration application remain separately forbidden until explicitly authorized.

### Automatic Generation Readiness Report Layout Remediation
- Date/gate/baseline: 2026-08-24 to 2026-08-25 implementation/remediation in the clean authorized worktree `D:\teachermateplus-readiness-mainline-integration`, branch `fix/readiness-mainline-integration`, exact starting/current HEAD `037d09a422d3b6013ffa446067b64065455dd60e`. No commit, push, deployment, restart, migration application, staging, reset, clean, stash, amend, rebase, revert, log edit, or secret access occurred.
- Completed presentation: screen and print now share exactly seven columns in this order: No., Authorized Course, Campuses Offered, Final Exam Items, Contribution Progress, Automatic Exam Generation Status, and Action Needed. `forloop.counter` numbers only rendered authorized/filtered rows, so every filtered result starts at 1. Final Exam Items is centered and fixed/preferred at 8%; after independent review found header overflow, screen and print now explicitly allow the header to wrap while keeping body values no-wrap and centered. Column widths favor Course, Contribution Progress, and Action Needed. Screen table/summary text remains 12px with compact spacing and inherited-size details; print body/table text remains 10.5pt in landscape with compact 3px/4px cells.
- Completed summary: both templates include one shared end-of-report partial headed `Report Summary`. It shows Total Authorized Courses plus Ready for Generation, Waiting for Deadline, Needs Questions, Faculty Incomplete, Blocked, Generated, and Exempt. The composer counts only the already rendered `rows` by their existing final `generation_status`; it does not recalculate readiness or include hidden/unauthorized courses. Contribution Progress, status precedence, Action Needed, generation feasibility, RBAC/direct-DENY, confidentiality, roster, campus, difficulty, AVAILABLE_WITH_WARNING/STRICT, and generator behavior are unchanged.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/departmental_exams/automatic_generation_readiness.py`; `apps/departmental_exams/tests_automatic_generation_readiness_report.py`; `templates/admin_portal/guide.html`; new `templates/departmental_exams/admin/_automatic_generation_readiness_summary.html`; and both Automatic Generation Readiness screen/print templates. No model or migration file changed; the index is empty.
- Validation: after the overflow remediation, the exact requested `python -B manage.py test apps.departmental_exams.tests_automatic_generation_readiness_report --verbosity 1` completed 37/37 in 875.738s with zero failures/errors. Coverage now explicitly requires the 8% dedicated column, wrap-capable/non-nowrap header, centered no-wrap body cells, and safe screen/print CSS while retaining seven headers, numbering, filtered renumbering, font contracts, summary scope/parity, confidentiality, and unchanged readiness semantics. The earlier supplemental required-guide class passed 19/19 in 88.405s and was not rerun because guide content did not change in this remediation. Post-remediation `git diff --check`, `python -B manage.py check`, and `python -B manage.py makemigrations --check --dry-run --no-color` all exited 0; migration drift reported `No changes detected`.
- Repository checks: final pre-handoff `git diff --check` exited 0 with only LF-to-CRLF working-copy notices; `python -B manage.py check` reported zero issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`. Read-only `python -B manage.py migrate --plan --no-color` exited 0 and listed the checkout's pre-existing 810-line unapplied chain because `db.sqlite3` is an existing zero-byte file; it applied nothing and this remediation requires no migration.
- Pending/risks/next step: authenticated browser visual smoke, print preview/physical output, MariaDB, and broad/full-suite validation were not run. Next perform an independent read-only diff/UI review of these ten paths; only after separate authorization may the exact approved paths be staged or committed. Commit, push, deployment, restart, and migration application remain forbidden.

### Readiness Mainline Integration
- Date/gate/baseline: 2026-08-24 implementation/remediation in fresh worktree `D:\teachermateplus-readiness-mainline-integration`, branch `fix/readiness-mainline-integration`, exact unchanged HEAD `02c9ceb59f286821050a13617d880ebf05f20202` from fetched `origin/main`. The approved readiness commit `acce1c7bebfe89f0d23993f264b4d0659db4fe4e` was applied with `git cherry-pick --no-commit` and unstaged; the cherry-pick auto-merged `apps/departmental_exams/contribution_services.py` without conflict. No commit, push, migration application, deployment, restart, amend, rebase, stash, revert, hard reset, clean, log edit, or secret access occurred.
- Completed integration review: the current-main `ContributionDifficultyDistributionService`, configurable Easy/Moderate/Difficult weights, Final Submission enforcement, dynamic Draft reclassification guidance, and CSV/Draft permissiveness remain intact. The readiness addition in the shared service is limited to persisting roster source tenant/campus through `ContributorEligibilityService.source_evidence_scope()`. That same canonical resolver remains in eligibility fallback, OPEN live/persisted comparison, and live roster evidence; genuine source/staffing changes remain stale, Submitted attribution remains frozen, and CLOSED roster semantics remain frozen.
- Report contract: screen and print retain exactly six columns: Authorized Course, Campuses Offered, Final Exam Items, Contribution Progress, Automatic Exam Generation Status, and Action Needed. Monitoring numerator is saved-question volume on Submitted contributions; denominator is the materialized DEFAULT/OVERRIDE quota multiplied by distinct participating campus snapshots. Submitted campuses appear beneath progress, actions render as bullets, routine duplicate statistics stay hidden, current Faculty Completion uses authoritative `ContributorRosterReadinessService` counts, and historical Submitted questions remain pool inputs without counting toward current completion. `AVAILABLE_WITH_WARNING` continues to use represented campuses when feasible; `STRICT` continues to block missing campus representation. The existing Stage 6 pool/generation services remain authoritative for usable/unique inputs, exact feasibility, and 30/50/20 per-set difficulty allocation.
- Missing-configuration remediation: `_row()` now leaves roster status detail blank when an authorized Included Automatic course has no `CourseExamConfiguration`, instead of dereferencing the absent object after execution status had already classified it as `BLOCKED`. The report remains read-only, does not create a configuration or initialize a roster, preserves `Configure the course examination.` as the action, and lets existing monitoring fields degrade to their unconfigured semantics. The only implementation/test files additionally modified within the preserved 13-path inventory are `automatic_generation_readiness.py` and `tests_automatic_generation_readiness_report.py`; this handoff was also updated as required.
- Changed inventory remains exactly 13 unstaged paths: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/departmental_exams/automatic_generation_readiness.py`; `apps/departmental_exams/blueprint_services.py`; `apps/departmental_exams/contribution_authorization.py`; `apps/departmental_exams/contribution_services.py`; `apps/departmental_exams/tests_automatic_generation_readiness_report.py`; `apps/departmental_exams/tests_stage6_lifecycle.py`; `templates/admin_portal/guide.html`; and both Automatic Generation Readiness templates. No model or migration file changed.
- Focused validation: the exact requested readiness/roster/help command passed 59/59 in 1058.446s; the 12 unique difficulty selectors introduced or modified across `52efc4c`, `0008fc6`, and `02c9ceb` passed 12/12 in 204.328s. Combined terminal evidence is 71 passed, 0 failures, 0 errors, 0 skips. No broad/full suite or authenticated browser/MariaDB smoke ran.
- Missing-configuration validation: the new direct-composer/screen/print regression passed 1/1; the required full readiness report module plus `ContributorRosterEffectiveScopeReadinessTests` passed 41/41 in 197.973s with 0 failures, 0 errors, and 0 skips. Both endpoints returned 200, rendered the course, `BLOCKED`, the Configure action, and six columns, while the configuration count remained unchanged. Difficulty-related code and tests were untouched by this remediation.
- Command validation: final `git diff --check` exited 0; `python -B manage.py check` reported zero issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`; read-only `python -B manage.py migrate --plan --no-color` exited 0 and listed the fresh worktree database's pre-existing 810-line unapplied chain. This integration requires no migration, and none was applied.
- Pending/risks/next step: authenticated browser smoke, MariaDB/concurrency validation, and broad/full suites remain unperformed. The next gate is independent read-only review or separately authorized selective staging/commit; commit, push, migration, deployment, and restart remain forbidden until explicitly authorized.

### Simplified Automatic Generation Readiness Management Report
- Date/gate/baseline: 2026-08-24 implementation/remediation in `D:\teachermateplus-roster-readiness-fix`, branch `fix/roster-readiness-effective-scope`, exact starting/current HEAD `8fcae0367d7923063f6345e88a4f50b2a0cd3632`. The expected 13-path legacy-null roster remediation was present and preserved. No staging, commit, push, migration application, deployment, restart, log edit, secret access, clean, stash, reset, rebase, amend, or broad suite occurred.
- Completed report/UI: screen and print now share exactly six columns: Authorized Course, Campuses Offered, Final Exam Items, Contribution Progress, Automatic Exam Generation Status, and Action Needed. Removed management columns remain internal only. Contribution Progress uses all saved questions on Submitted contributions for the exact CycleCourse as its non-deduplicated numerator and effective materialized DEFAULT/OVERRIDE quota multiplied by distinct participating campus snapshots as its monitoring-only denominator; Submitted campus names are deduplicated from participating frozen `source_campus` attribution. Exempt progress is Not applicable.
- Completed readiness/status/actions: the existing Stage 6 pool evaluator and generator policy remain authoritative for usable/unique questions, 30/50/20 per-set difficulty margins, campus handling, and exact two-set feasibility. Status precedence is EXEMPT, GENERATED, genuine operational/configuration/roster BLOCKED, REQUIRE_ALL FACULTY INCOMPLETE, technical NEEDS QUESTIONS, WAITING FOR DEADLINE, then READY FOR GENERATION. AVAILABLE_WITH_WARNING still permits a technically sufficient one-campus pool for a multi-campus course; STRICT remains blocking. Routine duplicate statistics are hidden. `action_items` contains plain concise strings and both templates render one `<li>` per issue/action.
- Completed GE213 correction: internal current-completion counts now use only `ContributorRosterReadinessService.required_active_count`, `submitted_required_count`, and `incomplete_active_count` when `roster.current` is true; stale counts are not presented as authoritative. Historical Submitted contributors retain pool questions and frozen campus attribution but do not count toward current OPEN-roster completion. The existing canonical legacy-null effective-scope resolver, genuine-stale behavior, Submitted immutability, Closed frozen boundary, and distinct Initialize/Synchronize wording are unchanged.
- Changed paths: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/departmental_exams/automatic_generation_readiness.py`; `blueprint_services.py`; `contribution_authorization.py`; `contribution_services.py`; `tests_automatic_generation_readiness_report.py`; `tests_stage6_lifecycle.py`; `templates/admin_portal/guide.html`; and both Automatic Generation Readiness templates. No model or migration file changed.
- Focused validation: final consolidated command passed 59/59 in 230.128s with 0 failures/errors/skips: all 35 Automatic Generation Readiness report tests, all 5 `ContributorRosterEffectiveScopeReadinessTests`, and all 19 `AdminHelpGuideTests`. A targeted correction rerun passed 3/3 and the repaired guide assertion passed 1/1. Earlier exploratory runs exposed and resolved one invalid-row expectation mismatch, two same-scope test-fixture uniqueness errors, and static/dynamic guide wording drift; no production failure remains. No broad/full suite ran.
- Command validation: `git diff --check` exited 0 with CRLF notices only; `python -B manage.py check` reported zero issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`; and read-only `python -B manage.py migrate --plan --no-color` exited 0 while listing the checkout database's pre-existing unapplied migration chain. This remediation requires no migration, and none was applied.
- Pending/risks/next step: authenticated browser smoke, MariaDB/concurrency validation, and broad/full suites were intentionally not run. Next gate should be independent read-only review of the exact 13-path diff, followed only by separately authorized selective staging/commit/push if approved.

### Contributor Roster Readiness Legacy-Scope Remediation
- Date/gate/baseline: 2026-08-23; implementation/remediation in clean isolated worktree `D:\teachermateplus-roster-readiness-fix`, branch `fix/roster-readiness-effective-scope`, exact starting and current HEAD `8fcae0367d7923063f6345e88a4f50b2a0cd3632` from verified `origin/main`. The dirty source checkout `D:\teachermateplus` retained its exact pre-existing status. No staging, commit, push, migration application, deployment, restart, log edit, or secret access occurred.
- Completed/root cause: `ContributionRosterService` persisted legacy `(NULL, NULL)` assignment evidence with offering tenant/campus fallback, but OPEN `ContributorRosterReadinessService` and `live_roster_evidence()` compared the same rows with raw null assignment IDs. `ContributorEligibilityService.source_evidence_scope()` now owns the persisted/live evidence formula; the approved `_effective_scope()` legacy fallback, roster source creation, OPEN persisted/live comparison, and live close-time hash all reuse it. Eligibility still rejects partial/mismatched scope, and new/removed/ineligible Faculty, assignment/offering evidence, Active/Blocked, effective tenant/campus, and direct-DENY/eligibility drift remain fail-closed stale conditions. Submitted rows still skip live attribution rewrites/comparison, and Closed readiness still uses only its frozen persisted boundary.
- Wording/UI: screen and print now render `Contributor roster has not been initialized.` only when the initialization marker is absent and direct Action Needed to `Initialize the contributor roster.` An initialized non-current roster renders `Contributor roster needs synchronization because the eligible faculty list has changed.` and retains the Synchronize action. The combined `stale or not initialized` sentence was removed.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/departmental_exams/automatic_generation_readiness.py`; `blueprint_services.py`; `contribution_authorization.py`; `contribution_services.py`; `tests_automatic_generation_readiness_report.py`; `tests_stage6_lifecycle.py`; `templates/admin_portal/guide.html`; and both Automatic Generation Readiness screen/print templates. No model or migration file changed.
- Focused validation: new legacy-null/readiness/report regressions passed 7/7 in 31.034s after one assertion-only refinement; they cover one and multiple legacy-null Drafts, AE105-FAR-equivalent 1 Submitted + 3 Draft, genuine new-staffing stale detection, Submitted frozen attribution, Closed frozen boundary, initialized/current report output, and distinct not-initialized/stale screen/print wording/actions. Twenty-two named existing Stage 5 synchronization/eligibility/direct-DENY, Submitted immutability, Stage 6 close/readiness, Automatic OPEN Restore, Automatic Generation Readiness, and due actual-generation regressions passed 22/22 in 108.249s. The focused Admin guide regression passed 1/1 in 0.996s. No broad/full suite ran.
- Command validation: final `git diff --check` exited zero with line-ending notices only; `python -B manage.py check` reported zero issues; `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`; and read-only `python -B manage.py migrate --plan --no-color` exited zero and displayed the checkout database's pre-existing 810-line unapplied migration chain. This remediation requires no migration, and none was applied.
- Pending/risks/next step: authenticated browser smoke, MariaDB/concurrency validation, and broad/full suites were intentionally not run. Perform an independent read-only review of the exact 13-path diff, especially canonical evidence scope, eligibility fail-closed behavior, submitted/closed freezing, and screen/print wording, before any separately authorized selective staging or commit gate.

### Automatic Generation Readiness Report
- Date/gate/baseline: 2026-08-23; implementation/remediation plus focused validation in new isolated worktree `D:\teachermateplus-generation-readiness-report`, branch `feat/automatic-generation-readiness-report`, exact clean starting HEAD/current `origin/main` `aa22a2bcfa3b298b66f844a65600229cddb9f09c`. The protected dirty checkout `D:\teachermateplus` was baselined before creation and left unchanged. No staging, commit, push, migration application, deployment, restart, log edit, or secret access was authorized or performed.
- Independent-review remediation: the later remediation session revalidated the named worktree at the same exact branch/HEAD with the existing 13-file implementation present and preserved. `AVAILABLE_WITH_WARNING` now carries authoritative named missing-campus warnings into screen, print, and Action Needed without changing a permitted pool from `READY`; `STRICT` still returns `NEEDS QUESTIONS`/`BLOCKED`. The Contributor Completion link is conditional on existing Included generation-management or visible Exempt inclusion-management authority and remains hidden/403 for Manual-only configurators/reviewers and feature OFF. Lightweight course/campus ID authorization now determines and validates the selected/default cycle before any configuration, contribution, offering-snapshot, or current-revision prefetch; only selected-cycle rows are evaluated. System-error and solver-limit wording now refers the course to the system administrator without logs, codes, stack details, or unexplained technical direction.
- Exempt-only visibility remediation: the implementation gate revalidated the same exact branch/HEAD and preserved the existing 15-path report implementation. `ContributionMonitoringSelector.visible_cycle_courses()` now considers only Included and Exempt Automatic candidates, retaining the existing Included-only downstream permission map for Included rows and reusing the existing status-independent inclusion-management map for Exempt rows. This lets a legitimately scoped Exempt-only inclusion manager open Contributor Completion, see the authorized Exempt course and readiness link, and open its `EXEMPT` report row. Tenant, feature, every-participating-campus, GLOBAL/NULL, and direct-DENY rules remain shared and fail closed. Exempt rows do not enter `automatic_manage_ids`, keep `can_configure=False`, and expose neither Initialize nor Synchronize; Manual configurer/reviewer and Included Automatic behavior are unchanged.
- Completed: added a visible `Automatic Generation Readiness` action from Contributor Completion, a separate cycle-bounded GET-only screen, and a standalone printer-friendly view. One shared composer applies set-based Included `manage_exam_generation` and Exempt inclusion-management authority across distinct participating Campus IDs with tenant/feature/active-role/GLOBAL-NULL/direct-DENY semantics; absent cycle input defaults to one newest authorized cycle, and malformed/stale/out-of-scope Cycle/Period/Course filters preserve their submitted values and return zero rows on screen and print. Rows show authoritative snapshot campuses, materialized effective faculty quota plus DEFAULT/OVERRIDE source, actual per-contributor quota completion, explicit campus Required/Available/Status, unique usable total, `READY` / `NEEDS QUESTIONS`, execution status, and concrete action.
- Architecture/safety: `Stage6ReadinessService.evaluate_automatic_pool()` reuses the existing Submitted validation, confirmed-import handling, normalized logical groups, campus/difficulty allocation, overlap, and exact Automatic two-set solver while omitting only cycle/deadline/contribution-close/roster gates from pool feasibility. Default `build_problem()` and `ExamGenerationService.generate()` behavior are unchanged. Actual status separately retains invalid Submitted row, REQUIRE_ALL, stale roster, unresolved Blocked Draft, deadline, lifecycle, processing-error, current-generation, and Exempt policy. Screen/print are private/no-store, evaluate the pool once per visible Included row, never call `AutomaticExamDeadlineService`, never mutate state/audit, and never render question text, choices, answers, fingerprints, source IDs, GenerationProblem, or solver-state details. Contributor Completion's existing four-column print template is unchanged.
- Effective quota source: `CourseExamConfiguration.questions_required_per_faculty` is the existing materialized effective value used by roster initialization/submission; `questions_required_per_faculty_source=DEFAULT` records inherited cycle-default materialization and `OVERRIDE` records the course-level value. Production report code has no literal 50 quota rule; 50 appears only in focused fixture/final-item data. Focused regressions prove cycle-default 60 and course-override 75 display/completion behavior.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; new `apps/departmental_exams/automatic_generation_readiness.py`; `contribution_selectors.py`; `generation_readiness.py`; new `generation_readiness_views.py`; `monitoring_views.py`; `services.py`; new `tests_automatic_generation_readiness_report.py`; `urls.py`; `templates/admin_portal/guide.html`; `templates/departmental_exams/admin/contributor_monitoring.html`; and new `automatic_generation_readiness.html` plus `automatic_generation_readiness_print.html`. No model, migration, permission seed, menu, CSV, cache, generation algorithm, deadline processor, log, secret, or Contributor Completion print file changed.
- Validation: the definitive feature module passed 14/14 in 78.208s with zero failures/skips, covering non-50 default/override quota, pre-deadline pool feasibility, unchanged generator gate, one/two/three-campus snapshot allocation and duplicate-campus dedupe, invalid/duplicate/shortage semantics, contributor policies, stale/unresolved roster blockers, all six execution statuses, every-campus/global/direct-DENY/tenant scope, fail-closed screen/print filters, GET no-mutation/confidentiality, parity, one feasibility call per row, and bounded shared query count. After the final aggregate-only confidentiality refinement, those 14 plus Automatic logical-duplicate generation/source-audit persistence and Manual immutable set/content-safe audit regressions passed 16/16 in 96.430s. The earlier existing readiness/generation selection produced 15 valid passes plus one mistyped loader selector; the intended historical-pool test was rerun under its correct class and passed, for 16/16 valid regressions. A combined feature/Admin-guide run passed 31/32; its only failure is a baseline-existing full-static-guide assertion for phrases already absent from exact HEAD before this work, so it was not broadened into this report remediation. The first pre-optimization feature command timed out without a terminal result and is not credited. `git diff --check` exited zero with line-ending notices only; `python -B manage.py check` reported zero issues; and `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`. Read-only `migrate --plan --no-color` exited zero and showed the isolated checkout/database's full pre-existing 810-line unapplied chain; no operation was applied and this feature adds no migration. Django logs were redirected under `%TEMP%`.
- Remediation validation: the expanded readiness-report module passed 22/22 in 126.430s with zero failures/skips. It covers named AVAILABLE_WITH_WARNING screen/print/action output with `READY` retained, STRICT blocking, Included/Manual-reviewer/Manual-configurer/direct-route/feature-off link RBAC, two unrelated historical cycles excluded from heavy prefetch, malformed-cycle zero-load behavior, one solver call per visible selected-cycle Included row, bounded query growth, mixed quota snapshots, unconfirmed imports, nonparticipating-campus questions, exact infeasibility, and academic-facing processing wording. A separate focused existing-generation plus Contributor Monitoring screen/print command passed 15/15 in 77.414s with zero failures/skips. The final strengthened two-historical-cycle SQL regression passed 1/1 in 5.099s. Two intermediate fixture setup errors from duplicate cycle keys were corrected in test setup and are not credited as passing. Final `git diff --check` exited zero with line-ending notices only; `python -B manage.py check` reported zero issues; migration drift reported `No changes detected`; and read-only `migrate --plan --no-color` exited zero with the same pre-existing 810-line unapplied local chain. No migration was created or applied.
- Exempt-only validation: the exact seven focused selector/monitoring/readiness regressions passed 7/7 in 31.570s with zero failures/skips, covering authorized Exempt-only screen/link/report access, visible `EXEMPT` status, absent roster actions, unauthorized every-campus fail-closed scope, direct-DENY precedence, Included behavior, Manual configurer/reviewer behavior, feature OFF, tenant/filter fail-closed behavior, and the existing Exempt presentation contract. The first attempt produced 6 passes plus one fixture-only duplicate-cycle-key setup error; it was corrected with a distinct exam period and is not credited as passing. Final `git diff --check` exited zero with line-ending notices only; `python -B manage.py check` reported zero issues; and `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`. Read-only `python -B manage.py migrate --plan --no-color` exited zero and showed the configured checkout/database's pre-existing 810-line unapplied chain; no migration was created or applied. Django logs were redirected under `%TEMP%`. No broad/full suite was run.
- Cross-mode RBAC remediation: `ContributionMonitoringSelector.visible_cycle_courses()` now restricts the Manual configurer and assigned-reviewer querysets to authoritative `MANUAL_REVIEW` cycles before unioning their IDs with Automatic scopes. Contributor Completion now computes Manual `can_configure` only from the Manual configurer set and Automatic `can_configure` only from the Included-only generation-management map; unknown modes fail closed. Automatic Included remains visible/configurable only with `manage_exam_generation` across every participating campus, Automatic Exempt remains visible only through status-independent inclusion management and non-configurable for roster actions, and Manual configure/reviewer behavior is preserved. Mixed Manual authority cannot bypass an Automatic direct DENY. Exact remediation files are `apps/departmental_exams/contribution_selectors.py`, `apps/departmental_exams/monitoring_views.py`, `apps/departmental_exams/tests_automatic_generation_readiness_report.py`, and this `HANDOFF.md` update; the other pre-existing 16-path implementation changes were preserved.
- Cross-mode validation: the five new mixed-mode regressions passed 5/5 in 19.368s; the full focused Automatic readiness-report module passed 30/30 in 156.150s; and 13 existing affected Contributor Monitoring presentation/print, Manual reviewer, Automatic list-partition, and filter regressions passed 13/13 in 60.542s. All terminal runs had zero failures/skips. One earlier five-test attempt had 4 passes and one fixture-only expectation failure because a separate authorized Main-campus Exempt row kept the page available; both Automatic fixture rows were then moved to the wrong campus and the batch passed, so the failed attempt is not credited. `git diff --check` exited zero with line-ending notices only; `python -B manage.py check` reported zero issues; migration drift reported `No changes detected`; and read-only `migrate --plan --no-color` exited zero with the same pre-existing 810-line unapplied local chain. No migration was created or applied, no broad/full suite was run, and Django logs remained redirected under `%TEMP%`.
- Pending/risks/next step: authenticated browser/device and print-preview smoke, actual MariaDB behavior, broad/full suite, and an independent read-only review remain unperformed. The configured local database is not migration-current, so its full plan is not evidence of a feature migration. Next perform an independent focused authorization/confidentiality/UI review of the exact 16-path diff before any separately authorized staging or commit gate.

### Contributor Monitoring Mainline Integration
- Date/gate/baseline: 2026-08-23; implementation/remediation plus focused validation in new isolated worktree `D:\teachermateplus-contributor-monitoring-mainline`, branch `fix/contributor-monitoring-mainline`, exact clean starting HEAD/current `origin/main` `dd75ab505e0737f04a9b21f45db7a1cf46f00f75`. The approved source was `eaef632fd1c672e997913f07ea2a4160fe1ec7a7`; its feature parent diverged from current main at `73ba82af35e1ef001f8cd1d14e1064e3cb7bd8e8`, so changes were integrated semantically without cherry-pick or commit. The dirty source checkout `D:\teachermateplus`, including `logs/system.log`, was inspected and left untouched.
- Completed: preserved newer mainline/Automatic OPEN Restore behavior while adding the approved Submitted success badge, Date Submitted column, authorized contributor filter with fail-closed invalid/out-of-scope handling and zero-match card removal, filter parity through print/roster/blocked-resolution redirects, Exempt warning badge, hidden Exempt roster controls, and locked service-level INCLUDED guard. The print report remains exactly four contributor columns, and RBAC/tenant/campus/department/direct-DENY selection boundaries are unchanged.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/departmental_exams/contribution_services.py`; `monitoring_views.py`; `stage6_views.py`; new `tests_contributor_monitoring_presentation.py`; `tests_stage6_lifecycle.py`; `templates/admin_portal/guide.html`; and both contributor-monitoring templates. No model or migration file changed.
- Validation: one initial 14-selector command timed out at 304.6s without a terminal summary, left only its two exact Python processes, and is not credited; those processes were stopped before retry. The unbuffered retry of the same 14 focused tests passed 14/14 in 283.975s with zero failures/skips, including approved presentation/filter/Exempt denial, existing print and Included roster behavior, Stage 6 redirect parity, existing Automatic monitoring filters, and two current-main Automatic Restore roster regressions. Django logs were redirected under `%TEMP%`. `git diff --check` exited zero with line-ending notices only; `python -B manage.py check` reported zero issues; and `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`. Read-only `python -B manage.py migrate --plan --no-color` exited zero with the isolated worktree's 810-line pre-existing unapplied migration chain; it applied nothing and this integration adds no migration. The exact eleven-path inventory and complete tracked diff were inspected; the four unchanged-overlap feature files match the approved source blobs exactly. No broad/full suite was run.
- Pending/risks/next step: authenticated browser/device smoke, MariaDB concurrency/locking, and the broad/full suite remain unrun. Perform a short independent read-only review of the exact eleven-path diff, focusing on fail-closed contributor scope, Exempt direct-action denial, print four-column stability, and preservation of Automatic Restore before any separately authorized staging or commit gate. No commit, push, migration application, deployment, restart, log edit, or secret access occurred.

### Automatic OPEN Restore
- Date/gate/baseline: 2026-08-23; missing-roster implementation/remediation plus focused validation in `D:\teachermateplus-automatic-open-restore-mainline`, branch `fix/automatic-open-restore-mainline`, exact clean starting HEAD `4b1397112bd55c5badd94ab6b5d26bd809dca3ee`. The protected `logs/system.log` was not touched; Django logs were redirected to `%TEMP%`. No staging, commit, push, deployment, restart, or normal-database migration was performed.
- Completed: Automatic Generation `OPEN` / `EXEMPT` Restore now inspects the locked configuration's authoritative `contributor_roster_initialized_at` marker. A null marker calls the existing `ContributionRosterService.initialize_for_open_locked(...)`; an initialized roster retains `_synchronize_locked(..., initializing=False)`. Both paths remain inside the existing atomic Restore transition and preserve the approved deadline, generated-revision, reviewer, RBAC/direct-DENY, Manual workflow, contribution/question/import/snapshot, Automatic reset, and audit behavior. Initialization uses current authoritative eligibility, creates at most one contribution per course/faculty, records all current assignment sources, and creates no questions.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/departmental_exams/services.py`; `apps/departmental_exams/tests_automatic_open_restore.py`; and `templates/admin_portal/guide.html`. No model or migration file changed.
- Validation: the first exact four-selector attempt timed out after 184.1s without a terminal summary and is not credited. The unbuffered retry of the same four selectors passed 4/4 in 113.795s with zero failures/skips, covering first-time initialization and deadline preservation, current eligibility plus duplicate prevention, existing-roster synchronization, and rollback after completed initialization writes. `git diff --check` exited zero with line-ending notices only; `python -B manage.py check` reported zero issues; and `python -B manage.py makemigrations --check --dry-run --no-color` reported `No changes detected`. Read-only `python -B manage.py migrate --plan` exited zero and displayed the checkout's pre-existing 810-line unapplied plan beginning with initial migrations; it applied nothing and this remediation adds no migration. No broad module or project suite ran.
- Pending/risks/next step: authenticated browser smoke, MariaDB lock-scheduling verification, and broader compatibility tests were not run under this narrow gate. Perform a short independent read-only review of the exact six-file diff, focusing on the locked initialized-state branch, initialization rollback, unchanged existing-roster synchronization, and unchanged Manual/RBAC/direct-DENY/current-generation boundaries before any separately authorized staging or commit.

### Production Reports + Question Bank Reconciliation
- Date/gate/baseline: 2026-08-21; reconciliation and validation only in `D:\teachermateplus-monday-faculty-print`, new branch `staging/production-reports-question-bank`, exact unchanged HEAD/base `7221a7b944d7bcebc3c2c67138ab37e06679a45e` from current `origin/main`. Applied only source commit `9150ebc6b0d865abe0557507bc7c6711b23c4f14` with `git cherry-pick -n`; no final commit was created.
- Reconciliation: the only conflicts were `CHANGE_LOG.md` and `HANDOFF.md`. They were resolved manually by retaining current Production entries for Administrative Faculty Assignment Acceptance and both Assigned Courses/Exempt Courses print reports, then adding the Question Bank presentation/migration history. `TEACHERMATEPLUS_CONTEXT.md` and all non-documentation Question Bank paths applied cleanly. Production implementation paths remain byte-identical to `origin/main`; Question Bank non-documentation paths remain byte-identical to the source commit.
- Candidate scope: 15 paths relative to `origin/main`: the three combined documentation files; Question Bank migration/view/help/compatibility tests; Faculty Help Guide source/tests; and five Faculty Departmental Exam templates. The change remains presentation-only; protected routes, views, models, RBAC/permission codes, workflow terminology, contribution identifiers, and Production report/acceptance behavior are unchanged. Candidate-specific migration impact is only additive data migration `navigation.0025_rename_faculty_question_bank_menu`.
- Validation: Question Bank migration/Stage 5/Help Guide/Answer Key/Personalized Answer Sheet/Questionnaire Print Release ran 104 tests in 447.447s with 103 passes, one expected SQLite `select_for_update` skip, and zero failures; Administrative Assignment Acceptance passed 72/72 in 145.872s; Assigned/Exempt Courses print reports passed 39/39 in 10.580s. New-session fast revalidation after `git fetch origin` confirmed exact `origin/main`/HEAD `7221a7b944d7bcebc3c2c67138ab37e06679a45e`: the two authorized modules passed 8/8 in 4.783s; `git diff --check`, `python -B manage.py check`, and `makemigrations --check --dry-run` passed; read-only `migrate --plan --no-color` included the configured database's full pre-existing unapplied chain plus `navigation.0025`; required file/presentation and anchored conflict-marker audits passed. No migration was applied.
- Pending/risks/next step: authenticated browser/physical print smoke, MariaDB, and the full repository suite remain unperformed. The two documented stale exact-set assertions were not part of this focused reconciliation run and remain separately non-blocking. The candidate is intentionally dirty/indexed; do not commit, push, migrate, deploy, or restart without a separate gate.

### Assigned Courses Print Reports
- Date/gate/baseline: 2026-08-21; focused implementation and validation only in new clean worktree `D:\teachermateplus-exempt-courses-print`, branch `feat/exempt-courses-print`, unchanged starting/ending HEAD `3e8235a379e3c85d3b62dfad7b9bde8bde6c7fc6` fetched from exact `origin/main`. The dirty source `D:\teachermateplus` was inspected and left untouched. No staging, commit, push, deployment, restart, normal-database migration application, log edit, or secret access occurred.
- Completed: added adjacent `PRINT ALL` and `Print Exempt Courses` actions on Assigned Course Examinations with separate GET-only standalone HTML print routes. One shared scope helper preserves the page's feature gate, tenant isolation, exact Manual configurer/assigned-reviewer contract, Automatic downstream/inclusion-management participating-campus authority, and direct-DENY precedence. PRINT ALL includes every authorized Included/Exempt row, globally sorts Course Code then title/cycle/ID, assigns sequential display numbers after sorting, lists accurate distinct cross-cycle contexts, and reproduces Exam Department, snapshot campuses, effective configuration sources, inclusion status, and readiness without Administer, Configure Override, links, forms, checkboxes, or mutation controls. Print Exempt Courses filters authoritative `CycleCourse.inclusion_status=EXEMPT` after authorization and adds category/reason. Both are unpaginated, have safe empty states, and perform no course/configuration/exemption/reviewer/audit writes. No permission, model, migration, PDF, or workflow change was added.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/departmental_exams/urls.py`; `apps/departmental_exams/views.py`; new `apps/departmental_exams/tests_exempt_courses_print.py`; `templates/admin_portal/guide.html`; `templates/departmental_exams/admin/assigned_course_examination_list.html`; and new `templates/departmental_exams/admin/assigned_courses_print.html` plus `exempt_courses_print.html`. Planning & Readiness, exemption services/models, logs, secrets, and unrelated modules are unchanged.
- Validation: the final combined PRINT ALL plus Exempt report module passed 39/39 in 46.973s. Its first expanded run passed 37/39 and exposed only two assertion defects around duplicate display text and partial normalized HTML; production code was unchanged and the complete rerun passed. Existing EXEMPT/Restore passed 21/21 in 67.112s; seven selected Assigned Courses/Automatic regressions passed 7/7 in 44.213s; and the Admin guide module passed 19/19 in 35.493s. `git diff --check` exited zero with line-ending notices only; final `python -B manage.py check` reported zero issues; `makemigrations --check --dry-run` reported `No changes detected`; and read-only `migrate --plan --no-color` exited zero with the clean worktree's 808-line existing unapplied plan and applied nothing. No migrations are required.
- Pending/risks/next step: authenticated browser print preview, physical printer output, MariaDB, and the full repository suite were not run. Next perform an independent read-only authorization/UI/print review, then proceed only under the separately authorized `COMMIT PREPARATION / SELECTIVE STAGING` gate. Recommended commit message: `feat(departmental-exams): add assigned courses print reports`.

### Administrative Faculty Assignment Acceptance
- Date/gate/baseline: 2026-08-19; implementation/remediation plus focused validation in new clean worktree `D:\teachermateplus-admin-assignment-acceptance`, branch `fix/administrative-faculty-assignment-acceptance`, unchanged HEAD `887db2fbdc39e43c41843528eb3c90738443d276`. It was created from clean source `D:\teachermateplus-monday-faculty-print` after verifying the same committed Personalized Answer Sheets HEAD. The source worktree was not modified.
- Completed/remediated: exact permission-grant-path authorization now prevents `faculty_assignments.update` from composing with unrelated Role department scope. Exact applicable direct ALLOW remains department-global within its tenant/campus; active permission-bearing memberships cover only their own null/global or hierarchical department scope; direct DENY remains first. Execution locks and re-fetches term-scoped assignments/offerings, the complete mutable hierarchy, faculty/actor accounts, FACULTY/actor memberships and Roles, and applicable permission rows before rebuilding all decisions and the hash. Inactive or tenant-inconsistent non-null Exam Departments are ineligible and Exam Department identity/state is hashed. Readiness separately reports inactive UserRole membership and inactive linked Role. Admin/Faculty wording now says Accepted may mean faculty acceptance or authorized administrative confirmation while retaining truthful `accepted_by`. The previously approved transition/audit/idempotency design is preserved; no model or migration was added.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; `apps/admin_portal/help_guide.py`; `apps/admin_portal/views.py`; `templates/admin_portal/dashboard.html`; `templates/admin_portal/guide.html`; `templates/faculty_portal/guide_manual.html`; new `apps/academics/administrative_acceptance.py`; new `apps/academics/management/commands/administratively_accept_faculty_assignments.py`; and new `apps/academics/tests_administrative_acceptance.py`.
- Validation: after the final MariaDB-oriented lock-query cleanup, the definitive feature suite passed 72/72 in 129.505s, including four `TransactionTestCase` stale-state/repeat-execution cases (an earlier completed run also passed 72/72 in 144.036s). The broader `apps.academics` package passed 78/78 in 153.494s. Existing assignment workflow passed 3/3 in 2.674s; the four exact Faculty Portal acceptance/clarification/undo/expiry methods passed 4/4 in 4.243s; combined Stage 5 Contributions plus Planning & Readiness passed 71/71 in 224.552s; and Admin/Faculty help guides passed 24/24 in 21.154s. Every test used a temporary Django test database. One initial focused attempt hit its 120-second command timeout and is not credited. `python -m compileall` passed; final `python manage.py check` reported zero issues; `makemigrations --check --dry-run` reported no changes; read-only `migrate --plan --no-color` exited zero with the isolated worktree's 808-line unapplied chain and applied nothing; `git diff --check` exited zero with only LF-to-CRLF warnings on tracked edited files. The full tracked diff and all three untracked files were read; the index remained empty, branch/HEAD remained exact, and the final status contained only the eleven authorized paths.
- Pending/risks/next step: no safe disposable MariaDB configuration exists in the checkout, so live/disposable MariaDB concurrency verification was not available; SQLite `TransactionTestCase` coverage validates locked-state rebuild behavior but not live InnoDB scheduling. The independent review verdict remains C until the requested independent read-only re-review. Authenticated operational command rehearsal against a disposable production-shaped database, the full Faculty Portal package, and full repository suite were not run. Do not execute against a real database without the approved hash, authority review, reason, and readiness-anomaly decision. No normal database mutation, migration application, staging, commit, push, deployment, restart, log edit, secret access, or source-worktree edit occurred.

### Question Bank Presentation Rename
- Date/gate/baseline: 2026-08-21; implementation plus focused validation in `D:\teachermateplus-monday-faculty-print`, branch `feat/faculty-questionnaire-print-release`, unchanged HEAD `887db2fbdc39e43c41843528eb3c90738443d276`. The initial rename implementation began clean; the final terminology remediation began from the exact independently reviewed dirty path set. No commit, push, deployment, restart, or normal-database migration application was authorized or performed.
- Completed: renamed only the Faculty user-facing module presentation from `Question Contributions` to exact Title Case `Question Bank` in the database-backed menu label, landing browser title/H1, workspace/Answer Key/Personalized Answer Sheet breadcrumbs, return actions, and Faculty Help Guide topic/opening instruction. `Open workspace`, Contribution Workspace, contribution deadline/window/status/roster/progress, Contributor Completion, Prepare Faculty Contributions, Draft/Blocked/Submitted, Final Submission, and other workflow terminology remain unchanged. Internal navigation/route/app/permission/model/related-name/table/help codes remain unchanged.
- Migration: new additive `apps/navigation/migrations/0025_rename_faculty_question_bank_menu.py` depends on `navigation.0024`. Forward and reverse resolve the exact `FACULTY` / `DEPARTMENTAL_EXAMS` group plus `DE_EXAM_FACULTY_CONTRIBUTIONS` item and expected source label, then update only `MenuItem.label`; wrong-group or unexpected-label rows are untouched. Schema impact is none, data impact is one Faculty navigation label, and no backfill exists. Historical `navigation.0018` is unchanged.
- Changed files: `CHANGE_LOG.md`; `HANDOFF.md`; `TEACHERMATEPLUS_CONTEXT.md`; new `apps/navigation/migrations/0025_rename_faculty_question_bank_menu.py`; new `apps/departmental_exams/tests_question_bank_menu_rename.py`; `apps/departmental_exams/tests_stage5_views.py`; `apps/departmental_exams/tests_answer_key_release.py`; `apps/departmental_exams/tests_personalized_answer_sheets.py`; `apps/faculty_portal/help_guide.py`; `apps/faculty_portal/tests_help_guide.py`; `templates/departmental_exams/faculty/contribution_list.html`; `contribution_workspace.html`; `answer_key.html`; `personalized_answer_sheet_overview.html`; and `error.html`.
- Validation: the final `Question Bank` remediation rerun discovered 104 focused tests and completed 103 passes, one expected SQLite row-lock capability skip, and zero failures in 455.824s: migration 3/3, Faculty Help Guide 5/5, Stage 5 Faculty views 28/28, Answer Key/Checking Master 25/25, Personalized Answer Sheets 14 passed plus one skipped, and Questionnaire Print Release 28/28. The earlier independent review reproduced two documented pre-existing stale exact-set compatibility failures in `tests_migration_safety.test_navigation_seed_is_idempotent_and_reverse_is_exactly_scoped` and `tests_stage4_protection.test_stage5_reuses_permissions_and_adds_only_approved_menu_surfaces`; they reject later committed Departmental Exam permissions/menu links, are unchanged by this wording-only remediation, and remain non-blocking. `git diff --check` exited zero with line-ending notices only; `python -B manage.py check` reported zero issues; `makemigrations --check --dry-run` reported `No changes detected`; read-only `migrate --plan --no-color` exited zero, included only `navigation.0025_rename_faculty_question_bank_menu`, and excluded the superseded `navigation.0025_rename_faculty_exam_question_bank_menu`. The normal development database remained unapplied.
- Pending/risks/next step: authenticated browser smoke, MariaDB, and the full repository suite remain unperformed. The two stale legacy exact-set tests remain a separately scoped test-maintenance issue. Next perform an independent focused diff/UI review before any separately authorized staging or commit gate; migration `navigation.0025` remains unapplied to the normal development database.
